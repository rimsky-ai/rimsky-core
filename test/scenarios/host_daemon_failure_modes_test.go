...
func TestHostDaemonNotConnected(t *testing.T) {
	...
	fx.waitForNodeEventKind(t, iid, "terminal/error/host_daemon_not_connected")
}

func TestBindingNotFound(t *testing.T) {
	fx := newHostDaemonFixture(t, fixtureOpts{withDaemon: true, blindProxy: true})
...
func TestHostDaemonDisconnectMidDispatch(t *testing.T) {
	fx := newHostDaemonFixture(t, fixtureOpts{withDaemon: true})
	...
	fx.waitForNodeEventKind(t, iid, "terminal/error/host_daemon_disconnected")
}
...
func TestProxyReconnectAfterDaemonRestart(t *testing.T) {
	fx := newHostDaemonFixture(t, fixtureOpts{withDaemon: true})

	fx.cancelDaemon()
	<-fx.daemonDone

	cancel, done, statusFile := startDaemon(t, fx.proxyAddr, fx.proxyCAPath,
		daemonStartOptions{APIKey: fx.adminKey})
	t.Cleanup(func() {
		cancel()
		<-done
	})
