...
const demoScriptRelPath = "test/fixtures/demos/host-daemon-control-plane-demo.sh"

var connectedLineRE = regexp.MustCompile(`rimsky daemon started \(pid (\d+), connected to [^)]+\)`)

func TestHostDaemonControlPlaneDemo(t *testing.T) {
	binPath := filepath.Join(t.TempDir(), "rimsky")
	proxyBin := filepath.Join(t.TempDir(), "rimsky-host-daemon-proxy")
	...
	buildRepoBinary(t, "./cmd/rimsky-host-daemon-proxy", proxyBin)
	...
	out, code := runHostDaemonDemoScript(t, scriptPath, binPath, proxyBin, migrateBin, controlAPIBin)
	if code != 0 {
		t.Fatalf("host-daemon-control-plane-demo.sh exited %d (want 0)\noutput:\n%s", code, out)
	}
	...
	if match == nil {
		t.Fatalf("demo did not print `rimsky daemon started (pid N, connected to ADDR)`; output:\n%s", out)
	}

	if !strings.Contains(out, "rimsky daemon: connected") {
		t.Fatalf("demo did not exhibit a `connected` status; output:\n%s", out)
	}

	if !strings.Contains(out, "rimsky daemon: stopped (pid "+match[1]+")") {
...
func TestHostDaemonControlPlaneDispatchReap(t *testing.T) {
	fx := newHostDaemonFixture(t, fixtureOpts{withDaemon: false})
	...
	if startCode != 0 {
		t.Fatalf("rimsky daemon start exited %d\nstdout:\n%s\nstderr:\n%s", startCode, startOut, startErr)
	}
	if !strings.Contains(startOut, "connected to "+fx.proxyAddr) {
		t.Fatalf("rimsky daemon start did not report `connected to %s`; stdout:\n%s", fx.proxyAddr, startOut)
	}

	pidRaw, err := os.ReadFile(filepath.Join(stateDir, "daemon.pid"))
	if err != nil {
		t.Fatalf("read daemon.pid: %v", err)
	}
	daemonPid, err := parseDaemonPid(string(pidRaw))
	if err != nil {
		t.Fatalf("parse daemon.pid: %v", err)
	}
	...
	if len(preStopChildren) == 0 {
		t.Fatal("no stubchild processes alive after dispatch — the daemon never spawned the child")
	}

	stopOut, stopErr, stopCode := runCmdWithEnv(t, binPath,
		[]string{"daemon", "stop", "--state-dir", stateDir},
		startEnv)
	if stopCode != 0 {
		t.Fatalf("rimsky daemon stop exited %d\nstdout:\n%s\nstderr:\n%s", stopCode, stopOut, stopErr)
	}
	if !strings.Contains(stopOut, fmt.Sprintf("stopped (pid %d)", daemonPid)) {
		t.Fatalf("rimsky daemon stop did not report `stopped (pid %d)`; stdout:\n%s", daemonPid, stopOut)
	}
	...
	if _, statErr := os.Stat(termLog); statErr != nil {
		t.Fatalf("stubchild term log %s missing — the daemon did not signal the spawned child during stop: %v",
			termLog, statErr)
	}
}

func runHostDaemonDemoScript(t *testing.T, scriptPath, binPath, proxyBin, migrateBin, controlAPIBin string) (string, int) {
	t.Helper()
	...
	if !errors.As(err, &exitErr) {
		t.Fatalf("host-daemon-control-plane-demo.sh fork error: %v\noutput:\n%s", err, combined)
	}
