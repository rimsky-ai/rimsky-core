...
- **Callback/dispatch DRY extraction (from finding 1502 fix, 2026-07-14).** `lib/runtime/callback.go::recoverResolvedAttributes` duplicates the fill→defaults→overrides resolution sequence of `lib/runtime/runner_dispatch.go::resolveAttributes` — the 1502 fix could not touch runner_dispatch.go (concurrently edited by the 1594 agent), so it copied the orchestration rather than sharing it. Both files are now settled; extract a shared pure core both call (Plumbline strict-DRY). Root module only; no external dependents.
