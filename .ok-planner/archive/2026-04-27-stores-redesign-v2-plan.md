...
**Tech Stack:** Go 1.21+ (root module `github.com/rimsky-ai/rimsky-core`; `go.mod` is at the repo root, NOT under `core/`), pgx/v5, postgres 15, testcontainers-go (real postgres for scenario tests), stdlib `log/slog` (no Zap/Zerolog — see lint rules), go-chi/chi, robfig/cron/v3, JSON Schema (santhosh-tekuri/jsonschema/v5).
