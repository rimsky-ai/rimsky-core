...
**Deviation:** Each cmd binary now imports `pgpersist "github.com/fallguy/rimsky/core/persistence/postgres"` directly (replacing the previous bare-`_` driver-registration import). `pgpersist.PoolFromDriver(driver)` extracts the pool for the supervisor / scheduler / controlapi pre-Task-26 pool requirement.
