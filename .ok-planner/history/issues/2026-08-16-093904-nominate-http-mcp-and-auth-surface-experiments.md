---
issue: nominate-http-mcp-and-auth-surface-experiments
kind: audit
category: test
artifacts:
  - concept:conformance
status: promoted
opened: 2026-08-16T09:39:05Z
sprint: 2026-08-18-recommended-intake-drain.md
---

# Nominate the 24 audit experiments over the HTTP routes, the MCP transport, and the api-key and permission surface for the project's suites

The audit measured user assumptions about the HTTP routes, the MCP transport, and the api-key and permission surface by building 24 experiments. Each experiment is a self-contained probe that boots its own containers from the tree's image tag, picks free ports, and exits non-zero when a check fails. All 24 pass at the audited tree: 1 shows the product honouring a prior (held), and 23 show it contradicting one (trap). The project's suites hold none of them. The audit adopts nothing itself and re-runs the probes only at the next audit. A held probe is a candidate ordinary regression test. A trap probe is a candidate expected-fail test that starts failing when someone closes the trap. The decision on scenario tests fixes how such tests are written, not which probes are promoted. The ruling decides the adoption policy for this family, and one policy should cover the four sibling families.

## Options

- Adopt held probes as scenario tests and trap probes as expected-fail scenario tests, and retire each experiment directory as its test lands; cost: 24 maintained tests, each rewritten to the scenario harness.
