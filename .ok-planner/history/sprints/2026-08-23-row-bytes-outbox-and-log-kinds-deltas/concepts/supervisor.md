...
Executor name resolution carries the dispatch's instance and run-scope identity, so a resolver can answer per instance: the late-bind resolver consults the instance's service bindings, and the address-book resolver ignores the extra context. The supervisor stages the run-scope-terminal lifecycle delivery for each child run scope it closes, in the transaction that closes it (see `concept:lifecycle-subscriber`). Every outbound service dial carries the originally requested service name, so a `concept:host-daemon-proxy` fronting a protocol can route the call by that name.
