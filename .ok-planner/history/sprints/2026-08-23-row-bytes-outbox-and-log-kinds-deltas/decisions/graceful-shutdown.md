...
host-daemon proxy's two gRPC listeners, and the supervisor's wait for
...
An operator tunes one window: the host daemon's wait for a spawned child to
...
thirty seconds gives the drain a genuine chance to complete. The host-daemon
proxy takes that window too. It ships as one of the four distributed core
images, and the calls it drains are dispatches bound for a host daemon, not a
...
daemon's child window is the exception: the operator, not rimsky, knows how
