...
- Every bundled service that dials a caller-supplied destination — a URL reaching it through node attributes or a subscription's config — routes that destination through the outbound-egress SSRF guard. A destination the operator sets instead of the caller sits outside the guard's trust class. The guard is a property of the bundled images, not of rimsky core; core keeps its inertness guarantee.
