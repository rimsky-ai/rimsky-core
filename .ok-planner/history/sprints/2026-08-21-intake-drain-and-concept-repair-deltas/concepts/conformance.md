...
Conformance is a runnable battery of checks that proves an independently written service matches one of rimsky's peer protocols. The author points a battery at a running service. The battery drives the protocol's operations against that service and reports each check as passed or failed. Rimsky carries one battery per peer protocol a service author can implement, plus one for rimsky's pluggable storage interface, which a battery exercises by constructing an implementation in process instead of dialing a service. Every battery is reachable two ways: as a command a service author in any language runs against an endpoint, and as an importable library a Go service author calls from a test against a target the test hosts itself. The batteries ship with rimsky rather than as a separate distribution.
