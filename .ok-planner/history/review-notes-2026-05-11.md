...
- `node-state` vs `last-outcome`: Defensible split — they are two columns on the same row, but load-bearing safety logic gates on each independently (dispatch eligibility reads `state`; cascade firing reads `last_outcome`). A merged `node-row-state` concept would also be defensible.
...
- `claimant-guarded`: Mentioned with backticks as if a noun in `concepts/lifecycle-handler.md` "claim release (see `auto-terminal`, `claimant-guarded`)". This is an invariant pattern (claimant-guarded release), not a concept. Could be promoted to a tiny invariant-shaped concept, but more naturally a backtick-stripped inline term.
