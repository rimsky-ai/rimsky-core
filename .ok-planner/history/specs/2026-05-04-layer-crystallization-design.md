...
- **`github.com/rimsky-ai/rimsky-core/foundation`** — cascade engine, lock manager, integration, foundation persistence contract. Depends on stdlib + `protocols`.
- **`github.com/rimsky-ai/rimsky-core/protocols`** — `ClaimProducer`, `Executor`, `LifecycleSubscriber` Go interface types and protobuf-generated bindings. Depends on stdlib only. External service authors import this module to write a custom service implementation without pulling rimsky's transitive deps.
- **`github.com/rimsky-ai/rimsky-core`** (root) — modeling layer (templates, instances, frames, schedules, attributes, control-plane API), cmd binaries, bundled service reference implementations (`stores/*`, `executors/*`). Depends on `foundation` + `protocols`.
