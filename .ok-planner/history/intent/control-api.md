...
- All HTTP routes live under a /v1/ prefix, including the executor async-callback URL and the observability sub-router; old bare paths were removed with no transition window (2026-06-08, corpus-bootstrap, artifact; supersedes the earlier bare-path posture). **User ruling 2026-07-17: the version prefix is a wire convention, not design.** The /v1 segment exists for user-facing future control-api back-compat; rimsky's own components carry no internal back-compat obligation and may track the latest vN when versions eventually multiply. Design docs neither commit to a particular version, nor demand, nor forbid a prefix — URL versioning is below the concept layer and stays out of concept bodies.
...
- External IdPs are out of scope by design — rimsky's surface is the API-key floor (2026-05-15 / 2026-05-24, artifacts). SUPERSEDED 2026-07-16 for the process-to-process half: rimsky now has optional internal service↔service auth via mTLS (`peer_auth: none|mtls`, default `none`). The control plane hosts the per-deployment CA and serves POST /v1/enroll, where a `service:enroll`-bearing key exchanges for a short-lived cert; under `mtls` startup fails closed if the CA encryption key is missing/malformed. Edge IdP termination stays out of scope (2026-07-16, peer-auth-posture, transcript; see the peer-auth dossier).
...
- **RESOLVED 2026-07-14 (user ruling, transcript tier): at any teardown that finds open frames, ALL open run scopes close and fire.** User: "would have to be B, right? otherwise nothing cleans up the claims. but this should be an emergency abort scenario — normally an instance should be left to finish its current frame and then be removed." Consequences: (a) the normal terminate path stays graceful per 2026-07-06 — the running frame finishes, pending messages are cancelled, no new frames open; by the time the instance stops nothing is open and scope fanout is a no-op; (b) an emergency abort that cuts live frames must close every open run scope in each frame's tree (children before parents), fire OnRunScopeTerminal once per scope, then OnInstanceTerminated — roots-only fanout (current code) is a defect: in-flight nested scopes (sub-graph, fan-out partitions) die unfired and their claims never clean up; (c) every scope that opens fires terminal exactly once is the invariant lifecycle subscribers may rely on; (d) verb taxonomy reaffirmed: instance terminate (graceful, frame finishes, row persists) ≠ template undeploy (refuses new instances) ≠ instance delete (removal after terminal).
- **RESOLVED 2026-07-14 (user ruling, transcript tier): ratify the capability, not the shape.** Required intent is wedged-instance diagnosis: an operator can query parked runs, held frames, wait-set edges, and claim holders. The current /v1/admin/diagnostics/* routes stand as the implementation of record; findings demanding May-era route specifics (exact paths, ?reason= filter shapes) are refuted; findings showing a capability gap are defects. Corollaries: (a) the duplicate parked route (`/admin/diagnostics/parked-nodes` + alias `/diagnostics/parked`) collapses to one route — the alias is residue; (b) the parked `?reason=` filter/grouping degrades to trivial under the 2026-07-14 snooze-only ParkReason ruling and may be dropped with that retirement.
- **RESOLVED 2026-07-14 (user ruling, transcript tier): remove the nil-permissive gate branch.** AuthState becomes required — a nil AuthState is a Go zero-value accident, not a designed state, and must fail loudly at construction/serve rather than run the API ungated; tests construct a small permissive-but-real AuthState. Rationale: opt-in/zero-config auth is anonymous-mode's job (data-derived, synthetic admin identity, audited, auto-flips on first key mint) and it requires a live AuthState to evaluate; the nil branch looked like "auth off" while lacking every safeguard the real auth-off has (no audit identity, no warnings, no flip when keys exist, auth routes unregistered). User: "the branch should be removed... if we are providing a synthetic default auth, how is it even possible to be nil?" Note: the registry-coverage guard the record listed as never-implemented EXISTS today (TestRegistryCoversRouter walks the router and asserts every route is registry-covered) — the no-exemption invariant is guarded on both fronts once the nil branch is gone.
