...
- **Substitution stringifies, never quotes** (user ruling 2026-07-17): the claim-scope substitution path inserts a JSON-string scope as its unquoted text (raw JSON text for any other form) — the substitution grammar's general stringification convention, shared with the address path. "Verbatim bytes" was imprecise doc wording, not a promise; the unquoting is intended.
- **Empty-open fallback is intended** (user ruling 2026-07-17): rimsky seeds the claim-handle row with its JSON-marshaled resolved selector before open; when open returns claim scope bytes they replace the seed verbatim, and when open returns an empty scope the seeded selector bytes stay — the request stands in for the unreported acquisition so audit and conflict detection never see a blank scope. Not a bug; the audit-trail-honesty rationale is qualified accordingly.
