...
- Subscriptions are **desired-state rows** with lifecycle `mounting → active | failed | stopped`: instance-create inserts `mounting` and returns fast; a reconciliation worker drives Subscribe at a fixed reconcile interval with **no attempt cap** (user ruling 2026-07-17: the "with backoff" clause that previously rode this entry was distillation drift — its sole artifact source evidences only the no-attempt-cap property; the flat-interval retry-forever behavior is ratified, with per-row backoff explicitly not required); `failed` is reserved for non-retryable errors; per-subscription state is visible on the instance-detail surface (2026-06-11).
