...
- The agent→proxy hop (dev-machine→deployment) is over TLS, verifying the proxy's server cert against a PINNED deployment-CA root, with the user's api-key inside the encrypted channel; the agent gets NO client cert on that hop (it is per-user session tooling, not a service enrolled in the deployment CA). The agent↔spawned-child loopback is secured by mandatory, always-on mutual mTLS: the agent runs a self-contained LOCAL enrollment authority — its own CA via the same pki machinery peer-auth uses, a trust domain separate from the deployment's peer_auth CA, minting no ledger keys and needing no rimsky permission — issues itself a leaf, and serves a plaintext bootstrap enroll endpoint (POST /v1/enroll). Per spawn it mints a fresh bootstrap token and provisions the child with RIMSKY_PEER_AUTH=mtls, RIMSKY_API_KEY=<token>, RIMSKY_CONTROL_API_URL=<the agent's local enroll base>; the child self-enrolls via the unchanged bundled peer-auth path, and both loopback legs (dispatch and callback) run mutual mTLS against the local CA across two local listeners (plaintext enroll vs mTLS callback). A port-squatter or plaintext-only binary has no local-CA cert and fails the handshake, closing forged-dispatch, forged-callback, and dispatch-interception uniformly — so a plaintext-only late-bound binary is no longer a valid executor (a deliberate pre-v1 contract change). Because the CA is self-contained, the loopback is secured independently of the deployment's peer_auth posture. This supersedes the earlier per-spawn shared-secret sketch, which was an ad-hoc reimplementation of the proof-of-possession the TLS handshake already provides (2026-07-16, peer-auth-posture, transcript; see the peer-auth dossier).
...
- All five protocols late-bindable through the proxy — executor, claim-producer, publisher, validation, data-processing — no protocol left as a gRPC Unimplemented stub (2026-06-08, corpus-bootstrap, artifact). SUPERSEDED 2026-07-14: user ruling narrows the late-bind surface to executor + claim-producer; other kinds are rejected loudly at registration/config (see host-agent-proxy dossier).
