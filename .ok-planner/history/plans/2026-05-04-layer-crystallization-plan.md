...
**Architecture:** Three Go modules — `github.com/rimsky-ai/rimsky-core/foundation` (cascade engine + lock manager + integration + persistence contract), `github.com/rimsky-ai/rimsky-core/protocols` (ClaimProducer/Executor/LifecycleSubscriber Go interfaces and protobuf bindings, stdlib-only deps), and the root module `github.com/rimsky-ai/rimsky-core` (modeling layer, cmd binaries, bundled service reference impls). Coordinated by `go.work`. `rimsky_dispatch` and `rimsky_lock_holders` collapse to `rimsky_worker_request` + `rimsky_claim_handle` with an `is_held` flag and a `phase` column expressing the active+held lifecycle.
...
4. Apply the 8 deltas listed in spec §5:
   ...
   e. **Module split settled** — update §11.5 to: "Foundation is its own Go module at `github.com/rimsky-ai/rimsky-core/foundation`. Rationale and implementation details in spec `2026-05-04-layer-crystallization-design.md` §4.2."

   f. **Implementation-status section retired** — replace §10 (the current-code-locations dump) with a single short paragraph: "The code in `github.com/rimsky-ai/rimsky-core/foundation` matches this contract. Cross-references from foundation packages to specific files are not maintained in this contract; see `@blessed-invariant N` annotations in source for current code locations."
...
1. Create `docs/specs/2026-05-04-service-protocol-contract.md` with header:
   ...
   **Layer position:** The protocols module (`github.com/rimsky-ai/rimsky-core/protocols`) carries Go interfaces and protobuf bindings; foundation calls a subset (claim verbs, executor dispatch); modeling calls a subset (lifecycle hooks).
   ```
...
1. Create `protocols/go.mod` with stdlib + grpc + protobuf only:
   ```
   module github.com/rimsky-ai/rimsky-core/protocols
...
4. Update import paths inside the moved files. Every file that previously imported `github.com/rimsky-ai/rimsky-core/core/persistence/...` now needs to import `github.com/rimsky-ai/rimsky-core/foundation/persistence/...`. Use:
   ```sh
   grep -rln 'github.com/rimsky-ai/rimsky-core/core/persistence' foundation/ | while read f; do
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/persistence|github.com/rimsky-ai/rimsky-core/foundation/persistence|g' "$f"
   done
   ```
...
! grep -rn 'github.com/rimsky-ai/rimsky-core/core/persistence' foundation/  # should print nothing
```
...
! grep -rn 'github.com/rimsky-ai/rimsky-core/core/store' foundation/  # foundation should not reference core/store paths
! grep -rn 'github.com/rimsky-ai/rimsky-core/core/persistence' foundation/  # already verified in Task 9
```
...
2. Update the `option go_package` line in each `.proto` file to reflect the new module path. Example for `executor.proto`:
   ```diff
   - option go_package = "github.com/rimsky-ai/rimsky-core/proto/v1/gen;rimsky_v1";
   + option go_package = "github.com/rimsky-ai/rimsky-core/protocols/proto/v1/gen;rimsky_v1";
   ```
...
5. Update import paths in all Go files that previously imported the proto bindings:
   ```sh
   grep -rln 'github.com/rimsky-ai/rimsky-core/proto/v1/gen' . \
     --include='*.go' | grep -v 'protocols/proto/' | grep -v 'docs/history/' \
     | while read f; do
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/proto/v1/gen|github.com/rimsky-ai/rimsky-core/protocols/proto/v1/gen|g' "$f"
   done
   ```
...
! grep -rn 'github.com/rimsky-ai/rimsky-core/proto/v1' . \
...
1. Globally rewrite the four import paths the foundation move broke:
   ...
   grep -rln 'github.com/rimsky-ai/rimsky-core/core/persistence' core/ stores/ executors/ test/ \
     --include='*.go' | while read f; do
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/persistence|github.com/rimsky-ai/rimsky-core/foundation/persistence|g' "$f"
   done

   # 1b. core/store types/conflict → foundation/locks
   grep -rln '"github.com/rimsky-ai/rimsky-core/core/store"' core/ stores/ executors/ test/ \
     --include='*.go' | while read f; do
     sed -i '' 's|"github.com/rimsky-ai/rimsky-core/core/store"|"github.com/rimsky-ai/rimsky-core/foundation/locks"|g' "$f"
   done

   # 1c. core/store/remote → foundation/integration/remote
   grep -rln 'github.com/rimsky-ai/rimsky-core/core/store/remote' core/ stores/ executors/ test/ \
     --include='*.go' | while read f; do
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/store/remote|github.com/rimsky-ai/rimsky-core/foundation/integration/remote|g' "$f"
   done

   # 1d. core/store/storetest → foundation/locks/storetest (if Task 10 moved it there)
   grep -rln 'github.com/rimsky-ai/rimsky-core/core/store/storetest' . \
     --include='*.go' --exclude-dir=docs/history | while read f; do
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/store/storetest|github.com/rimsky-ai/rimsky-core/foundation/locks/storetest|g' "$f"
   done
   ```
...
4. Run the full pipeline and resolve any remaining errors:
   ...
   - `core/store` package missing — already moved away. Any leftover `import "github.com/rimsky-ai/rimsky-core/core/store"` in code outside `core/store/` is a missed substitution; re-run the step-1 sed against the affected files.
...
1. Update import paths globally:
   ```sh
   grep -rln 'github.com/rimsky-ai/rimsky-core/core/' . \
     --include='*.go' --exclude-dir=docs --exclude-dir=foundation --exclude-dir=protocols \
     | while read f; do
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/cmd|github.com/rimsky-ai/rimsky-core/cmd|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/attributes|github.com/rimsky-ai/rimsky-core/modeling/attribute|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/canonical|github.com/rimsky-ai/rimsky-core/modeling/template/canonical|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/controlapi|github.com/rimsky-ai/rimsky-core/modeling/controlapi|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/frame|github.com/rimsky-ai/rimsky-core/modeling/frame|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/observability|github.com/rimsky-ai/rimsky-core/modeling/observability|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/qualityrule|github.com/rimsky-ai/rimsky-core/modeling/qualityrule|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/executor|github.com/rimsky-ai/rimsky-core/modeling/executor|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/cli|github.com/rimsky-ai/rimsky-core/modeling/cli|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/config|github.com/rimsky-ai/rimsky-core/modeling/config|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/scheduler|github.com/rimsky-ai/rimsky-core/modeling/scheduler|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/shared|github.com/rimsky-ai/rimsky-core/modeling/shared|g' "$f"
     sed -i '' 's|github.com/rimsky-ai/rimsky-core/core/internal|github.com/rimsky-ai/rimsky-core/modeling/internal|g' "$f"
   done
   ```
...
make lint   # exit 0
! grep -rn 'github.com/rimsky-ai/rimsky-core/core/' . \
  --include='*.go' --exclude-dir=docs --exclude-dir=foundation --exclude-dir=protocols
test ! -d core
```
   ...
   - Add a new depguard rule that prevents the modeling layer from importing `foundation/internal/`:
     ...
     foundation-internal-isolation:
       ...
       files:
         ...
         - "!**/foundation/**"
       deny:
         - pkg: "github.com/rimsky-ai/rimsky-core/foundation/internal"
...
1. Update `CLAUDE.md`:
   ...
   - "Package import rules" section — completely rewrite. The new rules:
     ```markdown
     - `foundation/` — own Go module (`github.com/rimsky-ai/rimsky-core/foundation`). Cascade engine + lock manager + integration + foundation persistence. Depends on `protocols` + stdlib.
     - `protocols/` — own Go module (`github.com/rimsky-ai/rimsky-core/protocols`). ClaimProducer / Executor / LifecycleSubscriber Go interfaces + protobuf bindings. Stdlib + grpc + protobuf only.
...
1. Update foundation/integration/ to import `github.com/rimsky-ai/rimsky-core/protocols/claimproducer` instead of the old `core/store` interface.
...
1. Update for the new module layout: external authors import `github.com/rimsky-ai/rimsky-core/protocols/executor` only.
...
2. Rewrite the body:
   ...
   - Import path: `github.com/rimsky-ai/rimsky-core/protocols/claimproducer` (and `protocols/lifecycle` if implementing both).
