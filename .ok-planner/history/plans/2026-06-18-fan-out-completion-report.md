...
### Intent drifts
None. The seven manifest stories are exhibited verbatim by their proof artifacts (cross-checked Proof field vs. demo.sh body for each). Files outside the manifest carrying `@story:` annotations modified in the diff (`code:lib/graph/attribute/substitution_test.go`, `code:lib/graph/node/subscription_edges.go`, `code:lib/graph/node/template_validator.go`, `code:lib/graph/node/template_validator_substitution_coverage_test.go`, `code:lib/runtime/runner_acquire_helpers_test.go`, `code:lib/runtime/supervisor.go`, plus several scenario tests) annotate `typed-message-substitution`, `uncovered-substitution-rejected`, `empty-message-wakes-roots`, `cascade-emit`, etc. — pre-existing stories untouched by this spec; their `Proof:` obligations remain satisfied.
