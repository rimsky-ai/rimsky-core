...
# the `auth` block that the webhook publisher takes (`hmac`, `secret_header`, `none`) is available on every publisher kind, so an HTTP-poll sensor can present credentials to its upstream the same way.
...
declare an `auth` block on an `http` publisher in a template and see whether registration accepts it.
