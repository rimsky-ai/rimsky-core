---
assumption: cli-duration-flags-share-syntax
commit: PENDING
disposition: trap
synthesized: 2026-08-16T05:48:16Z
---

# every duration-shaped flag and config key (`--timeout`, `--grace`, `--poll-interval`, `--older-than`, `--expires`, `dispatch_defaults.*`, `retention.*`) accepts the same duration grammar, e.g. `30s`, `5m`, `24h`.
...
pass `30s` and `5m` to each duration flag and config key and check that all parse identically.
