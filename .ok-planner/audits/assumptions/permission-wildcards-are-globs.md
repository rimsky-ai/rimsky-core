...
# the wildcard vocabulary behaves like ordinary globs, so `instance:re*` matches `instance:read` and `instance:resume`, and `template:*` and `*:read` also work.
...
submit grants using an infix wildcard and a prefix-partial wildcard and record whether key creation accepts them.
