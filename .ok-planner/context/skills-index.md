---
name: ok-planner
description: "ONLY activated by explicit slash command (/plan-sprint, /execute-sprint, /certify, /sketch, /audit, /prove, /discover-design, /true-up, /ok-version). Never auto-triggered by conversation content."
---
...
| `ok-planner:execute-sprint` | User types `/execute-sprint`. A thin launcher, not the builder: resolves the sprint under `sprints/`, assembles the unsupervised work-to-completion framing, and fires the native `goal` mechanism at it so the Stop hook drives the build to completion; the executing agent stages the work and dispatches subagents freely, closing with `/certify`. Returns ok-planner to an agent-with-subagents orchestrator working straight from the one sprint document — no spec/plan split, no `Workflow` engine. Stops once the goal is fired. |
| `ok-planner:certify` | User types `/certify`, or it is fired as the terminal step of a goal-driven run. The "am I done?" gate: aligns the working-tree change to its sprint (undershoot caught), runs `/prove` and `/audit`, runs the code-review and design-doc-compliance cycles, and drives every fixable finding to clean through a no-discretion fix loop (fixer subagent, cap 3, no orchestrator triage). Judgment findings are presented, never auto-fixed; audit's file to `issues.jsonl`. Presents outcomes and divergences to the owner, and archives the sprint once certified clean. |
