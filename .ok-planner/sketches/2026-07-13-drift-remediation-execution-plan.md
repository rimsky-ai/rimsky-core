...
**Test-determinism track (ruled 2026-07-14, user):** flakes are defects, never tuned away (rule in `.claude/rules/rules.md` "Tests Are Deterministic"). Phase 2 work items: (a) a mechanical lint ban on `time.Sleep` and bare `time.Now()` in `_test.go` files (custom check or golangci forbidigo rule); (b) a CI stress gate that runs the suite under artificial load / `-race -count=N` so timing-fragile tests *surface* rather than hide; (c) a flake-hunt sweep of existing timing-fragile tests, starting with the confirmed offender `TestDispatchClaimProducerUnary` (hostagent) — replace its wall-clock dispatch deadline with synchronization on the frame-arrival signal, failsafe timeout only. Each converted test must pass under the stress gate.
