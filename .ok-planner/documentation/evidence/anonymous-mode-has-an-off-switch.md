---
trap: anonymous-mode-has-an-off-switch
release: d977250c
---
# Evidence set — anonymous mode can be turned off by configuration, so an operator can guarantee a fresh deployment never admits an unauthenticated request.

Source of the prior: ecosystem-prior — bootstrap-open modes in control planes normally carrying a disable flag
...
## Experiment record (experiment:assumption-anonymous-mode-has-an-off-switch)
...
Runnables: `src:.ok-planner/experiments/assumption-anonymous-mode-has-an-off-switch/` at the stamped commit.
