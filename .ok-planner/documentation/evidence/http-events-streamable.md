...
release: d977250c
---
# Evidence set — `GET /v1/events` supports a streaming/follow mode (SSE or long-poll), because the CLI offers `rimsky watch` and a `--follow` flag.

Source of the prior: name-promise — `rimsky watch`, `--follow`, `rimsky logs` over a thin HTTP client
...
## Experiment record (experiment:assumption-http-events-streamable)
...
Runnables: `src:.ok-planner/experiments/assumption-http-events-streamable/` at the stamped commit.
