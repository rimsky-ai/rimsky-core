...
class Upstream(JSONHandler):
    def do_GET(self):
        if self.path == "/thing":
            self.send_json(200, {"id": "abc-123", "count": 7, "nested": {"ok": True}})
        elif self.path == "/limited":
            STATE["limited_calls"] += 1
            if STATE["limited_calls"] == 1:
                self.send_json(429, {"error": "slow down"}, {"Retry-After": "2"})
            else:
                self.send_json(200, {"cleared": True, "attempt": STATE["limited_calls"]})
        elif self.path == "/err-operator":
            self.send_json(400, {"code": "quota_exhausted", "message": "no quota"})
        elif self.path == "/err-pernode":
            self.send_json(400, {"reason": "bad_shape", "code": "operator_field_ignored"})
        elif self.path == "/err-absent":
            self.send_json(400, {"detail": "nothing here names a class"})
        else:
            self.send_json(404, {"detail": "no route"})


def http_node(node_type, path, port, extra_attrs=None, node_extra=None):
    props = {"url": {"type": "string",
                     "default": "http://host.docker.internal:%d%s" % (port, path)}}
    for key, value in (extra_attrs or {}).items():
        props[key] = {"type": "string", "default": value}
    node = {"type": node_type, "executor": "http-node",
            "attributes": {"schema": {"type": "object", "properties": props}}}
    node.update(node_extra or {})
    return node


def terminal_kinds(tl, node_type):
    return [r["kind"] for r in terminals(tl, node_type)]


def main():
    port = serve_http(Upstream)
    boot_rimsky(env={"RIMSKY_EXECUTOR_HTTP_NODE_EGRESS_ALLOWLIST": PRIVATE_CIDRS,
                     "RIMSKY_EXECUTOR_HTTP_NODE_ERROR_CLASS_FIELD": "code"})

    spec = {
        "name": "exp-http-node",
        "version": "1",
        "nodes": [
            http_node("fetch", "/thing", port),
            http_node("limited", "/limited", port),
            http_node("err_operator", "/err-operator", port),
            http_node("err_pernode", "/err-pernode", port,
                      {"error_class_field": "reason"}),
            http_node("err_absent", "/err-absent", port),
        ],
    }
    ...
    check("the node issued the request and routed the JSON response into its output attributes",
          deltas(tl, "fetch") == [{"id": "abc-123", "count": 7, "nested": {"ok": True}}],
          json.dumps(deltas(tl, "fetch")))

    parks = [r for r in transients(tl, "limited") if r["kind"].startswith("transient/park")]
    check("a 429 parks the dispatch instead of failing it", len(parks) == 1,
          json.dumps([r["kind"] for r in transients(tl, "limited")]))
    check("the park is annotated as rate-limited",
          bool(parks) and parks[0]["payload"].get("tags") == ["rate_limited"],
          json.dumps(parks[0]["payload"]) if parks else "")
    check("the park honours the upstream's retry-after directive",
          bool(parks) and parks[0]["payload"].get("resume_at"),
          json.dumps(parks[0]["payload"].get("resume_at")) if parks else "")
    check("the parked dispatch resumes by itself and succeeds against the cleared upstream",
          deltas(tl, "limited") == [{"cleared": True, "attempt": 2}],
          json.dumps(deltas(tl, "limited")))
    check("the resume re-ran the same node exactly once more",
          len(starts(tl, "limited")) == 2, str(len(starts(tl, "limited"))))

    check("the operator-configured error-class field names the class on a 4xx body",
          terminal_kinds(tl, "err_operator") == ["terminal/error/http/request_invalid/quota_exhausted"],
          json.dumps(terminal_kinds(tl, "err_operator")))
    check("a per-node error-class field overrides the operator default",
          terminal_kinds(tl, "err_pernode") == ["terminal/error/http/request_invalid/bad_shape"],
          json.dumps(terminal_kinds(tl, "err_pernode")))
    check("an error body without the configured field falls back to a stable class",
          terminal_kinds(tl, "err_absent") == ["terminal/error/http/request_invalid/_unspecified"],
          json.dumps(terminal_kinds(tl, "err_absent")))
    ...
    port = serve_http(Upstream)
    boot_rimsky(env={"RIMSKY_EXECUTOR_HTTP_NODE_EGRESS_ALLOWLIST": PRIVATE_CIDRS})
    spec = {"name": "exp-http-node-optout", "version": "1",
            "nodes": [http_node("accepting", "/limited", port,
                                node_extra={"attributes": {"schema": {"type": "object", "properties": {
                                    "url": {"type": "string",
                                            "default": "http://host.docker.internal:%d/limited" % port},
                                    "expect_status": {"type": "array", "default": [200, 429]}}}}})]}
    iid = new_instance(deploy(spec))
    send_message(iid)
    tl = quiet(iid)
    show(tl)
    check("a node that accepts 429 as an expected status does not park",
          not [r for r in transients(tl, "accepting") if r["kind"].startswith("transient/park")]
          and terminal_kinds(tl, "accepting") == ["terminal/success"],
          json.dumps([r["kind"] for r in tl if r["node"] == "accepting"]))

    teardown()

    port = serve_http(Upstream)
    boot_rimsky()
    spec = {"name": "exp-http-node-guarded", "version": "1",
            "nodes": [http_node("blocked", "/thing", port)]}
    iid = new_instance(deploy(spec))
    send_message(iid)
    tl = quiet(iid)
    show(tl)
    check("without the operator egress opt-in the same private-address request is refused",
          terminal_kinds(tl, "blocked") == ["terminal/error/http/network_error"],
          json.dumps(terminal_kinds(tl, "blocked")))
    finish()
