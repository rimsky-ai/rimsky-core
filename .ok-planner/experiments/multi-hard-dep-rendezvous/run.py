...
SPEC = {
    "name": "exp-multi-hard-dep-rendezvous",
    "version": "1",
    "messages": [{"type": "never/sent", "body_schema": {"type": "object"}}],
    "nodes": [
        counter_node("trigger"),
        counter_node("left", [sub("never/sent", "terminal/success")]),
        counter_node("right", [sub("never/sent", "terminal/success")]),
        passthrough_node("receiver",
                         [sub("trigger", "terminal/success"),
                          sub("left", "attribute/count/changed", force=True),
                          sub("right", "attribute/count/changed", force=True)],
...
def main():
    ...
    tl = quiet(iid)
    show(tl)
    check("each force-refreshed upstream ran exactly once",
          len(starts(tl, "left")) == 1 and len(starts(tl, "right")) == 1,
          "left=%d right=%d" % (len(starts(tl, "left")), len(starts(tl, "right"))))
    check("the receiver dispatched exactly once", len(starts(tl, "receiver")) == 1,
          str(len(starts(tl, "receiver"))))
    check("the receiver computed from the full refreshed set",
          deltas(tl, "receiver") == [{"from_left": 1, "from_right": 1}],
          json.dumps(deltas(tl, "receiver")))
    check("the instance reached rest with no live runs", not live_runs(iid))
    finish()
