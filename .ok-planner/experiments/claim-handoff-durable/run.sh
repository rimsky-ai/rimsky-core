...
# alias. The run shows the claim-handle row surviving the dispatch as one
# committed durable row, a later dispatch of the same instance co-holding that
# same row, a competing instance refused while it is held, and the claim
# released only when the operator deletes it or terminates the instance.
set -uo pipefail
...
wake() {
  curl -sS -XPOST -H 'Content-Type: application/json' -H "Idempotency-Key: wake-$1-$2" \
    -d '{"type":""}' "$E/v1/instances/$1/messages" >/dev/null
}
wake_node() {
  curl -sS -XPOST -H 'Content-Type: application/json' -H "Idempotency-Key: waken-$1-$3" \
...
import sys, json
print(json.dumps([h for h in json.load(sys.stdin)['claim_handles']
...
echo "--- a later dispatch of the same instance co-holds a durable claim on the same address"
before="$(posts_for co-holder)"
wake "$A" 2
until [ "$(posts_for co-holder)" -gt "$before" ]; do sleep 0.3; done
ok "the instance dispatched its co-holder again ($before then $(posts_for co-holder) runs)"
[ "$(last_addr co-holder)" = "$addr1" ] \
  && ok "the later dispatch's co-holder read the same claim address by alias" \
  || bad "the later dispatch's co-holder read $(last_addr co-holder), not $addr1"
...
[ "$(count_rows "$A" /asset/one)" = 1 ] \
  ...
  || bad "the later dispatch left $(count_rows "$A" /asset/one) rows: the acquirer opened a second claim rather than reusing the durable one"
...
echo "--- terminating the instance is the operator action that releases it"
...
rel="$(printf '%s\n' "$(verbs_for /asset/one)" | grep -c '^Release$')"
echo "    verbs after termination: $(printf '%s' "$(verbs_for /asset/one)" | tr '\n' ' ')"
[ "$rel" -ge 1 ] \
  && ok "terminating the instance released the durable claim through the producer" \
  || bad "terminating the instance sent the producer no Release; the durable claim is still held"
rows="$(rows_for "$A" /asset/one)"
echo "    claim handles after termination: $rows"
G="$(start "$HERE/template-competitor.json" '{"selector":"/asset/one"}')" || bad "competitor template did not register"
st="$(settle "$G" competitor)"; echo "    $st"
has "competitor fresh=1 failed=0" "$st" "the scope another instance was refused is claimable once the holding instance terminates"
