...
ITEMS = [{"key": "item-1"}, {"key": "item-2"}, {"key": "item-3"}]
...
def spec():
    ...
    return {"name": "exp-fanout-list-array", "version": "1",
            "params_schema": {"type": "object", "properties": {"items": {"type": "array"}}},
            "nodes": [passthrough_node("producer", {"items": {"type": "array", "source": "{{params.items}}"}}),
                      node]}


def main():
    boot_with_filesystem_producer(["data"])
    res = docker("logs", STATE["container"])
    logs = res.stdout + res.stderr
    check("the only claim producer in play is one the image bundles, not one written for this run",
          "claim-producer-filesystem" in logs and "bundled claim producer registered in-process" in logs)

    iid = new_instance(deploy(spec()), params={"items": ITEMS})
    send_message(iid)
    tl = quiet(iid)
    show(tl)

    produced = deltas(tl, "producer")
    check("the upstream node produced the list the fan-out reads",
          produced == [{"items": ITEMS}], json.dumps(produced))
    split = payloads(tl, "subclaim.acquired")
    check("the producer split the parent claim into one sub-scope per list item",
          bool(split) and split[0].get("sub_scope_descriptor_count") == len(ITEMS), json.dumps(split))
    dispatched = payloads(tl, "fan_out_dispatched")
    check("one work unit was dispatched per item, keyed by the item's key",
          bool(dispatched) and dispatched[0].get("child_keys") == [i["key"] for i in ITEMS],
          json.dumps(dispatched))
    seen = [d.get("pk") for d in deltas(tl, "fan_parent") if d and d.get("pk")]
    check("each work unit ran against exactly its own item",
          sorted(seen) == [i["key"] for i in ITEMS], json.dumps(seen))
    summary = [n["run_summary"] for n in nodes(iid) if n["node_type"] == "fan_parent"]
