...
SPEC = {
    "name": "exp-empty-message-wakes-roots",
    "version": "1",
    "messages": [{"type": "never/sent", "body_schema": {"type": "object"}}],
    "nodes": [
        passthrough_node("root_one", None, {"v": {"type": "integer", "default": 1}}),
        passthrough_node("root_two", None, {"v": {"type": "integer", "default": 2}}),
        passthrough_node("root_three", None, {"v": {"type": "integer", "default": 3}}),
        passthrough_node("downstream", [sub("root_one", "terminal/success")]),
        passthrough_node("not_a_root", [sub("never/sent", "terminal/success")]),
    ],
}

ROOTS = ("root_one", "root_two", "root_three")
...
def main():
    ...
    check("an empty request body is accepted as a message", status == 201, str(out))
    message_id = out["message_id"]
    tl = quiet(iid)
    show(tl)

    for root in ROOTS:
        check("structural root %s woke" % root, len(starts(tl, root)) == 1)
    check("a node with an upstream was not woken directly", not starts(tl, "not_a_root"))
    check("the cascade below a root still ran", len(starts(tl, "downstream")) == 1)

    ledger = call("GET", "/v1/instances/%s/messages" % iid)[1]["messages"]
    row = [m for m in ledger if m["id"] == message_id]
    check("the empty message is a row in the same ledger every message uses", len(row) == 1,
          json.dumps(row)[:200])
    check("the empty message carries the empty type", bool(row) and row[0]["type"] == "")
    frames = call("GET", "/v1/instances/%s/frames" % iid)[1]["frames"]
    check("the empty message opened a frame on the ordinary delivery path",
          [f for f in frames if f["triggering_message_id"] == message_id] != [],
