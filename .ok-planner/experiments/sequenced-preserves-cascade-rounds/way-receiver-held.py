...
SPEC = {
    "name": "exp-sequenced-receiver-held",
    "version": "1",
    "nodes": [
...
def main():
    ...
    bp = call("POST", "/v1/instances/%s/breakpoints" % iid,
              {"matcher": {"node_type": "observer"}, "checkpoint": "before_dispatch",
               "mode": "pause", "hit_ttl_seconds": 3600})[1]["breakpoint_id"]
    send_message(iid)
    hits = wait_hits(iid, 1)
    check("the observer's first round is held in flight",
    ...
    for round_index in range(2, ROUNDS + 1):
        ...
        wait_until(lambda: len(deltas(timeline(iid), "emitter")) >= round_index)
