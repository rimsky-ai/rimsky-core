...
def spec():
    return {"name": "exp-forensic-last-attribute", "version": "1",
            "nodes": [counter_node("counter", 3),
                      passthrough_node("summary",
                                       {"seen_count": {"type": "integer",
                                                       "source": "{{nodes.counter.attribute.count}}"},
                                        "note": {"type": "string", "default": "latest"}},
                                       subscribes=[{"node": "counter", "type": "attribute/count/changed",
                                                    "force_upstream_refresh": False}])]}


def latest_via_node_route(nid):
    return call("GET", "/v1/nodes/%s" % nid)[1].get("latest_attributes")


def latest_via_observability(iid, node_type):
    return call("GET", "/v1/observability/nodes/%s/%s" % (iid, node_type))[1].get("latest_attributes")
...
def main():
    ...
    check("the node ran more than once, so there is a most-recent bag to be wrong about",
          len(starts(tl, "counter")) == 3, json.dumps(deltas(tl, "counter")))

    want = {"count": 3, "max": 3}
    from_node_route = latest_via_node_route(node_id(iid, "counter"))
    check("the node read surface answers with the node's most recent resolved attribute bag",
          from_node_route == want, json.dumps(from_node_route))
    from_obs = latest_via_observability(iid, "counter")
    check("the observability read surface answers with the same bag",
          from_obs == want, json.dumps(from_obs))

    check("the bag is the latest one, not an earlier dispatch's",
          from_node_route.get("count") == 3 and deltas(tl, "counter")[0] == {"count": 1},
          json.dumps(deltas(tl, "counter")))

    event_payloads = [json.dumps(d, sort_keys=True) for d in deltas(tl, "counter")]
    check("no single event carries that bag, so reading it from the event log would mean reconstructing it",
          json.dumps(want, sort_keys=True) not in event_payloads, json.dumps(event_payloads))

    summary_bag = latest_via_node_route(node_id(iid, "summary"))
    check("a second node's most recent bag is read the same way and carries its own resolved values",
          summary_bag == {"note": "latest", "seen_count": 3}, json.dumps(summary_bag))
