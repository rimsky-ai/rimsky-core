...
SPEC = {
    "name": "exp-one-message-per-frame",
    "version": "1",
    "messages": [{"type": "ops/step", "body_schema": {
        "type": "object",
        "properties": {"label": {"type": "string"}},
        "required": ["label"]}}],
    "nodes": [passthrough("reactor", [sub("ops/step", "terminal/success")],
                          {"label": {"type": "string", "source": "{{messages.ops/step.label}}"}})],
}

LABELS = ("first", "second", "third")


def burst(template_id, mode):
    iid = new_instance(template_id, message_queue_mode=mode)
    bpid = pause_breakpoint(iid, "reactor")
    for label in LABELS:
        status, _ = send_message(iid, {"type": "ops/step", "payload": {"label": label}})
        if status != 201:
            die("send %s rejected: %s" % (label, status))
        if label == LABELS[0]:
            wait_hits(iid, 1)
    return iid, drain_breakpoint(iid, bpid)
...
def main():
    ...
    iid, tl = burst(template_id, "backlog")
    show(tl)

    fr = frames(iid)
    delivered = [m for m in messages(iid) if m.get("delivered_at")]
    check("three messages arrived while the instance was busy and each opened its own frame",
          len(delivered) == 3 and len(fr) == 3, "%d delivered, %d frames" % (len(delivered), len(fr)))
    check("no frame carries more than one message: each delivered message names a distinct frame",
          len({m["frame_id"] for m in delivered}) == 3,
          json.dumps([m["frame_id"] for m in delivered]))
    check("each frame's triggering message is one of the three, and no message shares a frame",
          sorted(f["triggering_message_id"] for f in fr) == sorted(m["id"] for m in delivered),
          json.dumps([f.get("triggering_message_id") for f in fr]))

    seen = [d["label"] for d in deltas(tl, "reactor")]
    check("the node reacting to the messages resolved exactly one body per run, in arrival order",
          seen == list(LABELS), json.dumps(seen))
    check("no run of the reacting node failed to resolve its message body",
          not [r for r in tl if r["kind"] == "template_resolution_failed"],
          json.dumps([r["payload"] for r in tl if r["kind"] == "template_resolution_failed"]))

    check("every frame the burst opened names the one declared message type",
          [f.get("message_type") for f in fr] == ["ops/step"] * 3,
          json.dumps([f.get("message_type") for f in fr]))

    iid_c, tl_c = burst(template_id, "coalesce")
    delivered_c = [m for m in messages(iid_c) if m.get("delivered_at")]
    check("even the mode that discards stale wakes never merges two bodies into one frame",
          len({m["frame_id"] for m in delivered_c}) == len(delivered_c) == len(frames(iid_c)),
          "%d delivered across %d frames" % (len(delivered_c), len(frames(iid_c))))
    check("the coalescing instance's reacting node also resolved one body per run",
          [d["label"] for d in deltas(tl_c, "reactor")] == [m["payload"]["label"] for m in
                                                            sorted(delivered_c, key=lambda m: m["received_at"])],
          json.dumps([d["label"] for d in deltas(tl_c, "reactor")]))
