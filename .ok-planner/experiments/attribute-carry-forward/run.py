...
def main():
    ...
    iid = new_instance(deploy(loop_spec()))
    send_message(iid)
    tl = quiet(iid)
    print("  first frame")
    show(tl)
    check("the executor's own output attribute is in the incoming bag on the next dispatch of the same node",
          counts(tl, "counter") == [1, 2, 3], json.dumps(counts(tl, "counter")))
    bag = call("GET", "/v1/nodes/%s" % node_id(iid, "counter"))[1].get("latest_attributes")
    check("the node's resolved bag carries the value its executor wrote",
          bag == {"count": 3, "max": 3}, json.dumps(bag))

    send_message(iid)
    tl = quiet(iid, want_frames=2)
    print("  after a second frame")
    show(tl)
    check("in a new frame the same node starts again from the schema default",
          counts(tl, "counter") == [1, 2, 3, 1, 2, 3], json.dumps(counts(tl, "counter")))

    iid2 = new_instance(deploy(fanout_spec()))
    send_message(iid2)
    tl2 = quiet(iid2)
    print("  fan-out partitions")
    show(tl2)
    keys = payloads(tl2, "fan_out_dispatched")
    check("the fan-out opened three partitions, each its own run-scope",
          bool(keys) and keys[0].get("child_keys") == ["p1", "p2", "p3"], json.dumps(keys))
