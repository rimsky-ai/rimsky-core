...
def loop_spec():
    return {"name": "exp-carry-forward-loop", "version": "1",
            "nodes": [counter_node("counter", 3)]}


def fanout_spec():
    return {"name": "exp-carry-forward-fanout", "version": "1",
            "nodes": [counter_node(
                "partitioned", 3, self_loop=False,
...
def counts(tl, node):
    return [d.get("count") for d in deltas(tl, node)]


import yaml

KNOWN_SUBGRAPH = os.path.join(os.path.dirname(HERE), "template-sub-graph-delegation", "template.yml")
SUBGRAPH_POLL_ROUNDS = 40


def counter_inside_subgraph_spec():
    spec = yaml.safe_load(open(KNOWN_SUBGRAPH))
    spec["name"] = "exp-carry-forward-subgraph-counter"
    worker = [g for g in spec["graphs"] if g["name"] == "worker"][0]
    mid = [n for n in worker["nodes"] if n["type"] == "inner-mid"][0]
    mid.pop("executor", None)
    mid["kind"] = "loop_counter"
    mid["attributes"] = {"schema": {"type": "object", "properties": {
...
def drive(spec):
    ...
    send_message(iid)
    for _ in range(SUBGRAPH_POLL_ROUNDS):
        time.sleep(2)
        fr = frames(iid)
        if fr and all(f["state"] in SETTLED_FRAME_STATES for f in fr) and not live_runs(iid):
            return True, [], timeline(iid)
    return False, [(r["id"][:8], r.get("state")) for r in live_runs(iid)], timeline(iid)


def main():
    boot_with_filesystem_producer(["data"])
    known = yaml.safe_load(open(KNOWN_SUBGRAPH))
    known["name"] = "exp-carry-forward-subgraph-control"
    settled, runs, tl = drive(known)
    check("a sub-graph invocation runs to settlement when its internal node names an executor",
          settled, "frames %s, live runs %s" % (json.dumps(settled), json.dumps(runs)))

    settled, runs, tl = drive(counter_inside_subgraph_spec())
    dispatched = [r["node"] for r in tl if r["kind"] == "work_started"]
    check("the stateful node inside the sub-graph invocation is dispatched at all",
          "inner-mid" in dispatched,
          "nodes dispatched %s, live runs %s" % (json.dumps(dispatched), json.dumps(runs)))
    check("the sub-graph invocation carrying that node settles",
          settled, "frames settled %s, live runs %s" % (json.dumps(settled), json.dumps(runs)))
    counts = [d.get("count") for d in deltas(tl, "inner-mid")]
    check("that node starts its run-scope from the schema default",
          counts[:1] == [1], json.dumps(counts))
    finish()
