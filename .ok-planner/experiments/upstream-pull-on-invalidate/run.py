...
def spec_for(force):
    return {
        "name": "exp-upstream-pull-%s" % str(force).lower(),
        "version": "1",
        "messages": [{"type": "never/sent", "body_schema": {"type": "object"}}],
        "nodes": [
            counter_node("trigger"),
            counter_node("pulled", [sub("never/sent", "terminal/success")]),
            passthrough_node("receiver",
                             [sub("trigger", "terminal/success"),
                              sub("pulled", "attribute/count/changed", force=force)],
                             {"from_pulled": {"type": "integer", "source": "{{nodes.pulled.attribute.count}}"}}),
        ],
    }
...
def main():
    ...
    iid = new_instance(deploy(spec_for(True)))
    send_message(iid)
    tl = quiet(iid)
    print("  force_upstream_refresh: true")
    show(tl)
    check("the declared upstream was brought current exactly once",
          len(starts(tl, "pulled")) == 1, json.dumps(deltas(tl, "pulled")))
    check("the upstream ran before the receiver dispatched",
          bool(starts(tl, "receiver"))
          and starts(tl, "pulled")[0]["seq"] < starts(tl, "receiver")[0]["seq"])
    check("the receiver dispatched on the refreshed upstream value",
          deltas(tl, "receiver") == [{"from_pulled": 1}], json.dumps(deltas(tl, "receiver")))

    iid2 = new_instance(deploy(spec_for(False)))
    send_message(iid2)
    tl2 = quiet(iid2)
    print("  force_upstream_refresh: false")
    show(tl2)
    check("without the declaration the same upstream is never brought current",
          not starts(tl2, "pulled"), json.dumps(starts(tl2, "pulled"))[:200])
    finish()
