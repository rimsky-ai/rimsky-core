...
OPERATOR_ONLY = "operator-only-plaintext-do-not-leak-92aa04"
WATCHED = "VALIDATOR_TOKEN,REVIEWER_SEED,OPERATOR_ONLY_SECRET"
...
def agent_node(node_type, expose):
    return {"type": node_type, "executor": "claude-agent",
            "attributes": {"schema": {"type": "object", "properties": {
                "model": {"type": "string", "default": "probe-model"},
                "system_prompt": {"type": "string", "default": "per-node expose-env probe"},
                "user_prompt": {"type": "string",
                                "default": agent_prompt(mode="observe", env=WATCHED)},
                "cli": {"type": "object", "default": {"expose_env": expose}}}}}}


SPEC = {
    "name": "exp-claude-agent-expose-env-per-node",
    "version": "1",
    "nodes": [
        agent_node("alpha", ["VALIDATOR_TOKEN"]),
        agent_node("beta", ["REVIEWER_SEED"]),
        agent_node("gamma", ["OPERATOR_ONLY_SECRET"]),
    ],
}
...
def observation(tl, node_type):
    ...
    if not rows:
        return {}
    return (rows[-1]["payload"].get("attributes_delta") or {}).get("cli_observation") or {}
...
def run_stack(probe, allowlist):
    env = {"CLAUDE_CODE_OAUTH_TOKEN": "probe-stand-in",
           ...
           "OPERATOR_ONLY_SECRET": OPERATOR_ONLY}
    if allowlist is not None:
        env["RIMSKY_CLAUDE_AGENT_EXPOSE_ENV_ALLOWLIST"] = allowlist
...
def persisted_state_dump(iid):
    blobs = [json.dumps(call("GET", "/v1/observability/events?instance_id=%s&limit=500" % iid)[1]),
             json.dumps(call("GET", "/v1/observability/node-runs?instance_id=%s&limit=200" % iid)[1]),
             json.dumps(call("GET", "/v1/instances/%s" % iid)[1]),
             json.dumps(call("GET", "/v1/audit?limit=500")[1])]
    for n in nodes_of(iid):
        blobs.append(json.dumps(call("GET", "/v1/nodes/%s" % n["id"])[1]))
    return "\n".join(blobs)


def main():
    probe = build_probe_agent()
    iid, tl = run_stack(probe, "VALIDATOR_TOKEN,REVIEWER_SEED")

    alpha = observation(tl, "alpha")
    beta = observation(tl, "beta")
    check("the node declaring VALIDATOR_TOKEN reads that variable's operator-set value",
          alpha.get("env_present") == ["VALIDATOR_TOKEN"]
          and (alpha.get("env_digest") or {}).get("VALIDATOR_TOKEN") == digest(VALIDATOR_TOKEN),
          json.dumps(alpha))
    check("the node declaring REVIEWER_SEED reads that variable's operator-set value",
          beta.get("env_present") == ["REVIEWER_SEED"]
          and (beta.get("env_digest") or {}).get("REVIEWER_SEED") == digest(REVIEWER_SEED),
          json.dumps(beta))
    check("neither node sees the other node's declared variable",
          "REVIEWER_SEED" not in (alpha.get("env_present") or [])
          and "VALIDATOR_TOKEN" not in (beta.get("env_present") or []),
          json.dumps([alpha.get("env_present"), beta.get("env_present")]))

    gamma = error_payload(tl, "gamma")
    payload = gamma.get("error_payload") or {}
    check("the node declaring a variable outside the operator allowlist fails its dispatch",
          gamma.get("error_class") == "agent/attribute_invalid", json.dumps(gamma)[:250])
    check("the refusal names the variable and the operator allowlist",
          payload.get("disallowed_env_var") == "OPERATOR_ONLY_SECRET"
          and "RIMSKY_CLAUDE_AGENT_EXPOSE_ENV_ALLOWLIST" in json.dumps(payload),
          json.dumps(payload)[:400])

    dump = persisted_state_dump(iid)
    for name, secret in (("VALIDATOR_TOKEN", VALIDATOR_TOKEN),
                         ("REVIEWER_SEED", REVIEWER_SEED),
                         ("OPERATOR_ONLY_SECRET", OPERATOR_ONLY)):
        check("no %s plaintext lands in rimsky's persisted state" % name,
              secret not in dump, "dump bytes=%d" % len(dump))

    teardown()

    iid, tl = run_stack(probe, None)
    check("with no operator allowlist the same template's OPERATOR_ONLY_SECRET node reads it",
          (observation(tl, "gamma").get("env_digest") or {}).get("OPERATOR_ONLY_SECRET")
          == digest(OPERATOR_ONLY),
          json.dumps(observation(tl, "gamma")))
    check("with no operator allowlist each node still reads only its own declaration",
          observation(tl, "alpha").get("env_present") == ["VALIDATOR_TOKEN"]
          and observation(tl, "beta").get("env_present") == ["REVIEWER_SEED"],
          json.dumps([observation(tl, "alpha").get("env_present"),
                      observation(tl, "beta").get("env_present")]))
    finish()
