...
SPEC = {
    "name": "exp-cascade-signal-blind",
    "version": "1",
    "nodes": [
        counter_node("emitter"),
        {"type": "failer", "executor": "http-node",
         "attributes": {"schema": {"type": "object", "properties": {
             "url": {"type": "string", "default": "http://127.0.0.1:9/unreachable"}}}}},
        passthrough_node("on_success", [sub("emitter", "terminal/success")]),
        passthrough_node("on_attribute", [sub("emitter", "attribute/count/changed")]),
        passthrough_node("on_error", [sub("failer", "terminal/error/*")]),
        passthrough_node("on_error_filtered",
                         [sub("failer", "terminal/error/*", when="type.startsWith('terminal/error/http')")]),
    ],
}

TRANSIENT_SPEC = {
    "name": "exp-cascade-signal-blind-transient",
    "version": "1",
    "nodes": [
        counter_node("emitter"),
        passthrough_node("on_transient", [sub("emitter", "transient/park")]),
    ],
}
...
def main():
    ...
    emitted = [r["kind"] for r in tl if r["kind"].startswith("terminal/") or r["kind"].startswith("attribute/")]
    check("all three cascade-firing signal kinds were emitted in the run",
          "terminal/success" in emitted
          and any(k.startswith("terminal/error/") for k in emitted)
          and any(k.startswith("attribute/") and k.endswith("/changed") for k in emitted),
          ",".join(sorted(set(emitted))))

    for receiver in ("on_success", "on_attribute", "on_error", "on_error_filtered"):
        check("receiver %s woke through its subscribes entry" % receiver,
              len(starts(tl, receiver)) == 1, json.dumps(deltas(tl, receiver)))

    entries = []
    for node in SPEC["nodes"]:
        for entry in node.get("subscribes", []):
            entries.append(sorted(entry.keys()))
    check("every subscription uses the same declaration keys",
          all(set(e) <= {"node", "type", "force_upstream_refresh", "when"} for e in entries)
          and all({"node", "type", "force_upstream_refresh"} <= set(e) for e in entries),
          json.dumps(entries))

    status, out = call("POST", "/v1/templates", {"spec": TRANSIENT_SPEC})
    check("a non-cascade-firing signal is refused by the same mechanism, by name",
          status == 400 and "transient" in json.dumps(out), json.dumps(out)[:220])
    finish()
