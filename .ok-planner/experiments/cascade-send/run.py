...
SPEC = {
    "name": "exp-cascade-send",
    "version": "1",
    "messages": [{"type": "loop/iterate", "body_schema": {
        "type": "object",
    ...
    "nodes": [
        counter_node("a"),
        {
            ...
            "subscribes": [sub("a", "terminal/success"), sub("a", "attribute/count/changed")],
            "attributes": {"schema": {"type": "object",
                                      "properties": {"value": {"type": "integer",
                                                               "source": "{{nodes.a.attribute.count}}"}},
                                      "required": ["value"]}},
        },
        passthrough_node("receiver", [sub("loop/iterate", "terminal/success")],
...
def main():
    ...
    status, out = send_message(iid)
    check("operator wake message accepted", status == 201, str(out))
    tl = quiet(iid)
    show(tl)

    types = set(node_types(iid).values())
    check("the send-node is a graph object listed on the instance", "sender" in types,
          "node types: " + ",".join(sorted(types)))

    messages = call("GET", "/v1/instances/%s/messages" % iid)[1]["messages"]
    sent = [m for m in messages if m["type"] == "loop/iterate"]
    check("the send-node dispatch landed one loop/iterate message", len(sent) == 1,
          json.dumps(sent)[:200])
    check("the sent message is attributed to the instance", bool(sent) and sent[0]["sender_kind"] == "instance",
          json.dumps(sent[0]) if sent else "")
    check("the message body carries the value the send-node composed",
          bool(sent) and sent[0]["payload"] == {"value": 1}, json.dumps(sent[0]["payload"]) if sent else "")

    frames = call("GET", "/v1/instances/%s/frames" % iid)[1]["frames"]
    opened = [f for f in frames if f.get("message_type") == "loop/iterate"]
    check("the sent message opened a frame of its own", len(opened) == 1, json.dumps(opened)[:200])
    check("the frame opened by the send names the instance as sender",
          bool(opened) and opened[0]["message_sender_kind"] == "instance")

    check("the downstream node ran on the message body", deltas(tl, "receiver") == [{"got": 1}],
          json.dumps(deltas(tl, "receiver")))
    finish()
