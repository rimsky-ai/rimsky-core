...
SPEC = {
    "name": "exp-message-queue-coalesce",
    "version": "1",
    "message_queue_mode": "coalesce",
    "messages": [{"type": "ops/wake", "body_schema": {
        "type": "object",
        "properties": {"n": {"type": "integer"}},
        "required": ["n"]}}],
    "nodes": [passthrough("worker", [sub("ops/wake", "terminal/success")],
                          {"n": {"type": "integer", "source": "{{messages.ops/wake.n}}"}})],
}

BURST = (1, 2, 3, 4)


def run_instance(template_id, override):
    extra = {} if override is None else {"message_queue_mode": override}
    iid = new_instance(template_id, **extra)
    mode = call("GET", "/v1/instances/%s" % iid)[1]["message_queue_mode"]
    bpid = pause_breakpoint(iid, "worker")
    for n in BURST:
        status, _ = send_message(iid, {"type": "ops/wake", "payload": {"n": n}})
        if status != 201:
            die("send %d rejected: %s" % (n, status))
        if n == BURST[0]:
            wait_hits(iid, 1)
    held = messages(iid)
    tl = drain_breakpoint(iid, bpid)
    return iid, mode, held, tl


def main():
    boot()
    template_id = deploy(SPEC)

    iid_c, mode_c, held_c, tl_c = run_instance(template_id, None)
    check("an instance takes the template's coalesce mode when it names none", mode_c == "coalesce", mode_c)

    iid_b, mode_b, held_b, tl_b = run_instance(template_id, "backlog")
    check("a sibling instance of the same template chooses backlog at create time", mode_b == "backlog", mode_b)

    def by_n(rows, pred):
        return sorted(m["payload"]["n"] for m in rows if pred(m))

    check("while the first frame was held, the coalescing instance dropped the stale wakes and kept the newest",
          by_n(held_c, lambda m: m.get("cancelled")) == [2, 3]
          and by_n(held_c, lambda m: not m.get("cancelled") and not m.get("delivered_at")) == [4],
          json.dumps([(m["payload"]["n"], bool(m.get("cancelled"))) for m in held_c]))

    check("while the first frame was held, the backlog instance kept every pending wake",
          by_n(held_b, lambda m: m.get("cancelled")) == []
          and by_n(held_b, lambda m: not m.get("cancelled") and not m.get("delivered_at")) == [2, 3, 4],
          json.dumps([(m["payload"]["n"], bool(m.get("cancelled"))) for m in held_b]))

    final_c, final_b = messages(iid_c), messages(iid_b)
    check("the coalescing instance ran the first wake and then the latest one, skipping the two it fell behind on",
          by_n(final_c, lambda m: m.get("delivered_at")) == [1, 4],
          json.dumps(by_n(final_c, lambda m: m.get("delivered_at"))))
    check("the coalescing instance never delivered a cancelled wake",
          all(not (m.get("cancelled") and m.get("delivered_at")) for m in final_c))
    check("the backlog instance ran every wake in its own frame",
          by_n(final_b, lambda m: m.get("delivered_at")) == [1, 2, 3, 4],
          json.dumps(by_n(final_b, lambda m: m.get("delivered_at"))))

    check("the coalescing instance opened two frames", len(frames(iid_c)) == 2, str(len(frames(iid_c))))
    check("the backlog instance opened four", len(frames(iid_b)) == 4, str(len(frames(iid_b))))

    check("every payload the backlog instance was sent reached the node",
          [d["n"] for d in deltas(tl_b, "worker")] == [1, 2, 3, 4],
          json.dumps(deltas(tl_b, "worker")))
    check("the coalescing node saw the first and the latest payload only",
          [d["n"] for d in deltas(tl_c, "worker")] == [1, 4],
          json.dumps(deltas(tl_c, "worker")))

    status, out = call("POST", "/v1/instances", {
        "template": template_id, "instance_key": "exp-" + uuid.uuid4().hex[:12],
        "target_agent": "audit-probe-agent", "message_queue_mode": "newest-only"})
    check("a mode outside the two the operator may choose is refused at create",
          status == 400 and "backlog" in json.dumps(out) and "coalesce" in json.dumps(out),
          json.dumps(out))
