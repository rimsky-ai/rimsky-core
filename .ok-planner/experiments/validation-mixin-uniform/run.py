...
STATE = {"base": None, "containers": [], "servers": [], "checks": []}
...
def build_peerstub():
    if shutil.which("go") is None:
        die("the go toolchain is required to build the peer stub this experiment drives rimsky against")
    workdir = tempfile.mkdtemp(prefix="exp-mixin-")
    STATE["workdir"] = workdir
    binary = os.path.join(workdir, "peerstub")
    res = subprocess.run(["go", "build", "-o", binary, str(HERE / "peerstub.go")],
                         cwd=str(REPO), capture_output=True, text=True)
    if res.returncode != 0:
        die("go build peerstub failed: " + res.stderr.strip())
    return binary


def start_peer(binary, name, kind, roles):
    port = free_port()
    log = open(os.path.join(STATE["workdir"], name + ".log"), "w+")
    proc = subprocess.Popen([binary, "-port", str(port), "-name", name, "-kind", kind, "-roles", roles],
                            stdout=log, stderr=log)
    STATE["peers"].append((proc, log))
    while True:
        probe = socket.socket()
        try:
            ...
            if proc.poll() is not None:
                log.seek(0)
                die("peer %s exited: %s" % (name, log.read()))
            time.sleep(0.2)


def stop_peers():
    for proc, log in STATE["peers"]:
        proc.terminate()
        try:
...
def boot_with_peers(exec_port, other_role_port, publisher_port):
    config = pathlib.Path(os.environ.get("TMPDIR", "/tmp")) / ("exp-mixin-%s.yml" % uuid.uuid4().hex[:8])
    config.write_text(
        "persistence:\n"
        "  driver: sqlite\n"
        "  sqlite:\n"
        ...
        "  publisher-peer:\n"
        "    endpoint: grpc://host.docker.internal:%d\n"
        "    protocols: [publisher, validation]\n" % (exec_port, other_role_port, publisher_port))
    STATE["config"] = config
    port = free_port()
...
SPEC = {
    "name": "exp-validation-mixin-uniform",
    "version": "1",
    "messages": [{"type": "probe/event",
                  "body_schema": {"type": "object", "properties": {}, "additionalProperties": True}}],
    "publishers": [{"name": "publisher-peer", "kind": "probe", "config": {}, "message_type": "probe/event"}],
...
def consulted(body):
    ...
    for entry in (body or {}).get("validation_warnings") or []:
        if entry.get("class") != "mixin_consulted":
            continue
        out[entry.get("path")] = entry.get("msg")
    return out


def main():
    binary = build_peerstub()
    exec_port = start_peer(binary, "executor-peer", "executor", "executor")
    other_port = start_peer(binary, "other-role-peer", "executor", "claim_producer")
    pub_port = start_peer(binary, "publisher-peer", "publisher", "publisher")
    boot_with_peers(exec_port, other_port, pub_port)

    status, body = call("POST", "/v1/templates/validate", {"spec": SPEC})
    check("the template validates", status == 200, json.dumps(body)[:300])
    seen = consulted(body)
    print("  findings: " + json.dumps(seen, indent=2))
    check("the executor peer's validation mix-in was consulted",
          "/executor" in seen, json.dumps(sorted(seen)))
    check("the executor peer was consulted for the executor role it declared",
          'role "executor"' in seen.get("/executor", ""), seen.get("/executor", ""))
    check("the publisher peer's validation mix-in was consulted",
          "/publisher" in seen, json.dumps(sorted(seen)))
    check("the publisher peer was consulted for the publisher role it declared",
          'role "publisher"' in seen.get("/publisher", ""), seen.get("/publisher", ""))
    check("the peer that declared a role it is not used in was never consulted",
          not any("other-role-peer" in msg for msg in seen.values()), json.dumps(seen))

    status, body = call("POST", "/v1/templates", {"spec": SPEC})
    seen = consulted(body)
    check("both mix-ins are consulted on registration too, not only on validate",
          status in (200, 201) and "/executor" in seen and "/publisher" in seen,
          "%s %s" % (status, json.dumps(sorted(seen))))
