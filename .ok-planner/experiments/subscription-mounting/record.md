...
The instance exposes one subscription per declared publisher entry, carrying the
publisher name, its kind and its message type. The operator saw that
subscription in the `mounting` state and then in the `active` state, and in no
other state. A `tick/minute` message attributed to the publisher `tick` then
arrived and the node the template wired to that type ran, so `active` names a
sensor that is feeding the instance.
