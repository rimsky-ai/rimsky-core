...
Both nodes settled: the clean one fresh, the violating one failed with the
check's own error class. The control API's node read returned the node's whole
state in one document — id, instance, node type, executor, declared tags,
cascade mode, creation time, the run tallies, the attributes the run left
behind (including the offending row and the check that rejected it), and the
settled signal `terminal/error/verifier/check_failed/no_nulls`. The healthy
node's same read carried `terminal/success`, so the marker distinguishes the
two. The CLI's `node get` renders a narrower projection of that document —
...
its id, its executor, its run tallies and the check's findings were byte-for-byte
what they had been — the failure marker is gone and nothing else about the
node's readable state moved. The clearing itself appears in the instance's
event log as one operator-override entry against that node.
