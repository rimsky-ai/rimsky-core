...
- Workspace module count: the record's endpoint is five modules including `examples` (2026-06-08, artifact-only — both design and completion report attest it), but no transcript-tier entry ever corroborates or mentions the examples module. If today's tree has four modules, the record alone cannot say whether examples was later deliberately retired (unrecorded) or silently lost; adjudicators should treat the five-module claim with artifact-only weight.
