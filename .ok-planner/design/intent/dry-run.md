...
- ~~backfill:create rejects (400) a target_node that is not a fan-out node wired for the partition override — refused at submit, not warned; the dry-run branch applies the same validation (2026-05-29, artifact; tightened the earlier warn-only invariant).~~ STRUCK 2026-07-20, ledger 2507: backfill:create never exists — backfill was retired as a first-class primitive (2026-06-14, bfc9febb, transcript — a backfill is now a message carrying a partition override; see message dossier), so there is no action, route, or handler for this validation to apply to. Verified: no backfill:create in the action registry, `lib/control/controlapi`, or `cmd`.
