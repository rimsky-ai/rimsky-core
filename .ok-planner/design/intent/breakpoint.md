...
- **RESOLVED 2026-07-14 (user ruling, transcript tier): breakpoint.hit event-log emission is IN scope; the 06-18 out-of-scope assumption is corrected.** The 2026-06-18 "out for v1" verdict addressed a sketch bundling two features — F8 (SSE live-tail on /events) and F9 (breakpoint.hit onto the event log) — and its rationale targeted F8; F9 was swept out with the bundle while it genuinely did not exist. F9 was subsequently built anyway: hit row + breakpoint.hit event-log row land in one transaction (breakpoint_eval.go). Ruling: F9 stands as required intent (uniform /events poll surface; atomic forensics; not strictly necessary for breakpoint usability — the hits route alone suffices for hit/resume — but ratified as shipped), WITH a Phase 2 completeness review: prove co-transactionality, prove /events?kind=breakpoint.hit filtering, verify `rimsky watch` consumes it, verify the hit payload carries hit_id/checkpoint/mode. F8 (SSE/push delivery) remains OUT — poll-only /events is the ratified v1 surface. (Same ruling recorded in the event-log dossier.)
