...
None open. Resolved 2026-07-14 (user ruling, transcript tier):

- **Entry-node identity model → absorption is canonical.** The artifact tier (2026-05-15 data-platform-extensions; 2026-05-19 crimefinder; 2026-05-21 matcher-overlay) defines structural absorption — the entry node is absorbed into the calling node at canonicalization (merged executor/claim-producers/holds/attributes, one dispatched node_run on the caller, entry-alias references resolving to the caller). The later transcript tier (2026-06-22/23, 10cf843b) described the entry as a dedicated node receiving copied caller attributes. User ruling 2026-07-14: "if it works, and it's simple and easy to test and maintain, then we can keep absorption. conceptually it's a new node_run that gets a composition of a caller, but if that adds complexity, fine to keep the status quo." The copy/composition model is the conceptual ideal, not a requirement; the June transcript language reads as describing absorption's per-invocation effect, not a redesign. Consequences: (a) entry-alias subscription edges MUST resolve to the calling node at the cascade walker — finding 134's missing runtime consumer is a fix-code defect, not an invariant to retract; (b) one node_run per invocation on the caller is required intent; (c) the inert per-entry rimsky_nodes row materialized at instance creation is an implementation wart, not intent — permissible to keep or remove; (d) a sub-graph may be invoked by multiple callers (only delegate cycles are rejected); internal nodes are shared rows with per-invocation separation in run scopes.
