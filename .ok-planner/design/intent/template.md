...
- **RESOLVED 2026-07-14 (user ruling, transcript tier): the erase rule is absolute; the two migration-error carve-outs are expired scaffolding.** Retired shapes are erased with no detection code and no errors naming them. The `nodes.<X>.event` migration message (2026-06-17) and the `stores:` directing rejection (2026-06-19) dated from mid-migration weeks when the project's own template corpus carried the old forms; pre-v1 and project-agnostic, no external corpus exists, and generic unknown-field rejection suffices. Both directing errors are removed (fix-code). Adjudicators: "missing migration error" findings → refute; "present migration error" findings → defect. (Same ruling recorded in the validation dossier.)
