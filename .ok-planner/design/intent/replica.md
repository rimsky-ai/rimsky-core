...
- Per-tick advisory lock on dispatch; session advisory lock on migration batches; SQLite single-process posture (2026-05-04, foundation-contract, artifact).
...
- Launch-topology hardening direction (ratified sketch, 2026-06-21, ecde6dd1, transcript): typed Topology parameter replacing the stringly RIMSKY_PROCESS_ROLE seam; migrate ownership as a typed launcher-plan field instead of a literal 'rimsky-control-api' string match; the SQLite-plus-replicas warn-only gate riding the same typed machinery as the memory-blob hard gate. (Direction ratified; delivery not confirmed in this record.)
