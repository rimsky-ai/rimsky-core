...
- The 2026-05-21 execSchemaVisible divergence (skip schema rules whenever the executor schema is invisible) sits uneasily against the 2026-05-24 `late_bind_services` promise (strict for every name NOT explicitly listed). If both shipped, an unlisted service whose schema is simply not yet in the cache gets lax treatment the later promise says it must not get. The record shows no explicit resolution of the earlier divergence after `late_bind_services` landed.
