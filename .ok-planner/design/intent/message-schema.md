...
- **RESOLVED 2026-07-14 (user ruling, transcript tier): message bodies are schema-validated at LEDGER INSERTION, every producer.** The 2026-06-14 "(a) is fine" no-receipt-validation ruling belonged to the substitution-pull model (receivers pulled fields via {{messages.T.field}} through the attribute-validation gate at dispatch — validation was lazy but guaranteed); the 2026-06-22 push-the-bag redesign removed that gate, which is why it added "after checking schema." Ruling: one chokepoint at insertion into the message ledger — POST /instances/{id}/messages (operator and publisher) rejects an invalid body synchronously with a 400 naming the schema violation; cascade-emitted messages hit the same check at insert, surfacing as a sender-side error; the ledger only ever contains schema-valid messages and delivery trusts it (no check at bag population). The May inertness constraint permits this explicitly ("never validates beyond schema gates"). Current code validates nowhere (type-check only at ingest, wholesale bag dump at delivery) — that is a defect against this ruling.
