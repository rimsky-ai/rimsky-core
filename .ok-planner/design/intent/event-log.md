...
- **RESOLVED 2026-07-14 (user ruling, transcript tier): breakpoint.hit rows on /events are canonical.** The 06-18 out-of-scope verdict targeted the SSE live-tail half of a bundled sketch; the event-log emission half was swept out with it, then built anyway (co-transactional hit row + event row). Ruling: the emission stands as required intent, with a Phase 2 completeness review (co-transactionality proof, ?kind= filter, watch consumption, payload fields); SSE/push delivery remains out — poll-only /events is the v1 surface. Full ruling text in the breakpoint dossier.
