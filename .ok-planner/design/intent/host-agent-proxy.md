...
- Five-protocol routing vs interceptor scope: the 2026-06-06 artifact invariant and 2026-06-08 artifact promise require all five protocols dispatchable through the proxy with no stubs, while the 2026-06-13 transcript (c41b7afe) — both later and higher tier — ratifies that the ServiceName routing interceptor exists only on the executor and claim-producer dial paths as intentional v1 scope, citing the 2026-05-24 spec's optionality. The transcript addresses the rimsky-side dial paths without engaging the gap-closure promise of full transparency, and the record never explains how publisher/validation/data-processing dispatches reach the proxy without the interceptor. A human must rule whether all-five-protocol proxy routing is presently required or an executor+claim-producer subset is the sanctioned v1 surface.
