...
- **RESOLVED 2026-07-14 (user ruling, transcript tier): feature-index.md is an INTENTIONAL ABSENCE — killed at every layer.** The user had killed it repeatedly and it kept resurrecting; the vector was found: the Plumbline SOURCE repo had already dropped the mandate, but the published plugin (≤0.5.1) and rimsky's materialized cheatsheet copy still carried "Update feature-index.md when features/dependencies change," so agents obeying the rules file kept recreating it. Kill executed 2026-07-14: cheatsheet re-materialized from plumbline source (no feature-index references), repo-root feature-index.md deleted (git rm). Restorers must NOT resurrect it — navigation lives in CLAUDE.md, the concept catalog, and grep. The 29 index-mismatch ledger findings close as moot (subject deleted). Remaining outward item (plumbline repo, not rimsky): cut a plugin release from current source so other projects stop re-importing the mandate.
- **RESOLVED 2026-07-14 (user ruling, transcript tier): the drift-cleanup relaxation EXTENDS to tension files, scoped to this remediation effort.** Phase 1 agents may resolve/move a tension only when a dossier-recorded ruling directly answers it (citing the ruling), and may file new tensions for genuine ambiguities discovered during adjudication. Anything not backed by a ruling stays untouched; the refine-design-pipeline-only rule resumes for ordinary work once remediation completes. Rationale: leaving known-stale tensions standing propagates the exact "persistent misunderstanding every session inherits" failure the 07-05 session identified.
