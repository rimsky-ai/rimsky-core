...
- **`foundation/`** — module `github.com/fallguy/rimsky/foundation`. Cascade engine + claim/lock primitives + integration runner + persistence drivers. Direct deps: `protocols`, `uuid`, `pgx`. Stdlib + minimal third-party.
- **`protocols/`** — module `github.com/fallguy/rimsky/protocols`. The three service-protocol Go interfaces + protobuf bindings. Direct deps: `grpc`, `protobuf`, `uuid`. Tightest budget; stdlib + grpc only.
- **Root** — module `github.com/fallguy/rimsky`. Modeling layer, cmd binaries, bundled stores, bundled executors. Pulls in heavier libraries (jsonschema, robfig/cron, jcs, testcontainers).
