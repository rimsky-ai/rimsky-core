...
**Emission side**: `foundation/integration/runner_named_events.go` is the receiver for `NamedEvent` events streamed from the executor during `Execute`. Per the file head comment, the payload bytes are never logged, formatted with `%v`, transformed, or attached to traces — they're inert in rimsky. The receiver INSERTs into `rimsky_node_events` (migration 006), spilling via `BlobBackend` if over threshold.
