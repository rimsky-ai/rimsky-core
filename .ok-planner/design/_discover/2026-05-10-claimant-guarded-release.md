...
This is `@blessed-invariant 4`, annotated at:
...
The rule's value is concentrated at the orphan-reaper site: the periodic sweep runs every tick (default `5 × heartbeat_interval` cutoff per `@blessed-invariant 6` — `foundation/integration/conductor.go:30-50`). Without `AND holder_supervisor_id = $1`, the sweep could race against an owner whose heartbeat is about to refresh and clobber a live row. With the predicate, even if the timing argument fails (clock skew, scheduler precision), the live owner's identity prevents the clobber: the SQL `WHERE` simply doesn't match.

`@blessed-invariant 5` (verify-before-run) is the second guard at the same column: after the acquisition tx commits, the runner re-reads `rimsky_worker_request.claimed_by` and bails if ownership moved (`foundation/integration/runner_acquire.go:756-810`). The two guards together make "still owned by this supervisor" the only safe predicate for any mutation.
...
- `foundation/integration/supervisor.go:25-38` — invariant 4 annotation.
