...
The reason these matter is documented at `foundation/integration/runner_acquire.go:538-557`: under READ COMMITTED, two supervisors can each pass the in-Go conflict predicate against the other's still-uncommitted INSERT into `rimsky_claim_handle`; the per-named-lock and per-scope advisory lock closes that window before the conflict check even runs. The acquisition flow walks `(lock_kind, sort_key)` order (deterministic ordering — `@blessed-invariant 3`) so two concurrent acquirers that need both N1 and S1 always queue on the same advisory lock first, preventing the (N1-held, S1-wait) ⨯ (S1-held, N1-wait) deadlock.
