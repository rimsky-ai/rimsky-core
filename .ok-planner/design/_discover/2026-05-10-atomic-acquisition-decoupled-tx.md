...
The single-writer-per-scope guarantee still holds: rimsky's conflict predicate (`evaluateScopeConflict` at `foundation/integration/runner_acquire.go:670-705`) gates new claim-handle INSERTs against `rimsky_claim_handle` only. Producer-side orphan state is invisible to the conflict predicate, so a producer's leftover staging directory after a rimsky rollback does not cause a phantom conflict on the next acquisition.

The acquisition tx walks the per-spec lock list in deterministic `(lock_kind, sort_key)` order (the deterministic lock-ordering rule at `foundation/integration/runner.go:20-29`); the per-scope advisory lock (`TakeScopeLockInTx`) is what closes the READ COMMITTED window between conflict-check and claim-handle INSERT.
...
- `Open` happens **inside** rimsky's acquisition tx (per the in-tx-Open rule at the proto comment); this means the in-tx HTTP/gRPC call holds a row-level lock on the `rimsky_worker_request` row for the duration of the producer's response. A slow producer therefore widens the contention window — visible at `foundation/integration/runner.go:46-51` as a deliberate choice.
