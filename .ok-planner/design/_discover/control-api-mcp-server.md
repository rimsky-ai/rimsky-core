...
**Implementation: Go, not TypeScript.** Package `controlapimcp` is a third Go module (`github.com/fallguyconsulting/rimsky/mcp-servers/control-api`, own `go.mod`) implementing JSON-RPC 2.0 over HTTP on a single `POST /mcp` endpoint using `go-chi/chi` and stdlib `encoding/json`. The package doc on `server.go` explicitly motivates the choice: "Per plan K2: implements `initialize`, `tools/list`, `tools/call` over POST /mcp using stdlib encoding/json + go-chi/chi. No third-party MCP SDK; the wire surface is small enough that a direct implementation is cleaner."
