...
The JCS library version (`github.com/cyberphone/json-canonicalization`) is pinned in `go.mod`. The annotation at `jcs.go:13-15` is explicit: "the canonical-hash function is the registry's identity function. Any change that alters output bytes for previously-registered specs is a breaking change. The JCS library version is pinned in go.mod." A transitive minor bump that changed canonicalization output bytes would invalidate every existing template id; this is therefore a `@blessed-invariant`-style discipline annotation in the modeling layer.
