...
I declare a node of the loop-counter kind with a maximum count of three; cascade re-fires the node three times via a subscription on the `loop` tag; on the third dispatch the node emits the `done` tag instead of the `loop` tag; a downstream subscriber on the `done` tag fires.

## Falsifier

The loop-counter carries the `loop` tag after reaching the maximum count. OR: it carries the `done` tag before reaching the maximum count. OR: count does not carry across dispatches and the `done` tag never fires.
