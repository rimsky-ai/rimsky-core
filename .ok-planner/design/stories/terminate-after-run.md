...
fires and the instance's `terminated_at` column is stamped. The
...
`concept:instance-lifecycle` is preserved.
...
`terminated_at` column is non-null. A subsequent message POST to
the now-terminated instance is rejected as a terminal-state
conflict. An equivalent run without the flag leaves
`terminated_at` null past node settlement and accepts subsequent
messages, preserving the durable-by-default shape.

## Falsifier

The `terminated_at` column is still null after the single run's
...
Executable proof — `TestTerminateAfterRun_EndToEnd` deploys a
single executor template, creates the instance with
`terminate_after_run: true`, drives the executor to fresh through
the bundled Success stub, asserts `GET /instances/{id}` shows
`terminated_at` set, and asserts a follow-up message POST is
rejected with a terminal-state conflict. The durable-by-default
counter-shape is pinned by the sibling `TestSensorHTTP_DurableAcrossFires`
proof on `story:sensor-http`, which exercises the no-flag path and
asserts the instance stays alive across N fires. Together the two
proofs pin both lifecycles end to end against the real assembled
product.
