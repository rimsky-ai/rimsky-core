...
The supervisor reads the env, not the executor. For all-in-one deployments this is identical to the prior behavior. For split deployments, operators set the env on the supervisor container rather than the per-executor container — a deliberate consolidation of the secret-bearing surface to one process. This trades the prior executor-locality property for one substitution surface; pre-v1, the deployment story is shaped by this choice rather than the choice accommodating an existing operator habit.
