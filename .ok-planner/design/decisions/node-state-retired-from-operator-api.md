...
Removing the field entirely forces operators to consume run lifecycle through surfaces that are unambiguous by construction. The categorical summary answers "what is the distribution of run states for this node" — the question operators almost always actually have. The per-run endpoint answers "what state is run X in" — the question operators ask when they have a specific run in mind. Neither surface conceals run-level detail behind a synthesized aggregate, and neither requires the API to make a "which run is the canonical one" choice the API cannot make correctly.
