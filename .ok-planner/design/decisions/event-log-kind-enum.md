...
An integrity-check constraint at persistence (couples schema to enum, requires migration per new kind, redundant when the enum gates the app boundary); a registry-table-with-foreign-key approach (introduces a mutable kind catalog through an API, which the model doesn't want); leaving operational kinds free-form (accepts the footgun for audit consumers filtering by canonical kind name).
