...
- The carry-forward bag built at row creation, scoped to the current frame's RunScope:
  - **`operator_invalidate` / `recalculate`** — both fire inside a running frame. Carry-forward reads the immediately-prior run of the same node **in the same RunScope in the current frame**; if no such prior exists, the bag is the schema's static defaults. Cross-frame carry-forward is impossible by construction (RunScopes never span frames per `concept:run-scope`; the runtime cannot supply a prior from any other frame).
  - **`message_delivery`** — fires when a new frame opens on a triggering message. There is no in-scope prior at the moment the receiver's run is created (the frame's root RunScope is freshly-minted and empty). The bag is the message envelope's payload, copied verbatim onto the row as both its attribute bag and its dispatch input bag — no schema-defaults pass and no substitution pass (per `concept:attribute`). No prior-frame run is consulted; the frame-isolation invariant (per `concept:frame`) forbids reading across the boundary, and the RunScope-per-frame constraint means the query would return nothing regardless.
...
Non-cascade re-runs have a fundamentally different shape than cascade-driven re-runs. They originate from an explicit human action (operator-invalidate), a fanout-parent recalculate after a child completes, or an in-graph message-delivery that targets the receiver-node directly. In each case, the runtime knows the input bag the new run should see — for the two intra-frame paths, the carry-forward from the immediately-prior in-scope run (optionally pre-mutated by an explicit `set_attribute` action for operator-invalidate); for message-delivery, schema defaults overlaid with the message envelope's payload. There is no wait-set to drain, no upstream cascade to overlay, and no mode rule that could justify dropping the row. Policy-retry is NOT a non-cascade re-run path — it is handled in-place on the existing node-run row (see `decision:in-place-retry`); it never creates a new row.
