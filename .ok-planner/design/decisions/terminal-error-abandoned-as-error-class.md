...
Abandoned is also semantically an error in the rollback sense: the held work was rolled back, the executor's output is not authoritative, downstream effects predicated on the run succeeding need compensation. It is not a benign termination like `terminal/success` or a deliberate hold like `terminal/parked/*`. The error namespace is the right home.
