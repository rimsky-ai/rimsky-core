...
Cascade-driven message send lives on a dedicated node-kind, declared on a node-type by `sends_message: <type>` instead of `executor:`. A per-node-type `emits:` block is not introduced.
