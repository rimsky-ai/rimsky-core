...
With a per-instance main RunScope, those same properties would all bleed across frames: carry-forward would carry state from one frame's mid-flight run into the next frame's gate evaluation (the session-resume race described in `decision:frame-isolation`), the dispatcher's serialization gate would refuse to claim a row in one frame while a sibling frame had a same-(node, scope) run mid-flight, and the per-(node, scope) sequence numbering would mean "the latest run in this scope" pointed at a run from a possibly-unrelated frame. Re-introducing a frame qualifier on every query would re-establish isolation at the call site but expose the surface to every missing-filter bug; making the RunScope itself per-frame is the strictly safer structural form.

## Alternatives

Main RunScope: one per instance, created at instance creation, frames and RunScopes orthogonal (the retired model) — rejected. This was the model attempted in mid-2026. Its consequences surfaced empirically as a carry-forward race in session-resume scenarios where RunScope-bounded carry-forward read state from a sibling frame's mid-flight run before that frame's executor writeback committed. The orthogonality framing was incompatible with the frame-isolation property the cascade model requires (per `decision:frame-isolation`); the per-instance singleton was a persistence shape that imposed a model the runtime could not honor.
