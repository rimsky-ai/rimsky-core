---
decision: host-daemon-late-bind-schema-check-deferred
---
...
Rimsky checks a late-bound executor's attribute-schema conformance at each dispatch, once the spawned binary's own capabilities handshake has completed, rather than at template registration. A mismatch settles that dispatch as a contract error (see `concept:host-daemon-proxy`, `concept:template`).
