...
Use the filesystem blob backend rooted at `<run>/blobs/`, with the default spill threshold. The filesystem backend keeps values at or under the threshold inline in the SQL row and spills values above the threshold to a file under the root.
...
Pure-inline backend (large-payload risk; explicitly errors on values that exceed the row-size limit). The memory blob backend (audit gap; tracked separately as the memory-blob-audit-gap tension).
