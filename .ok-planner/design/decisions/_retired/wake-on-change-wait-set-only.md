...
Retired with the removal of the `wake_on_change` field from the subscription language. The original decision drew a distinction between "inserts the wait-set row" (true for any matching subscription) and "stale-marks the receiver" (gated by `wake_on_change`). Under the current model, subscription declarations are exclusively wake declarations — a subscription either wakes the receiver on a matching signal or isn't declared. Substitution input no longer reads from wait-set rows at all (`decision:substitution-deps-from-persisted-senders` reads each subscribed sender's most-recent fresh-settled attribute store directly), so the "wait-set row carries data for substitution even when wake is suppressed" capability the decision protected no longer exists.
