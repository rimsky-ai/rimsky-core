...
- A sequence field — monotonic per (node_id, run_scope_id), assigned at row creation. Drives dispatcher claim order (lowest unblocked sequence wins), the gate evaluator's predecessor lookup for bag composition, and the latest-run lookup that operator surfaces project into the per-state categorical summary (the per-scope monotonicity makes "the latest run for this node in this scope" well-defined within the RunScope; RunScopes never span frames per `concept:run-scope`, so per-scope monotonicity is also per-frame).
