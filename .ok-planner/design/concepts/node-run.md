...
- A settling-signal field — records the signal type the run settled with; cascade-firing is subscriber-driven from the emitted signal (see `concept:signal`), not gated by a stored per-node outcome.
...
`fresh` and `failed` are terminal — no outgoing transitions. Cascade events targeting a settled (or in-flight) node-run create a NEW node-run instead; they never mutate the existing row's bag or identity — with one narrow exception, the parked-wake carve-out (see `concept:cascade`), where a cascade walk targeting an in-flight `parked` run transitions that same row `parked → stale` in place rather than creating a new row.
...
- The queue's claimant guard is unconditional: every claim-guarded queue verb (release, complete, remove) verifies the caller's expected claimant against the row's recorded claim and no-ops on mismatch — a stale supervisor's late write can never clobber a reassigned row. Administrative callers with no claim to assert (pure-cascade settle, parked-row sweep) use an explicitly-named force variant; no sentinel argument value disarms the guard.
...
- In-flight states (`pending`, `stale`, `running`, `held`, `parked`) are sealed against cascade-driven mutation per `concept:cascade`. Cascade events targeting an in-flight run create a new node-run; the existing row's bag and identity are never mutated, except for the narrow parked-wake carve-out (`concept:cascade`), which transitions an existing `parked` row `parked → stale` in place.
