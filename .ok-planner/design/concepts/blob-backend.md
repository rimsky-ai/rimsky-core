...
The blob-backend interface is the abstraction that backs spilled byte streams from attribute values, event payloads, parked payloads, and scratch. It exposes byte-stream IO with a backend-name accessor. Multiple pluggable backends exist, distinguished by where bytes live.
