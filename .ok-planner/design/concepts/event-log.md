...
- `auth.key_revoked` — emitted by the control-plane key-revocation handler and the runtime rotation-grace sweep. Payload: `key_id`, `key_name`, `revoked_by_key_id`, `reason` (`manual | rotation_grace`). Key expiry is a passive per-request check against the key row's expiry timestamp and never produces a revocation event.
