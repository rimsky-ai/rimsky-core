...
Rimsky needs an append-only record of "what happened" for incident review, operator dashboards, and debugging — a record rimsky owns (rimsky-readable JSONB). Adding a new operational kind = adding an enum value in the events proto + regenerating Go bindings (no schema migration; the storage column stays `TEXT`). Rimsky's app logic consumes typed values exclusively, never raw strings, so typo-induced silent observability blind spots are prevented at the app boundary.
