...
- **Self-subscription is first-class.** The "drain my own queue" idiom — a node subscribing to its own signal — keeps iteration inside the current frame: one long-running frame, with the supervisor picking up each new pending self-run as it lands. The cascade walker's insert-then-drain-in-same-tx pattern makes this safe: the new pending self-run's wait-set blocker (keyed on the just-committed run) is drained at the end of the terminal-complete handler in the same transaction, before the supervisor sees it. The cascade stale-mark does not touch the per-instance node row's state — it only inserts a new run row and re-stamps the frame reference — so the just-committed fresh state survives intact.
