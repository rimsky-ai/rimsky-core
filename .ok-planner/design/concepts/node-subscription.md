...
A node-subscription declares a target signal (a canonical signal type-path, exact or wildcarded per `concept:signal`) plus an optional payload predicate, plus two required cascade-shape booleans (a wake-on-change flag and a force-upstream-refresh flag). A sender-side filter selects the specific upstream node-type the subscription reacts to. Subscriptions are declared per node in the template.
...
- Every subscription entry carries both cascade-shape flags; entries missing either are rejected at registration.
