...
> **Retired** by `.ok-planner/specs/2026-05-15-data-platform-extensions-design.md`. Quality rules are replaced by the verifier-executor pattern: a regular executor (e.g. `executors/verifier-shape-checks/`, `executors/verifier-http/`) that co-holds the upstream claim via `holds:`, runs checks, returns `Complete{changed: false}` on pass or `Error{error_class: "verifier_failed"}` on fail. The `graph/qualityrule/` Go package is deleted; the `template_node.QualityRules` field is removed; `quality_rule_failed` events roll into `executor_errored`. There is no successor concept — the pattern is documentation under `docs/agents/verifier-pattern.md`, not a rimsky-core concept.

# Quality rule

## What it is

A template-node-level declarative validation against a node's writeback. Each rule is `QualityRuleSpec{Type, Config, Severity}`, defined in `foundation/spec/qualityrule.go` (the row-type primitive) and re-exported as `qualityrule.Spec` for graph-layer call sites (`graph/qualityrule/spec.go`). The field on the node lives at `foundation/spec/template.go` as `QualityRules []QualityRuleSpec` (re-exported at `graph/node/template.go` as `node.TemplateNodeDef`). Evaluators implement the `Evaluator` interface (`graph/qualityrule/spec.go`):

```go
Evaluate(ctx context.Context, input EvalInput) (passed bool, details string, err error)
```

`EvalInput` carries `NewData` (proposed writeback), `PreviousData` (nullable), and `Cfg` (the rule's `Spec.Config`). Three builtins are pre-registered at `init()` in `graph/qualityrule/eval/rules.go:149-155`:
...
Spec types live in `graph/qualityrule/` (Apache); runtime evaluators live in `graph/qualityrule/eval/` (AGPL per `licensing.yml:25,57`).
...
- The licensing split inside `graph/qualityrule/` (spec Apache, eval AGPL) is unusual within a single concept — adjacent to `module-layout` (Licensing boundary subsection).
