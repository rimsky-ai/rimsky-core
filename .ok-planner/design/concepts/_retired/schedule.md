...
> **Retired** by `.ok-planner/specs/2026-05-15-data-platform-extensions-design.md`. The per-node `schedule:` template field, the `rimsky_schedules` table, and the scheduler tick's cron-fire path are all retired. Cron-style firing is now a sensor kind (`concept:sensor`): templates declare a `sensors:` block with the bundled `sensor-cron` service, which fires observations on a cron expression and pushes invalidate-class messages to rimsky. The "missed fires NOT backfilled" semantic is preserved in the sensor. See `concept:sensor`, `concept:message`. The `rimsky-scheduler` process keeps its periodic-sweep orchestration role (orphan reapers, parked-node timeout, frame-progress watchdog) but loses the cron-fire path.
