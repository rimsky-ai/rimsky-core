...
- Held-durable claim handles persist across instance dispatches (invariant 22). Auto-terminal commit on a `lifetime: durable` claim promotes the row to a committed state; the retention sweep skips committed-durable rows so they live until explicit Release.
