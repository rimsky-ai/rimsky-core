...
- Pipeline order at template registration: (1) the static expected-attributes schema check from the executor's advertised observability capabilities, applied against the merged effective attribute schema (pure rimsky-side, no RPC); (2) validate RPC against each service advertising validation for the relevant role — per-node for the executor and claim-producer roles, per-template for the publisher role; (3) errors at either step reject the registration, warnings surface to the operator.
