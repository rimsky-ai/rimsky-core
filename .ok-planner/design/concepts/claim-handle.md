...
- A version identifier — the canonical version returned by the producer's commit verb, persisted from the base commit response on any producer; surfaces in lineage records (claim-terminal kind) and, for data-processing-capable claims, asset version-history queries.
