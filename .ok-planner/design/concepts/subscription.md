...
A subscription is an impactee-side declaration of "fire me when this upstream topic transitions." Three topic kinds: `state` (any state-machine transition), `attribute` (an upstream node's attribute write), `event` (a named-event emission). Scope: per-node (`node: <upstream>`) or cross-cutting (`instance: true`, matches every node in the instance). Optional filters narrow the match: `when` / `outcome` / `error_class` / `reason` for state topics, `name` for attribute or event topics. Frame modifier (`in` | `next`) controls whether the subscriber fires inside the current cascade frame or in a follow-up frame.
