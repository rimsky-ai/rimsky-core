...
- The cancel walker SKIPS three classes of sibling rows: (a) held-durable rows (`held_durable = TRUE`), preserving the `concept:claim-lifetime` durable-Commit contract; (b) rows already Deleted by an inner recursive walker (defensive re-check after `LockForUpdate`); (c) **rows held by a different supervisor** (`holder_supervisor_id != args.SupervisorID`), per `@blessed-invariant 4` (claimant-guarded release).
- The parent claim_handle row is also gated on `parent.HeldDurable == false` — symmetric with other auto-terminal paths (`CheckAndFireResolution`, `resolveParentClaimChain`).
