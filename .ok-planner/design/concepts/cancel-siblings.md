...
**Cancel-siblings is scoped to the supervisor that holds the parent.** Under multi-supervisor deployments (replicas > 1 on the supervisor StatefulSet), sub-claims of the same parent can be acquired by different supervisor processes. The cancel walker filters mismatched-supervisor siblings out of its walk per invariant 4: a supervisor cannot release claims held by a different supervisor.
