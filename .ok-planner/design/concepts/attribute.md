...
- The "single sanctioned introspection site" claim is actually three sites (the path-walker, the raw-stringifier, and the claim-handle constructor) — see `tension:substitution-introspection-site-count`.
- The grammar lists six kinds inline but some external prose passes cite five — see `tension:substitution-grammar-count-drift`.
...
- 2026-05-19 — Grammar text corrected (retired `deps.*`, added live `trigger.*` and `child.*`) and whole-directive value-lift documented per spec:2026-05-19-multi-instance-template-ergonomics-design. Adjacent `tension:substitution-grammar-count-drift` is partly addressed by this update; the cross-doc-prose sweep over external docs remains open.
...
- 2026-05-28 — open-schema extension-property exemption clarified in the attribute-surface invariant per spec:2026-05-28-quality-of-life-features; an unenumerated property under an executor schema that does not constrain it (no properties block, or additionalProperties not false) is admitted without source/default/executor-readOnly and may be author-marked readOnly, while enumerated properties remain fully checked.
