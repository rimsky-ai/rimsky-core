...
Dry-run resolves from EITHER the request flag OR an identity-bound grant mode, and the grant mode is a **floor** — a key whose matched grant entry is dry-run runs every covered write in dry-run regardless of the flag, and the caller cannot escalate past it (absence of the flag, or a flag explicitly set to false, does NOT lift an identity-bound dry-run floor; the flag can only ADD dry-run on top of an execute grant). The auth middleware threads the resolved mode through the request context; handlers read it back and gate the side-effectful path through a shared preview-response helper that emits the synthetic envelope.
