...
A boundary-crossing dispatch unit. Pushed envelope matched via subscription. Persisted in a message ledger on receipt; delivered to subscribers at frame boundary per the per-instance `frame_delivery_mode` (`coalesce` default; `serial_queue` opt-in).
