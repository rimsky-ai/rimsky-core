...
The envelope carries an identity, the target instance, the typed body (inert), a receipt timestamp, and sender attribution (a sender identifier plus a sender-kind discriminator that distinguishes the three origin classes — operator, publisher, instance). Receivers are decided by subscription to the message type as a node-type. For named message types, one message-receiver-node is materialized in `rimsky_nodes` per declared type at instance creation (`node_type = <message-type>`, empty executor, no `subscribes:`, no `source:` directives); each delivery creates a node-run for that node directly in `stale` state with the message body as its bag, dispatched via the empty-executor `pure_cascade` settle path, with the standard cascade walker propagating `terminal/success` to subscribers. For the empty-message type, no node materializes (the empty-type is reserved-for-runtime per `concept:message-schema`); the empty-message virtual is an edge-level structural-root wake mechanism (sender=`""` in the subscription edge map, per `decision:subscription-edges-only-from-explicit-block`). There is no envelope-side routing field.
