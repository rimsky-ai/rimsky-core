...
- Frame processing never mutates the instance row. The only legitimate frame-processing writes to `rimsky_instances` are message-queue append (on send) and coalesce cancellation (on coalesce-mode enqueue); every other instance-row transition (`terminated_at`, `paused`, params updates) originates from a control-API path, not from frame settlement.
