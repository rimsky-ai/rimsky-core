...
Drained rows are the durable record of "which senders woke this receiver in this frame" — a wake-causality trace, not a data carrier. The gate evaluator builds the receiver's substitution input by querying each subscribed sender's most-recent fresh-settled attribute store directly (per `concept:attribute`), independent of which wait-set rows drained. Cleanup happens via frame cascade-delete.
