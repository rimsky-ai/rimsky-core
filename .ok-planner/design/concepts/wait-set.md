...
- Drain stamps the drain timestamp on rows whose sender matches the settling sender. Drained rows remain queryable by the gate evaluator's bag-overlay step (and by forensic queries).
...
- Non-cascade rows (`creation_reason ∈ {operator_invalidate, policy_retry, infra_reenqueue}`) have NO wait-set rows. They are created directly in state `stale` per `decision:non-cascade-direct-to-stale`; the wait-set / gate-evaluator path applies only to cascade-driven creation.

Drained rows are the durable record of "which senders contributed to this receiver's dispatch in this frame." The gate evaluator's bag-overlay step reads them (filtered to attribute-topic rows, sender state checked against settled-success outcomes) to populate the receiver's input bag at pending→stale transition. Cleanup happens via frame cascade-delete.
