...
- Both wake paths transition the parked row directly to `stale`; the next dispatcher claim re-invokes the executor with the persisted dispatch-time attribute bag (loaded by run-id). The cascade wake fires only for a subscribed upstream's settling cascade and mutates state only — the walker never rewrites the parked row's bag or scratch, and the cascade round lands on a new pending row per `concept:cascade`. A parked row is force-failed only by an instance kill, a cross-cutting termination that applies to any in-flight row and is not park-specific machinery. None of the resume paths transition directly to `running`.
