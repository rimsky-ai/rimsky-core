...
- The body-schema is a registration-time check on substitution references and a receipt-time check on the body bytes themselves: every insertion into the message ledger — operator send, publisher send, and cascade-send alike — validates the body against its declared type's body-schema through the one shared ledger-insertion chokepoint, and a violation refuses the insert naming the offending type and field before any row lands. Delivery trusts the ledger and performs no second validation pass. The body remains inert at receipt (see invariant: 24).
