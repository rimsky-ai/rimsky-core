...
- Write-semantics-envelope → write-semantics-allowed key rename per `spec:2026-05-12-nomenclature-resolution` Group C.2. Single-value config shortcut retired per Group C.1. Resolves `tension:_resolved/yaml-write-semantics-alias`.
...
- 2026-06-06 — Per spec:2026-06-06-comprehensive-gap-closure-design: the sign-off gate binds the run's **effective bound attributes** — the terminal-final delta merged with any incremental writebacks the agent emitted during the run (last-write-wins, the terminal-final delta on top), not whatever rides the terminal report. When the agent uses incremental writeback and omits the terminal-final delta, the executor MUST reconstruct the accumulated writeback state the supervisor will commit and bind the gate to THAT value; otherwise an incremental-writeback run could pass the gate over an absent (empty) terminal delta, letting unsigned or stale-signed output through. The value the gate binds is exactly the value committed, so the bound output equals the persisted output.
