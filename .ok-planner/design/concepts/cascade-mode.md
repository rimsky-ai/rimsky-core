...
|---|---|
| `most-recent` | Prior cascade-driven stale rows for the same `(receiver, run-scope)` in the current frame that have not yet been claimed by the dispatcher are deleted, then the transitioning row becomes the surviving stale. This delete runs before the gate evaluator's advanced-sibling check (per `concept:wait-set`'s conjunct (c)); it is what clears that check for `most-recent`, where `sequenced` has no such delete and instead waits in `pending` until the prior round fully settles. |
