...
- **CA private key encrypted at rest under mtls.** The CA private key is encrypted with AES-256-GCM under an operator-supplied 32-byte key from the environment (`env:RIMSKY_CA_ENCRYPTION_KEY`, base64). When `peer_auth` is `mtls` and that key is missing or malformed, startup fails closed.
- **Leaf certs are short-lived, memory-only, always-rejoin.** An enrolled service receives a leaf certificate with a 24h TTL, its private key, and the CA root via `route:POST /v1/enroll`. Certificates are never persisted service-side; the service auto-renews at ⅔ of TTL. Revoking the api-key stops renewal, and the cert ages out within its TTL — there is no CRL.
