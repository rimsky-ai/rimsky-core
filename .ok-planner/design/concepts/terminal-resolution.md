...
1. **Wire to internal terminal kind** — the executor-outcome reader maps each Outcome variant to an internal terminal-event of kind `Complete | Errored | Park | Infra`. The settling verdict's `tags` set rides into the cascade-walk's CEL filter on `payload.tags` (see `concept:terminal-tag`).
