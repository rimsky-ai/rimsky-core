...
1. **Wire to internal terminal kind** — the executor-outcome reader maps each Outcome variant to one of four internal terminal-kinds: completion, error, park, infra. The settling verdict's tag set rides into the cascade-walk's CEL filter on the tag set (see `concept:terminal-tag`).
