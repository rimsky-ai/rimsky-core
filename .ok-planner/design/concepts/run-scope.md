...
- Depth gating: the persisted parent-chain backs two layers of defense against sub-graph recursion — the canonicalizer's static sub-graph-recursion rejection per `concept:sub-graph` is the primary defense, and a runtime walk of the chain at dispatch time (rejecting a delegate invocation whose target sub-graph is already open in the invoking RunScope's ancestor chain) is the backstop behind it.
...
- A root RunScope's row lifetime is tied to its frame's row lifetime, not merely to closure: frame-trace retention deletes the root RunScope alongside its frame once every RunScope in the tree has closed and cascaded away. Closure (the `closed_at` stamp) and row deletion are distinct events — a root RunScope closes at frame settlement but its row persists, addressable by prior-frame audit and forensics tooling, until trace retention eventually prunes the frame.
