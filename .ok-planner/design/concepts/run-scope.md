...
- A closed RunScope means parent-run rendezvous has fired (sub-graph carry-rule or fan-out aggregation), the owning frame has ended (root), or the owning instance was administratively terminated — termination closes every remaining open RunScope in the instance's frames, root and child alike, firing each scope's run-scope-terminal peer fan-out. The lazy-allocation primitive refuses to allocate into a closed RunScope, surfacing a closed-scope error. Cascade walker reaching INTO a closed RunScope is a bug.
