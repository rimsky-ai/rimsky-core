...
A **claim-terminal record** captures the data-promotion unit at a claim-handle terminal: the handle's identity and place in the claim tree, the producer that resolved it, the substrate identity (as a hash), the disposition outcome, and a cause discriminator for abandon provenance. The disposition outcome distinguishes commit from natural-abandon from force-cancelled abandon; the cause discriminator names which path produced an abandon (the error-policy give-up path, the sibling-cancel walker, or the parent-abandon recursive descent). Concrete value sets and emit-site mappings live with the lineage spec.
