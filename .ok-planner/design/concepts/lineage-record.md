...
The set is deliberately small and paired in lockstep with the OpenLineage facet schema. (A pre-dispatch acquisition failure resolved via a `pass` error-type policy does NOT emit a leaf_run record; the resolution happens before the run enters `running` and there is no run-row yet to anchor the lineage record.)
