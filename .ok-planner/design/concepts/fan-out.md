...
The mechanism: the node holds a parent claim, the producer's partition-split operation takes the parent claim handle plus a partition request and returns N sub-scope descriptors, rimsky opens N sub-claim handles (in the parent-acquisition transaction, per `concept:claim-tree`), and hands the already-acquired sub-claims to the shared dispatch helper (see `concept:child-execution`) as N partitions × 1 child — the child being the calling node itself. Each clone settles individually through the **aggregate** settle primitive, which records the outcome on the parent claim-handle and applies the **aggregation policy** (drawn from `concept:error-policy`'s four-value family). When the policy resolves the parent, the parent's claim Commits or Abandons and the parent-settlement cascade fires.
