...
- A fan-out node may also declare a delegate target (see `concept:delegation`); the two are not mutually exclusive. Each partition clone absorbs the sub-graph entry and dispatches its own delegated sub-graph invocation independently of its siblings; the sub-claim backing that partition settles once that partition's own sub-graph exit carries its outcome back, and the aggregation policy resolves over each partition's carried exit outcome exactly as it would over a plain leaf clone's terminal outcome.
