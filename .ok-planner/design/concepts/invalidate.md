...
- 2026-05-15: **`invalidate` is one `kind` of message (the V1 kind)**. The boundary-crossing messaging primitive is `concept:message`; it has a `kind` field that in V1 carries only the value `invalidate`. Operator-API sends (`POST /instances/{id}/messages`) and sensor observations (`POST /sensors/{watch_id}/observations`) construct invalidate-kind messages and enqueue them in `rimsky_messages`; the cascade walk (in-frame subscription-edge match) is NOT a message — it's a direct stale-mark inside the frame. The retired per-emit `frame: in | next` discipline is subsumed by message-vs-cascade distinction: messages always create a new frame (or join the pending coalesce row); cascade walks always run within the current frame. See `concept:message`, `concept:frame`, `concept:backfill` (backfills are invalidate-kind messages with a `partition_request_override` payload).
