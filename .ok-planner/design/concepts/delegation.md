...
- **The entry node's configuration is absorbed into the calling node; the entry never gets its own node-run.** At canonicalization, the calling node inherits the entry's executor, claim-producer bindings, holds bindings, and attribute schema, merged with what the calling node declared externally (other entry-declared fields do not carry over). The entry keeps its own node identity — its own template node definition and its own persisted node row — but that identity is never independently dispatched: no node-run of the entry is ever created. Internal nodes that subscribe to the entry alias resolve to the calling node's run, not to a separate entry run. The calling node's run remains in the parent execution context (per `concept:run-scope`); the sub-graph's other internal nodes (exit plus any intermediates) run in the child execution context that the dispatch primitive allocates.
