...
Introduced by `spec:2026-05-15-data-platform-extensions-design`. The verifier pattern is the canonical use case: a verifier executor co-holds an upstream staging claim, runs checks, and its terminal contributes to the parent's aggregation; the aggregation outcome drives `Commit` (atomic swap) vs `Abandon` (drop staging).
