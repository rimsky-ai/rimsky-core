...
Without claim-pass, every downstream consumer would need to re-acquire the same scope, risking a different snapshot or a different queue item. The acyclic `from:` rule (see Invariants) is the deliberate constraint that keeps claim lifetimes legible: a node can never (transitively or directly) hold from itself. There is no transitive auto-holdership through subscription chains; if you need a chain, declare co-holdership at every link.
