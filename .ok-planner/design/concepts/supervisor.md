...
One of the three rimsky runtime binaries. Implements the acquisition transaction, dispatch, terminal handling, auto-terminal. Registers itself in a persisted supervisor-registry record at startup carrying its `accepted_executors` / `accepted_stores` / `concurrency` / `callback_host` / `callback_port`. Per-dispatch liveness is observable via `last_progress_at` on the persisted node-run row (bumped by attribute writeback and keepalive POSTs) and via the supervisor's outgoing gRPC client connection state for sync dispatches.
...
Owns: the acquisition tx, the dispatch call, terminal-handler resolution, callback HTTP server, liveness tracking (last_progress_at maintenance via writeback and keepalive callbacks), breakpoint checkpoint evaluation at before_dispatch and after_terminal, blocked-runner polling for resume. Does NOT own: scheduling (see `concept:sensor`), control-plane (see `concept:control-api`), claim-state mutation outside the tx (see `concept:claim-producer`). Adjacent: `concept:node-run`, `concept:claim-handle`, `concept:executor`, `concept:frame`, `concept:error-policy`, `concept:auto-terminal`, `concept:lifecycle-subscriber`, `concept:host-agent-proxy`.
