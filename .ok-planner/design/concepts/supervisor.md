...
Owns: the acquisition tx, the dispatch call, terminal-handler resolution, callback HTTP server, heartbeating. Does NOT own: scheduling (see `scheduler`), control-plane (see `control-api`), claim-state mutation outside the tx (see `claim-producer`). Adjacent: `worker-request`, `claim-handle`, `executor`, `frame`, `lifecycle-handler`, `auto-terminal`.
