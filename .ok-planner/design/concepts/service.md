...
- A bundled service's image declares exactly the ports its binary opens at its built-in defaults. The image declares no port that only an operator's configuration opens, because it carries no default for one. An operator reads these declarations to learn what the service opens, so a check compares each image's declared ports against its binary's built-in defaults.
