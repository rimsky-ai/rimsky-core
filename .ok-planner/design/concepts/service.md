...
- Every bundled out-of-process service binary resolves its serving port through one shared precedence: `env:RIMSKY_AGENT_PORT` first, falling back to the service's own port variable (or its unified-config value) when unset, falling back to a built-in default when neither is set. `env:RIMSKY_AGENT_PORT` is the port-discovery contract a `concept:host-agent` uses to late-bind a bundled binary as a local, per-invocation service; a binary that ignores it cannot be late-bound — the agent's readiness poll against the port it chose simply never observes a listener.
