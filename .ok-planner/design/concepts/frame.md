...
- 2026-05-22 — Clarified orthogonality with `concept:run-scope` per spec `.ok-planner/specs/2026-05-22-fan-out-safety-scope-first-design.md`. Frames and RunScopes are orthogonal: a single cascade frame can span multiple RunScopes (cascade propagation across sub-graph entry-success or fan-out parent settlement); a single RunScope can host multiple frames (the same graph firing across multiple cascade resolutions).
