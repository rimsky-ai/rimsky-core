...
- **The frame row is cascade-immutable.** After the pickup insert, the only write to the frame row is the frame-engine reaper stamping `ended_at` once, atomically, when every node_run has reached a terminal state. Cascade processing never touches the frame row itself; frame-lifecycle observability derives from `ended_at` plus the terminal states of the frame's node_runs, not from a stored state column.
