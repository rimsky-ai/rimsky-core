...
A frame is **held** when one or more of its node-runs is in a non-terminal pause state — `parked` (executor entered the park terminal waiting for a time-based or callback-based wake) or `held` (executor returned a held-claim terminal awaiting auto-terminal commit/abandon per `concept:node-run`). The held-frames diagnostics endpoint on the control API surfaces frames held by a `parked` node-run; held-claim holds are not reported there. They are normal during agent-driven work that includes external decisions and during held-claim work that spans multi-node holding subgraphs; persistently held frames may indicate stuck reviews and warrant investigation. Held-claim auto-terminal fires once every node in the holding subgraph completes, so held-claim release happens at the end of the holding subgraph, not at the per-node held-terminal boundary. A held frame is precisely a running frame with a node_run in `held` or `parked`; every in-flight state holds the frame open for frame-end purposes, but the held-frames diagnostic reports only the `parked` subset of that population.

## Boundaries

Owns: the per-instance concurrency rule (≤1 running frame), the serial-per-instance ordering, the last-progress timestamp, the triggering-message-id pointer that every frame carries, the root-RunScope pointer (created at frame start). Does NOT own: node state (lives on the node-run, see `concept:node-run`), claim conflict (lives in `concept:claim-handle`), scheduling cadence (lives in `concept:sensor`), the message itself (see `concept:message`), the message queue and its coalesce mode (lives on the instance, see `concept:instance`), RunScope internals (see `concept:run-scope`). Adjacent: `concept:cascade`, `concept:node`, `concept:node-run`, `concept:message`, `concept:instance`, `concept:run-scope`, `concept:sensor`.

## Invariants

- At most one `running` frame per instance.
- **The frame row is cascade-immutable.** After the pickup insert, the only writes to the frame row are the frame-engine reaper stamping `ended_at` once, atomically, when every node_run has reached a terminal state, and the last-progress timestamp refreshed on every node-run state transition inside the frame's own lifetime (a liveness heartbeat consumed only by stall detection). Neither write stores cascade state: cascade processing never puts data in the frame row, and frame-lifecycle observability derives from `ended_at` plus the terminal states of the frame's node_runs, not from a stored state column.
- Frames have no pre-run state: every frame row is inserted at the pickup moment and is `running` from that moment on. There is no `queued` state; work waiting for a busy instance sits on the instance's message queue instead.
...
- **No frame processing may mutate persistent identity rows.** The `rimsky_nodes` table row is per-instance identity, set at instance creation, immutable during frame processing (per `concept:node`). The `rimsky_frames` row is likewise immutable during cascade — post-insert it takes only the reaper's one-shot `ended_at` stamp and the last-progress liveness heartbeat refreshed on node-run state transitions, never cascade data. Frame processing may mutate only per-run rows (`rimsky_node_runs`, `rimsky_node_attributes`), wait-set rows, and RunScope rows that were themselves created inside the same frame, append-only writes to the audit-event ledger, plus the instance's message queue via the two channels enumerated in `concept:instance`.
...
- **Confusing "cross-frame coupling" (of workflow shape) with "cross-frame state coupling" (of persisted data reads).** Multi-frame workflows are legitimate — a node sends a message, the next frame opens on that envelope, the receiver reads the message body. What travels is the message body. What does not travel is any observation of prior-frame node-run state.
