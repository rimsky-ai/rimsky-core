...
- **The frame row is cascade-immutable.** After the pickup insert, the only writes to the frame row are the frame-engine reaper stamping `ended_at` once, atomically, when every node_run has reached a terminal state, and the scheduler refreshing the last-progress timestamp as runs progress inside the frame's own lifetime (a liveness heartbeat consumed only by stall detection). Neither write stores cascade state: cascade processing never puts data in the frame row, and frame-lifecycle observability derives from `ended_at` plus the terminal states of the frame's node_runs, not from a stored state column.
