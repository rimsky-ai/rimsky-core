...
- **Root** (`github.com/fallguy/rimsky`) — graph layer (`graph/`) + runtime layer (`runtime/`) + control layer (`control/`) + cmd binaries (`cmd/`) + bundled stores (`stores/`) + bundled executors (`executors/`) + bundled sensors (`sensors/`) + bundled lifecycle-subscribers (`subscribers/`) + reference examples (`examples/`) + dashboards (`dashboards/`). Pulls heavier libs (jsonschema, robfig/cron, jcs, testcontainers). The shared pgtest fixture lives at `internal/pgtest/`.
