...
- **Root** (`github.com/fallguy/rimsky`) — graph layer (`graph/`) + runtime layer (`runtime/`) + control layer (`control/`) + cmd binaries (`cmd/`) + bundled stores (`stores/`) + bundled executors (`executors/`) + dashboards (`dashboards/`). Pulls heavier libs (jsonschema, robfig/cron, jcs, testcontainers). The shared pgtest fixture lives at `internal/pgtest/`.
