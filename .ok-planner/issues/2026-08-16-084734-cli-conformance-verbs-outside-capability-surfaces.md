...
The rimsky concept describes the operator binary as six capability surfaces (dev-loop, compose, resource, context, authentication, host-agent control) and states an invariant that the binary is an HTTP+JSON client with no protobuf. The conformance verb group — eight subcommands that dial a peer service over gRPC to prove it implements a protocol, shipped as its own image — fits none of the six and speaks protobuf. The concept explicitly declines to enumerate verb membership, so nothing in it settles whether conformance is a seventh surface, out of scope, or misplaced in this binary; and the "no proto" invariant was already false at binary scope for the self-host modes the concept blesses. The ruling decides where conformance lives in the model.
