...
The audit measured user assumptions about the HTTP routes, the MCP transport, and the api-key and permission surface by building 24 experiments — each a self-contained probe that boots its own containers from the tree's image tag, picks free ports, and exits non-zero when a check fails. All 24 pass at the audited tree: 1 demonstrate the product honouring a prior (held), 23 demonstrate it contradicting one (trap). None is in the project's suites; the audit adopts nothing itself and re-runs them only at the next audit. A held probe is a candidate ordinary regression test; a trap probe is a candidate expected-fail test that starts failing the day the trap is closed. The decision on scenario tests fixes how such tests are written, not which probes are promoted. The ruling decides the adoption policy for this family — one policy applied to four sibling families is the natural shape.

## Options

- Adopt held probes as scenario tests and trap probes as expected-fail scenario tests, retiring each experiment directory as its test lands; cost: 24 maintained tests, each rewritten to the scenario harness.
