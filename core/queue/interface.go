...
//
// # Claim-time eligibility model (spec §7.3 step 1–§7.3)
//
...
// supervisor's runner.go orchestrates the §7.3 atomic-acquisition tx end
...
//     named-lock count, region conflict via byte-equal comparison on
//     `region_data` bytes (per spec §7.7).
//  3. On the first eligible candidate, takes per-named-lock advisory
//     locks (§7.3 step 3b), calls ClaimDispatchRow to do the claimant-
//     guarded UPDATE (§7.3 step 3c), re-evaluates region locks under tx
//     (§7.3 step 3d), then for each spec calls Store.Open (for
//     ClaimSpec) and inserts a rimsky_lock_holders row (§7.3 step
...
type DispatchQueue interface {
	...
	// a nil tx.
	//
	// This is spec §7.3 step 1. Per-spec lock eligibility (step 2) is
	...
	// occur — the guard is defensive per spec §7.3 step 3c).
	...
	// older than cutoff. Spec §7.5 step 1: the dispatch-claim sweep
	...
	// for every row claimed by supervisorID. Spec §7.5 — paired with the
