...
func TestSubstitute(t *testing.T) {
	...
	cases := []tcase{
		...
		// claim region
		{name: "claim region", raw: "{{claim.topics-ring.region}}", want: "row-7"},
		{name: "claim region takes no field", raw: "{{claim.topics-ring.region.x}}", wantMissing: true,
			missingSubstr: "region takes no further field path"},
