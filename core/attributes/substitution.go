...
//     entry carries Address / Payload / Region as opaque bytes.
...
type ResolveContext struct {
	...
	Claim  map[string]store.ClaimResult
...
//   - claim.<alias>.region   → leaf is ClaimResult.Region bytes
//
...
func resolveClaim(directive string, rest []string, claims map[string]store.ClaimResult) (string, error) {
	if len(rest) < 2 {
		return "", &ErrMissingSource{Directive: directive, Reason: "claim directive needs <alias>.{address|region|payload.<field>}"}
	}
	alias := rest[0]
	cr, ok := claims[alias]
	if !ok {
	...
	case "region":
		if len(rest) != 2 {
			return "", &ErrMissingSource{Directive: directive, Reason: "claim.<alias>.region takes no further field path"}
		}
		if len(cr.Region) == 0 {
			return "", &ErrMissingSource{Directive: directive, Reason: "claim region is empty"}
		}
		return stringifyRaw(cr.Region), nil
	case "payload":
		if len(rest) < 3 {
