...
func newSQLiteDriver(t *testing.T) persistence.Driver {
	t.Helper()
	dir := t.TempDir()
...
func TestHandler_ListDispatches_Empty(t *testing.T) {
	...
	r := newRouter(t, deps)
	req := httptest.NewRequest("GET", "/v1/observability/dispatches", nil)
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
	if w.Code != 200 {
		t.Fatalf("status = %d, body = %s", w.Code, w.Body.String())
	...
	if len(body.Dispatches) != 0 {
		t.Fatalf("dispatches = %+v, want empty", body.Dispatches)
	}
}
...
func TestHandler_SystemSummary_DispatchCounts(t *testing.T) {
	...
	if _, ok := body["dispatches_claimed"]; !ok {
		t.Fatalf("missing dispatches_claimed: body=%+v", body)
	}
	if _, ok := body["dispatches_pending"]; !ok {
		t.Fatalf("missing dispatches_pending: body=%+v", body)
	}
}
