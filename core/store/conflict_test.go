...
// TestRegionsByteEqual covers the rimsky-side byte-equal region conflict
// predicate (per v3 spec §7.7). Empty regions never conflict; identical
// bytes conflict; different bytes do not.
func TestRegionsByteEqual(t *testing.T) {
	cases := []struct {
		name string
		a, b []byte
		want bool
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := RegionsByteEqual(tc.a, tc.b)
			if got != tc.want {
				t.Fatalf("RegionsByteEqual(%q, %q) = %v, want %v", tc.a, tc.b, got, tc.want)
			}
		})
	}
}
...
func TestModeCoexistsMatrix(t *testing.T) {
	...
	syncSemantics := []WriteSemantics{WriteSemanticsDirect, WriteSemanticsStagedBlocking}
...
func TestModeCoexistsCrossQuadrant(t *testing.T) {
	if !ModeCoexists(IntentRead, WriteSemanticsDirect, IntentRead, WriteSemanticsStagedAsync) {
		t.Fatal("cross-quadrant r×r should report no conflict")
	}
	if !ModeCoexists(IntentReadWrite, WriteSemanticsDirect, IntentReadWrite, WriteSemanticsStagedAsync) {
...
func TestModeCoexistsSymmetric(t *testing.T) {
	...
	semantics := []WriteSemantics{WriteSemanticsDirect, WriteSemanticsStagedBlocking, WriteSemanticsStagedAsync}
