...
func handleCreateInstance(deps AppDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		// rimsky_store_lifecycle bookkeeping), so a partial-failure on
...
// against the stores recorded in rimsky_store_lifecycle for the given
...
func fanOutInstanceTerminatedFromLifecycleRows(
	...
	if deps.Stores == nil {
		return errors.New("store registry not initialized")
	}
	for _, r := range rows {
		s, ok := deps.Stores.Get(r.StoreRegistrationName)
		if !ok {
			deps.Logger.Warn("instance_delete.unknown_store_in_lifecycle",
