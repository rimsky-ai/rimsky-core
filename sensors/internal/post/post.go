...
// Package post implements the shared publisher-side message-emit
// retry-with-backoff helper used by every bundled sensor (sensor-cron,
// sensor-http, sensor-object-store, sensor-webhook).
//
// Spec
...
//
// Single ~30-line helper deliberately co-located in `sensors/internal`
// rather than `foundation/`: the helper is publisher-side only (rimsky
// is the receiver, not a caller), and `sensors/internal/post` is
// invisible to non-sensor code by Go's internal-package visibility
// rules.
package post
