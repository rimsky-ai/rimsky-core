...
// tickerFixture bundles a real Postgres-backed persistence.Database plus a
// seeded template + instance so each test can insert nodes and schedule
// rows directly through the persistence.Tables accessors.
...
func newTickerFixture(t *testing.T) *tickerFixture {
	...
	return &tickerFixture{persist: d.Tables(), instance: inst}
}
