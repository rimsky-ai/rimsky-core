...
- **Removed cross-repo references from this repo's build/release tooling; the repo is now self-contained.** Two reorg leftovers reached into sibling repos by relative filesystem path. (1) **Deleted `scripts/release.sh`** — its only function was the pre-release docs-reconciliation gate, which shelled into a `../rimsky-docs` checkout to run the docs-lint binaries. Docs reconciliation is now owned by the `rimsky-docs` repo, which lints against this tree via the `RIMSKY_REPO` convention; this repo carries no docs gate. (2) **Removed the broken `cli-sync-embedded` Makefile target** — it regenerated the embedded CLI compose from a root `deploy/` directory that the P4 reorg moved out to `rimsky-docs`, so the target's source no longer existed. The embedded copies under `control/cli/embedded/deploy/` are now the canonical, self-contained assets; `embedded.go`'s package comment was updated to say so, and the target was dropped from `.PHONY`. (3) **Scrubbed sibling-repo relative paths from comments** — `.golangci.yml`'s `consumption-side-isolation` rationale (`../rimsky-services`), the `Makefile` docs-tooling comment (the `../rimsky-docs/cmd` invocation example + the `scripts/release.sh` gate note) and the `core-images` comment (`rimsky-docs/deploy/build-images.sh`). (4) **Repointed pointer docs** that named the deleted `scripts/release.sh` — `CLAUDE.md` and `feature-index.md` now state that docs reconciliation lives in `rimsky-docs`. (5) **`licensing.yml`** lost its now-dead `deploy/` entry (moved to `rimsky-docs`, replaced with a breadcrumb comment matching the `docs/`/`dashboards/` pattern) and its `scripts/` entry (emptied by the `release.sh` deletion). Pointer docs that merely note where a moved surface now lives (e.g. "docs live in `rimsky-docs`") were kept — they orient a future session without creating a filesystem dependency. **Verification:** `go build ./...`, `make lint`, `make license-lint` (256 apache / 536 agpl, 0 violations) all clean; `make -n core-images cli-image` parses.

- **GitHub org migration: `fallguyconsulting` → `rimsky-ai` (git paths only).** The repo and its siblings (`rimsky-docs`, `rimsky-services`, `rimsky-dashboard`) moved to the `rimsky-ai` GitHub org. Search/replaced `github.com/fallguyconsulting/` → `github.com/rimsky-ai/` and `ghcr.io/fallguyconsulting/` → `ghcr.io/rimsky-ai/` across the tree: the four `go.mod` module paths (root + `foundation` + `protocols` + `testpg`), all Go import sites, `.golangci.yml` depguard rules, the `.proto` `go_package` options (with `make proto-gen` regenerating `protocols/proto/v1/gen/*.pb.go` — the bare text-replace had corrupted the length-prefixed `go_package` strings inside the `rawDesc` blobs), deploy image references (`control/cli/embedded/deploy/docker-compose.yml`), and the doc/pointer files (`CLAUDE.md`, `README.md`, `feature-index.md`, `Makefile`, `licensing.yml`, `scripts/release.sh`). The committed `origin` remote URL had a transposition typo (`rimksy-ai`, a nonexistent org) and was corrected to `git@github.com:rimsky-ai/rimsky-core.git`. Because the repository is named `rimsky-core` (not `rimsky`), the Go module path was then renamed `github.com/rimsky-ai/rimsky` → `github.com/rimsky-ai/rimsky-core` (root module + the `foundation`/`protocols`/`testpg` submodules + every import site + `.golangci.yml` depguard + `.proto` `go_package`, with `make proto-gen` rerun) so the module path matches the repo and external `go get` resolves — the original `/rimsky` path matched no repo. Sibling-repo references (`rimsky-services`, `rimsky-docs`, `rimsky-dashboard`) were left intact. Image-registry references are interim `ghcr.io/rimsky-ai/` placeholders: image publishing will target the DockerHub org `rimskyai` when it begins (deferred — no images pushed yet). Left untouched per "git paths only": the `@fallguyconsulting.com` contact emails in `CONTRIBUTING.md`, `CLA.md`, `COPYRIGHT`, `TRADEMARKS.md` (the company name, not a git path). **Verification:** `make proto-gen` regenerated all 10 gen files; `go build ./...`, `go test ./...`, `make lint` clean.

- **Fixed Docker image builds + remaining prose left pointing at the deleted `sdk/go` module by the collapse.** Follow-up to the two bullets below; these surfaces are outside the Go-source scrub the prior follow-up covered, so `make build-all`/`make lint` did not catch them. (1) **Build-breaking bug:** the five Dockerfiles (`Dockerfile.go-base`, `Dockerfile.all`, `Dockerfile.cli`, `stores/stub/Dockerfile.stub`, `executors/stub/Dockerfile.stub`) still `COPY sdk/go/go.mod sdk/go/go.sum ./sdk/go/` before `go mod download`. Since `sdk/go/go.mod` was deleted and `go.work` now lists `./testpg`, every image build (`make core-images`, `make cli-image`, `deploy/build-images.sh`) would fail at that COPY step (missing source) and would not stage the new `testpg` manifest the workspace requires. Repointed all five to `COPY testpg/go.mod testpg/go.sum ./testpg/`. (2) `scripts/release.sh` header comment's future-work note read "tagging (root + sdk/go lockstep)"; updated to "root + protocols lockstep" (`protocols` is the public module now). (3) `CLAUDE.md` said "`go.work` ties the three Go modules together" (always four; the fourth was `sdk/go`, now `testpg`) and its module-layout pointer omitted the fourth module — corrected the count to four, named all four (root, `foundation`, `protocols`, `testpg`), and added `protocols-purity` + `consumption-side-isolation` to the depguard-rule list.

- **Scrubbed stale `sdk/go` / `SDK` / `sdk-purity` references left in doc-comments, annotations, and one runtime error string by the `sdk/go`→`protocols` collapse.** Follow-up to the collapse below; no behavior change beyond the user-visible error string. (1) The `protocols/conformance/executor` client error returned on an unsupported transport now reads "protocols conformance supports grpc only" (was "sdk conformance"). (2) Repointed doc-comment/annotation references in `protocols/conformance/executor/{client,runner,scenario}.go`, `protocols/conformance/blobbackend/runner.go`, and `protocols/publisherkit/publisher.go` from the deleted `sdk/go` paths and "SDK" wording to their new homes (`protocols/conformance/...`, the protocols module, `protocols-purity`). (3) Rewrote the 13 `pkg:sdk/go/conformance/...` citations + "the SDK's" wording across the 7 `cmd/rimsky-*-conformance` binaries to `pkg:protocols/conformance/...` / "the conformance library's". (4) `foundation/internal/pgtest/pgtest.go` `@reason:` blocks now name `testpg` and give the correct rationale — `foundation` does not import `testpg` to avoid a `foundation→testpg` cross-module test dependency (not a depguard rule, which never named `testpg`). (5) `internal/ops/slog.go` doc no longer frames the package as SDK-exported. (6) Three test doc-comments (`test/scenarios/{parked_lifecycle,executor_blocked}_test.go`, `runtime/sweep_claim_handle_retention_test.go`) repoint the testcontainer-latency cross-reference from `sdk/go/testpg/testpg.go` to `testpg/testpg.go`. Verified: `grep -rn 'sdk/go\|sdk-purity' --include='*.go'` returns nothing; `make build-all` + `make lint` green; `foundation` builds + vets clean.

- **`sdk/go` collapsed into `protocols` — one public Go module (`plan:2026-05-26-collapse-sdk-into-protocols`, per `spec:2026-05-26-collapse-sdk-into-protocols-design`).** `protocols` is now the single public-facing Go module a service implementer depends on; the satellite `sdk/go` module is deleted. Rationale: `protocols` is the true dependency leaf (everything depends on it; it depends on no rimsky package), so the genuinely-contract helper code belongs *inside* it, not re-exported through a satellite that carried a postgres/docker dependency a contract surface has no business shipping. Changes: (1) the `action` claim-producer pick-policy vocabulary, the `conformance` library, the `server` scaffolding, and the `publisher` helpers moved into `protocols` — landing as `protocols/action`, `protocols/conformance/...`, `protocols/serverkit` (was package `server`), and `protocols/publisherkit` (was package `publisher`); contract packages keep bare names, convenience scaffolding takes a `kit` suffix to make the contract-vs-convenience line visible in the import path. `protocols/go.mod` gains exactly two pure-Go direct deps as a result: `github.com/google/uuid` (from `conformance`) and `gopkg.in/yaml.v3` (from `action`'s custom YAML unmarshaler); grpc + protobuf were already present. (2) The generic `ops` glue (DSN/slog/health, no rimsky-specific content) was demoted to `internal/ops`, making it core-internal and non-public; its per-file headers flipped Apache → AGPL (the `internal/` prefix is AGPL — correct, since `ops` is being de-published precisely because it is not contract). (3) The plain-Postgres testcontainer helper (`testpg`) was carved into its own opt-in, test-only top-level module (`./testpg`) carrying the testcontainers + pgx dependency tree alone, so neither `protocols` nor any consumer that doesn't want a Postgres test container inherits it; `go.work` drops `./sdk/go` and adds `./testpg`. (4) The `foundation/locks` contract-type leak is closed: the 17 `claimproducer` re-export aliases in `foundation/locks/types.go` are removed, making `protocols/claimproducer` the single canonical Go home for the claim-producer contract types; rimsky-internal users that need a contract type import `protocols/claimproducer` directly. After this, no external consumer needs `foundation` at all. (5) Lint: the `sdk-purity` depguard rule was renamed to `protocols-purity` and retargeted at `**/protocols/**` (denies every rimsky-internal layer plus `testcontainers-go`); `pgx-isolation`'s allow-list entry flipped from `sdk/go` to `testpg`; the `foundation-purity` / `graph-purity` rules dropped their now-dead `…/sdk/go` deny entries; and `consumption-side-isolation` gained an exemption for the in-repo test-infra stub (the stub store/executor entrypoint), so it may import the now-AGPL `internal/ops`. (6) Licensing (`licensing.yml`): the `sdk/go/` entry was removed, `testpg/` added under the Apache block, and the stale top-level `conformance/` entry (a dead leftover from the 2026-05-24 SDK extraction) cleaned up. A Pass-5 reconstruction additionally reclassified the stub store's `cmd/` entrypoint as AGPL (flipping that one file's header) so it can import the now-AGPL `internal/ops` — the depguard exemption covers layering, but the separate license-direction boundary (Apache may not import AGPL) needed this carve-out. The move is boundary-clean: all moved code was already Apache, lands under an Apache prefix, and imports no AGPL rimsky package, so the import-direction license lint stays green.

- **Host-agent/proxy correctness fixes from final code review of `plan:2026-05-24-host-agent-and-proxy`.** Eleven defects fixed across the proxy, host-agent, supervisor runtime, and control-api. (1) **`OnRunScopeTerminal` reap was a complete no-op.** The proxy keys lazily-spawned children by instance id (its v1 dispatch-observable scope), but the reap matched on `run_scope_id`; production fires real run-scope ids (≠ instance id), so the spec-mandated primary reap path silently found nothing and spawned dev-machine children leaked until agent disconnect. Added `string instance_id` to `proto:lifecycle.proto::OnRunScopeTerminalRequest` (regenerated bindings; the Go alias `code:protocols/lifecycle/types.go::OnRunScopeTerminalRequest` gained `InstanceID`), populated it at all three firing sites (`code:control/controlapi/instance_terminator.go`, `code:control/controlapi/instances.go::handleDeleteInstance`, `code:runtime/subgraph_dispatch.go`, `code:runtime/auto_terminal_chain.go` via the two `FanOutRunScopeEvent` helpers which gained an `instanceID` param), and the proxy now reaps by `instance_id` (`code:cmd/rimsky-host-agent-proxy/lifecycle_handler.go::OnRunScopeTerminal`, falling back to `run_scope_id` only for pre-field callers). (2) **Test `TestOnRunScopeTerminalReapsSpawns` masked the bug** by passing the instance id as the run-scope id — now fires a distinct run-scope id + the instance id, and a new scenario `code:test/scenarios/host_agent_reap_test.go::TestHostAgentReapOnRunScopeTerminal` drives a real DELETE → main-run-scope close and asserts the spawned child is SIGTERM'd (via a new `STUBCHILD_TERM_LOG` marker). (3) **Claim-producer Abandon/Release silently executed Commit on the child.** The agent decoded any non-Open claim-producer payload as a `CommitRequest` and called `Commit` — committing work that an `Abandon`/`Release` should have discarded (a state-integrity defect, since the three request messages are byte-identical at `claim_id`). Added `ClaimProducerVerb claim_producer_verb` to `proto:host_agent.proto::DispatchFrame`; the proxy stamps the verb (`code:cmd/rimsky-host-agent-proxy/claim_producer_handler.go::forwardUnary`) and the agent dispatches the exact RPC (`code:runtime/hostagent/dispatch.go::forwardClaimProducerUnary`). New `code:runtime/hostagent/spawn_test.go::TestDispatchClaimProducerVerbFidelity` drives Abandon+Release through the real agent and asserts the child saw exactly those verbs (via a new `STUBCHILD_VERB_LOG` in the stubchild fixture). (4) **Async callback routed to the wrong ack-id when a spawn served >1 dispatch.** The proxy returned the spawn's stored `originalCallback` verbatim, ignoring the per-callback path; a node firing twice in one run-scope sent its second callback to the first dispatch's `/v1/callback/{ack_id}`. `code:cmd/rimsky-host-agent-proxy/http_forward.go::targetURL` now un-rewrites using the forward's own path against the recorded supervisor host:port. New `code:cmd/rimsky-host-agent-proxy/lifecycle_handler_test.go::TestLocalHttpForwardReusedSpawnPerCallbackPath`. (5) **Send-on-closed-channel panic race on agent displacement.** `deliverDispatch` sent outside the lock that `closeAllStreams` closes under; a displacement racing an in-flight dispatch could panic the proxy. Both now operate under `pendingMu` (`code:cmd/rimsky-host-agent-proxy/state.go`). (6) **Disconnect-race child leak in the agent.** A `runSpawn` goroutine in flight at stream close could register a child after `reapAllOrphans` drained the map. `code:runtime/hostagent/run.go::serve` now cancels the connection context and joins a `spawnWG` before returning so the drain sees every child. (7) **Supervisor run-scope fan-out could roll back the load-bearing close on a lifecycle-idempotency DB error.** `code:runtime/lifecycle_fanout.go::FanOutRunScopeEvent` now logs-and-continues on `Get`/`Upsert` errors (returns nothing; the close stands) — matching the per-peer subscriber-error convention and the spec's "close must not roll back on fan-out failure" intent. (8) **Uncancellable hot-path DB call.** The late-bind binding lookup ran on `context.Background()`. Threaded the caller's context: `code:runtime/executor/resolver.go::DispatchContext` gained a `Ctx` field (set at `code:runtime/runner_dispatch.go`) and `code:foundation/locks/registry.go::GetWithContext` gained a leading `ctx` param (set at all three runtime call sites). (9) **Supervisor-side cancellation never signalled the agent.** `code:cmd/rimsky-host-agent-proxy/executor_handler.go::pump` now sends a `DISPATCH_FRAME_KIND_CANCEL` frame on `ctx.Done()`, and the agent honors an inbound CANCEL by cancelling the child's `Execute` stream (`code:runtime/hostagent/dispatch.go::handleDispatchFrame` + a per-stream cancel registry). New `code:cmd/rimsky-host-agent-proxy/executor_handler_test.go::TestExecuteSupervisorCancelSendsCancelFrame`. (10) **Idempotent instance re-create fanned out request bindings + the re-creator's identity, not the persisted row's bindings + owner.** `code:control/controlapi/instances.go::handleCreateInstance` now fans out the persisted row's `service_bindings` AND `created_by_api_key_id` on the idempotent re-create path — the request body's bindings could diverge and clobber the proxy's cache, and a re-create by a *different* api-key would stamp the wrong owner on the `OnInstanceCreated` payload and poison the proxy's owner-routing (so a dispatch would route to the re-creator's agent instead of the original creator's). Both the binding catalog (`fanOutBindings`) and the owner (`fanOutOwner`) are now hoisted from the persisted row. (11) **Stale-spawn spurious error on displacement.** `code:cmd/rimsky-host-agent-proxy/state.go::registerAgent` now drops the prior connection's spawns on displacement so a dispatch can't resolve a spawn the new agent has no child for (which surfaced a spurious `executor_crashed`). New `code:cmd/rimsky-host-agent-proxy/state_test.go::TestRegisterAgentDropsPriorSpawnsOnDisplacement`. **Verification:** `cmd:make proto-gen` + `cmd:go build ./...` + `cmd:make lint` clean; `cmd:go test ./cmd/rimsky-host-agent-proxy/... ./runtime/hostagent/... -race -count=1` + `cmd:go test ./runtime/... ./control/controlapi/... ./control/cli/... -count=1` + `cmd:cd foundation && go test ./locks/... ./persistence/... -count=1` + `cmd:go test ./test/scenarios/... -run TestHostAgent -count=1` clean.

- **`rimsky run` late-bind flags + `rimsky auth login` + per-context api-key + end-to-end scenario tests + design-doc catalog landed (Pass 10 / final of `plan:2026-05-24-host-agent-and-proxy`, per `spec:2026-05-24-host-agent-and-proxy-design`).** (1) **`code:control/cli/run.go::RunRun`** gains additive flags: `--template <name>` (resolves an already-registered template via `route:GET /templates/{ref}`, mutually exclusive with the positional `<file>`; both → exit 2), `--param k=v` (repeatable, merged over `--params <json>` with later-wins precedence; bool/int/float coercion), and `--service <name>=<path>|<name>` (repeatable; `name=path` explicit, bare `name` alias-resolved). When any `--service` is supplied and no live agent is recorded, `run` auto-starts the host-agent inline — v1 connection-state contract is PID-existence only (read `~/.rimsky/agent.pid` + signal-0). Resolved bindings ride the `route:POST /instances` body as `service_bindings`. (2) **`code:control/cli/auth_login.go::RunAuthLogin`** — interactive `rimsky auth login` (sibling to `auth init`, not a replacement): prompts for URL (default = active-context endpoint) + api-key (`golang.org/x/term.ReadPassword` on a terminal, scriptable line-read off a pipe otherwise), optionally verifies via `route:GET /auth/status`, writes the key into the active context and `SaveConfig`s. Wired into `code:control/cli/auth.go::RunAuth` as `case "login"`. (3) **`code:control/cli/config.go::Context`** gains `APIKey string` (`yaml:"api_key,omitempty"`; existing configs without it still load), consumed by the `concept:host-agent` for outbound auth. (4) **`code:control/cli/aliases.go::LoadServiceAliases`** — merges `~/.rimsky/aliases.yml` (global) under `.rimsky/aliases.yml` (project-local overlay) for client-side `--service` resolution; missing/malformed files are skipped. The server never sees aliases. (5) **Fixed a latent bug**: `route:GET /instances/{id}` (`code:control/controlapi/instances.go::instanceItem`) now exposes `service_bindings` + `created_by_api_key_id`, which the `concept:host-agent-proxy`'s GET-fallback fetcher reads but the response previously omitted — the documented fallback could never populate the binding cache. (6) **Scenario tests** under `test/scenarios/` (`host_agent_late_bind_executor_test.go`, `host_agent_failure_modes_test.go`, shared `host_agent_harness_test.go`) stand up the real stack end-to-end — the `cmd:rimsky-host-agent-proxy` binary exec'd on a free port + the in-process `code:runtime/hostagent.Run` daemon + the `runtime/hostagent/testdata/stubchild` binary as the late-bound service — and exercise the happy path (dispatch traverses supervisor → proxy → agent → stub → `terminal/success`) plus the failure modes (`host_agent_not_connected`, `binding_not_found`, `spawn_failed`, `host_agent_disconnected`, proxy-reconnect-recovery). The `code:graph/scenario` harness gained `HarnessOpts.LateBindServiceProxies` / `ExecutorProtocols` (wires a `LateBindResolver` + lifecycle-peer dialing + `LifecyclePeersForSpec`) and `MintAdminKey` / `CreateInstanceWithServiceBindings` / `DeployTemplateSpecMap` helpers. (7) **Design docs** (`concept:host-agent`, `concept:host-agent-proxy` created; mutations to `concept:executor` / `concept:claim-producer` / `concept:service` / `concept:instance` / `concept:rimsky-yml` / `concept:template` / `concept:lifecycle-subscriber` / `concept:supervisor` / `concept:error-policy` / `concept:conformance` / `concept:rimsky`; the `callback-hostname-split` tension addendum + three new tensions `unreachable-service-row-stall` / `anonymous-mode-locks-out-late-bind` / `internal-service-auth-unspeced`). **Verification:** `cmd:go build ./...` + `cmd:go test ./control/cli/... ./test/scenarios/... -count=1` + `cmd:make lint` clean.
- **Host-agent daemon + `rimsky agent` CLI subcommand landed (Pass 9 of `plan:2026-05-24-host-agent-and-proxy`, per `spec:2026-05-24-host-agent-and-proxy-design`).** The agent end of the `proto:host_agent.proto::HostAgent.Connect` stream whose proxy end landed in Pass 8 — `concept:host-agent`. It dials the proxy outbound, exec()s local binaries as rimsky services, tunnels their gRPC streams + local HTTP callbacks back through the bidi stream, and reaps on signal. (1) **New importable `pkg:github.com/rimsky-ai/rimsky-core/runtime/hostagent`** — `code:runtime/hostagent/config.go::Config` (config-as-visible-object: `RimskyURL`, `APIKey`, `ListenAddr` (empty → `127.0.0.1:0`), `AllowPaths`, `AgentLabel` (default `<hostname>-<pid>`), `LogLevel`, `HeartbeatInterval` default 10s, `ReapGracePeriod` default 30s) with `LoadConfigFromEnv` reading `env:RIMSKY_URL`/`env:RIMSKY_API_KEY`/`env:RIMSKY_AGENT_LISTEN`/`env:RIMSKY_AGENT_LABEL`/`env:RIMSKY_LOG_LEVEL`/`env:RIMSKY_AGENT_HEARTBEAT_SEC`/`env:RIMSKY_AGENT_REAP_GRACE_SEC` and a `withDefaults` for hand-built configs. (2) **`code:runtime/hostagent/run.go::Run`** — binds the local HTTP listener once, then reconnect-with-backoff loops: `connectOnce` dials the proxy (insecure transport, v1), opens `Connect`, sends `Register{api_key, agent_label, agent_version, local_callback_base_url=http://<bound-addr>}`, awaits `RegisterAck`. Per connection a single-writer-locked `send` (gRPC client streams forbid concurrent Send), a heartbeat goroutine emitting `HostAgentHeartbeat` at the configured cadence, and a reader loop routing inbound `ServerFrame`s (Spawn/Reap/DispatchFrame out-of-band via goroutines so a slow child can't wedge the reader; HttpResponse delivered inline). On stream close every still-live child is orphaned and SIGKILLed after `ReapGracePeriod`; on ctx-cancel the daemon shuts down cleanly. (3) **`code:runtime/hostagent/local_http.go`** — the catch-all listener wraps each spawned-child HTTP request as `LocalHttpForward{forward_id, method, url, body, headers}`, tunnels it, and awaits the matching `LocalHttpResponse` on a `forward_id`-keyed pending-channel map (30s bound → 504; no live stream → 503). (4) **`code:runtime/hostagent/spawn.go::handleSpawn`** — validates `binding.path` against `AllowPaths` (`filepath.Match` globs; empty → permissive), allocates a free localhost port, exec()s the binary with `env:RIMSKY_AGENT_PORT=<port>` injected (the plan-invented v1 contract: spawned rimsky-service binaries read it and bind their gRPC server there; the child communicates no port back), `cmd.Dir = spawn.Cwd`, poll-dials the port until ready or `ready_timeout_seconds`, runs the `Capabilities` handshake for each `expected_protocol` (`ExecutorObservability.Capabilities` for `executor`, `ClaimProducer.Capabilities` for `claim_producer`), `proto.Marshal`s each into `SpawnAck.Capabilities[protocol]`, registers the live child, and returns `SpawnAck{READY, capabilities}` or `{FAILED, error{spawn_failed, msg}}`. `handleReap` SIGTERMs, SIGKILLs after `sigterm_grace_seconds` (fallback `ReapGracePeriod`), waits, returns `Reaped{clean}`. (5) **`code:runtime/hostagent/dispatch.go::handleDispatchFrame`** — looks up the live child and forwards by protocol: `executor` opens server-streaming `Execute`, sends the request, and streams each `ExecuteEvent` back as a DATA `DispatchFrame` terminating on the inner `StreamClose`; `claim_producer` forwards a unary verb (inferred from the request shape — Open carries claim_id+producer_name, else Commit-shaped — since the wire `DispatchFrame` carries only `protocol`, matching the proxy's contract) and returns one DATA frame. Failures surface as a terminal CANCEL frame the proxy translates into the supervisor-facing error_class. (6) **New `code:cmd/rimsky-host-agent/main.go`** — signal-handled standalone wrapper calling `hostagent.Run`. (7) **New `code:control/cli/agent.go::RunAgent`** wired into `code:cmd/rimsky/main.go` as `rimsky agent {start|status|stop}` (returns `int` exit code per the prevailing dispatch convention): `start` (`--allow-paths`, `--listen`, `--foreground`) runs `hostagent.Run` in the foreground or daemonizes by forking self with `--foreground` and writing `~/.rimsky/agent.pid`; `status` reads the pid file + signal-0 liveness probe; `stop` SIGTERMs the recorded pid, waits, and removes the pid file. **Verification:** `cmd:go build ./cmd/rimsky-host-agent/... && go test ./cmd/rimsky-host-agent/... ./runtime/hostagent/... ./control/cli/... -count=1` clean (unit/component tests stub the proxy stream via an in-process `HostAgent` server and exec the real `runtime/hostagent/testdata/stubchild` fixture honoring `env:RIMSKY_AGENT_PORT`; full end-to-end proxy+agent+stub wiring is Pass 10); `cmd:go test ./runtime/hostagent/... -race` clean; `cmd:go build ./...` whole-tree clean; `cmd:make lint` clean.

- **Host-agent/proxy `cmd/rimsky-host-agent-proxy` binary landed (Pass 8 of `plan:2026-05-24-host-agent-and-proxy`, per `spec:2026-05-24-host-agent-and-proxy-design`).** The new proxy binary that consumes all of Passes 1–7's wiring — `concept:host-agent-proxy`. It implements every supervisor-facing rimsky protocol on one gRPC port plus the agent-facing `proto:host_agent.proto::HostAgent.Connect` bidi stream. (1) **`file:cmd/rimsky-host-agent-proxy/main.go` + `config.go`** — single gRPC server registering `HostAgent` (agent-facing) + `Executor`/`ExecutorObservability`/`ClaimProducer`/`ClaimProducerObservability`/`LifecycleSubscriber` (supervisor-facing) + UNIMPLEMENTED `Publisher`/`Validation`/`DataProcessing`; signal-handled `GracefulStop`. Config from `env:RIMSKY_PROXY_GRPC_PORT` (default 9090), `env:RIMSKY_CONTROL_API_URL`, `env:RIMSKY_CONTROL_API_TOKEN`, `env:RIMSKY_LOG_LEVEL`. (2) **`file:cmd/rimsky-host-agent-proxy/state.go`** — RWMutex-guarded `proxyState`: `agents` (api-key → connection, latest-wins displacement), `spawns` + `runScopeBindings` dedup index, `instances` binding cache, and a `claimRoutes` map (claim-id → spawn) so `Commit`/`Abandon`/`Release` — which carry only a `claim_id`, not an `instance_id` — route back to the producer that served `Open`. Each `agentConnection` carries a `sendCh` (writer-drained), close-once semantics, and per-correlation pending maps for spawn-acks / reaped-acks / dispatch-stream frames. (3) **`file:cmd/rimsky-host-agent-proxy/agent_server.go`** — `Connect` requires a first `Register` frame (else `InvalidArgument`), displaces any prior connection for the same api-key (`RegisterAck{displaced_prior:true}` + old `sendCh` closed), runs a reader goroutine routing inbound frames by oneof and a writer goroutine draining `sendCh`; on disconnect drops the agent and closes all in-flight dispatch channels so readers synthesize `host_agent_disconnected`. (4) **`file:cmd/rimsky-host-agent-proxy/executor_handler.go` + shared `dispatch.go`** — `Execute` extracts `x-rimsky-service-name` (missing → `binding_not_found`), resolves the instance (cache → `GET /instances/{id}` fallback), owner (empty → `host_agent_not_connected`), agent (missing → `host_agent_not_connected`), binding (missing → `binding_not_found`), lazily spawns (timeout/ack-failure → `spawn_failed`), rewrites `callback_url` host:port onto the agent's `local_callback_base_url` (path preserved), tunnels the serialized `ExecuteRequest` in a `DispatchFrame`, streams inner `ExecuteEvent`s back, and synthesizes `StreamClose{Error, host_agent_disconnected}` on mid-stream disconnect. `ExecutorObservability.Capabilities` → empty envelope. (5) **`file:cmd/rimsky-host-agent-proxy/claim_producer_handler.go`** — `Open`/`Commit`/`Abandon`/`Release` share the resolve→spawn→forward-unary flow; errors return a gRPC status carrying `google.rpc.ErrorInfo{Reason:<error_class>}` (`FailedPrecondition` for `binding_not_found`, `Internal` otherwise) — the shape `code:runtime/peer/errors.go::extractErrorClass` decodes. `ClaimProducer.Capabilities` advertises the full four-value `write_semantics_allowed` envelope (the proxy is transport; per-claim realized semantics come from each producer's `Open`). **Divergence:** the plan's prose put the write-semantics envelope on `ClaimProducerObservability.Capabilities`, but the gen `ClaimProducerObservabilityCapabilities` has no write-semantics field — it lives on the main `ClaimProducer.Capabilities` `CapabilitiesResponse`, where it is now served. (6) **`file:cmd/rimsky-host-agent-proxy/lifecycle_handler.go` + `http_forward.go` + `unimplemented_handlers.go`** — `OnInstanceCreated` parses `service_bindings`/`params` JSON and caches; `OnRunScopeTerminal` drops the scope's spawns and fans `Reap{sigterm_grace_seconds:30}` out to the owning agents (awaiting `Reaped` bounded by `ReapTimeout`); the other 5 lifecycle methods no-op-ack. `LocalHttpForward` looks up the spawn's recorded `originalCallback`, POSTs the body upstream, and replies `LocalHttpResponse`. `Publisher`/`Validation`/`DataProcessing` embed their generated `Unimplemented*Server` (every method → `codes.Unimplemented`). **Spawn-scope keying note:** the wire `ExecuteRequest`/`OpenRequest` carry `instance_id` but not `run_scope_id`, so the dispatch-observable spawn-dedup scope is the instance id (one-per-main-run-scope, the common case); `OnRunScopeTerminal` reaps by the run-scope-id control-api fires. **Verification:** `cmd:go build ./cmd/rimsky-host-agent-proxy/... && go test ./cmd/rimsky-host-agent-proxy/... -count=1` clean (unit tests stub the agent connection + a scripted in-process spawned binary; full end-to-end scenario wiring is Pass 10); `cmd:go test ./cmd/rimsky-host-agent-proxy/... -race` clean; `cmd:go build ./...` whole-tree clean; `cmd:golangci-lint run ./cmd/rimsky-host-agent-proxy/...` clean.

- **Host-agent/proxy supervisor outbound lifecycle + admit-list + Registry production wiring landed (Pass 7 of `plan:2026-05-24-host-agent-and-proxy`, per `spec:2026-05-24-host-agent-and-proxy-design`).** The supervisor side of late-bind resolution + run-scope-terminal fan-out is now live end-to-end. Like the Pass-2..6 fields, the new wiring stays inert until rimsky.yml declares `late_bind_service_proxies` and a template declares `late_bind_services`. (1) **`code:control/config/supervisor.go::SupervisorConfig` gained four fields** — `LifecyclePeersForSpec func(node.TemplateSpec) []string` (late-bind-aware peer filter, supplied as a function pointer so `runtime/` never imports `control/` — the `runtime-purity` depguard rule), `LifecycleSubs *locks.LifecycleRegistry` (outbound subscriber registry), `Executors ExecutorsConfig` (needed by `DialLifecycleSubscribers`), and `LateBindServiceProxies map[string]string`. `StartSupervisor` now resolves `persistStore` before `dialRemoteStores`, dials its own outbound lifecycle subscribers via the newly-exported `code:control/config/stores.go::DialLifecycleSubscribers` (renamed from unexported `dialLifecycleSubscribers`; control-api's in-package caller updated), threads `LifecycleSubs`/`LifecyclePeersForSpec`/`LateBindServiceProxies` onto `runtime.Config`, and closes the lifecycle registry at handle shutdown. (2) **`code:cmd/rimsky-supervisor/main.go` builds the `peersForSpec` closure** — `func(tplSpec node.TemplateSpec) []string { return controlapi.LifecyclePeersForSpec(controlapi.AppDeps{LateBindServiceProxies: ...}, tplSpec) }` — and sets `Executors`, `LifecyclePeersForSpec`, `LateBindServiceProxies` from the parsed rimsky.yml. The closure lives in `cmd/` (control layer) so the runtime never takes a control-layer import. (3) **New `code:runtime/lifecycle_fanout.go::FanOutRunScopeEvent`** — the supervisor-side analogue of control-api's helper, with explicit params (`persist`, `lifecycleSubs`, `peersForSpec`, `tplSpec`, `runScopeID`, `terminalReason`, `tx`) instead of `AppDeps` (runtime cannot import control). No-op when `lifecycleSubs == nil || peersForSpec == nil`; same idempotency Get/skip/`OnRunScopeTerminal`/Upsert shape against `run_scope`/`run_scope_terminal`, per-peer continue-on-subscriber-error. (4) **`LifecycleSubs` + `LifecyclePeersForSpec` threaded through `runtime.Config` → `RunArgs` + `CallbackServer` → `PropagationArgs`** (the last also gained `Persist persistence.Tables`); populated at the RunNode dispatch site, the CallbackServer construction site (async-callback close path), `driveTerminal`'s RunArgs, and both `PropagationArgs{}` literals. (5) **Fan-out fired at the two supervisor close sites** — `code:runtime/subgraph_dispatch.go::CarryExitWriteback` (reason `"subgraph_exit"`) and `code:runtime/auto_terminal_chain.go::resolveParentClaimChain` (reason `"fanout_partition_terminal"`), each resolving `tplSpec` via the two-step `Instances().Get` → `Templates().GetByHash` lookup and firing inside the same tx as the `RunScopes().Close` (explicit tx, never nil — the Pass-6 correction). A resolution miss is silently skipped so the close never rolls back on a fan-out lookup failure. (6) **`code:runtime/runner_acquire.go::selectCandidatesShortTx` populates the admit-list extension** — `LateBindExecutorProxy: args.LateBindServiceProxies["executor"]` + `LateBindClaimProducerProxy: args.LateBindServiceProxies["claim_producer"]` on the `SelectCandidatesRequest`, plumbed from `SupervisorConfig` → `runtime.Config` → `RunArgs`. (7) **`code:control/config/stores.go::dialRemoteStores` signature extended** to `(ctx, cfg, persist persistence.Tables, lateBindServiceProxies map[string]string)` — builds a `lookupBindings` closure (parse instanceID via `uuid.Parse`; `persist.Instances().Get(ctx, instID, nil)`; unmarshal `row.ServiceBindings`) and constructs `locks.NewRegistry(WithLookupInstanceBindings(...), WithLateBindServiceProxies(...))`. All three callers updated: `StartSupervisor` + `StartControlAPI` pass their late-bind map; `StartScheduler` passes `nil` (scheduler does not dispatch). (8) **`code:control/controlapi/templates.go::validatorHooksFor` now takes the template spec** and short-circuits `StoreDeclared` / `ExecutorDeclared` / `ExecutorExpectedAttributesSchema` to "declared, no schema cross-check" for names in `spec.LateBindServices` — those services aren't in rimsky.yml or the discovery cache at registration; the spawned binary's Capabilities supplies the schema at dispatch. **Verification:** `cmd:go build ./... && go test ./runtime/... ./foundation/persistence/postgres/... ./control/controlapi/... -count=1` clean (testcontainers Postgres); `cmd:make lint` clean.

- **Host-agent/proxy main-RunScope close + control-api `OnRunScopeTerminal` fan-out landed (Pass 6 of `plan:2026-05-24-host-agent-and-proxy`, per `spec:2026-05-24-host-agent-and-proxy-design` §"Reap" + §"Firing sites for OnRunScopeTerminal").** Control-api now closes an instance's main run-scope and fans `OnRunScopeTerminal` out to the late-bind-extended peer set at both instance-close sites, so a host-agent-proxy can reap processes scoped to that main run-scope before `OnInstanceTerminated` fires. (1) **New `code:control/controlapi/lifecycle.go::FanOutRunScopeEvent`** — the run-scope-scope analogue of `FanOutInstanceEvent`. Computes peers via `LifecyclePeersForSpec(deps, tplSpec)` (the late-bind extension, so the proxy receives run-scope fan-out for late-bound catalogs); per peer it idempotency-checks `LifecycleIdempotency().Get(... LifecycleIdempotencyScopeRunScope, runScopeID ...)`, skips peers already at `LifecycleIdempotencyStateRunScopeTerminal`, dispatches `sub.OnRunScopeTerminal(ctx, locks.OnRunScopeTerminalRequest{RunScopeID, TerminalReason})`, and `Upsert`s the `run_scope`/`run_scope_terminal` bookkeeping row. Unlike `FanOutInstanceEvent` (where a subscriber fault aborts the iteration), a per-peer subscriber error is collected in the returned map and the loop continues to the next peer — main-scope reap is best-effort and must not wedge instance termination. Every `Get`/`Upsert` is wrapped in `withOptionalTx` for the same nil-tx→fresh-tx / inside-tx-passthrough semantics as the sibling helpers. (2) **Close + fan-out inserted at both control-api close sites, immediately before the existing `EventInstanceTerminated` fan-out.** `code:control/controlapi/instance_terminator.go::tick` (the polling worker — sub-tick lag vs the row's terminal mark is acceptable for v1) reuses the already-loaded `inst persistence.InstanceRow` + `tpl *persistence.TemplateRow` (no second `Templates().GetByHash`), guards `inst.MainRunScopeID != (shared.UUID{})`, closes via `RunScopes().Close`, then fires `FanOutRunScopeEvent(..., "instance_terminated", nil)`. `code:control/controlapi/instances.go::handleDeleteInstance` (the synchronous explicit-DELETE path) does the same in the request context with terminal reason `"instance_deleted"`. The two reason strings are spec-internal and never persisted. **Divergence from the plan snippet:** the plan showed `RunScopes().Close(ctx, nil, id)`, but the postgres `RunScopes` Table methods panic on a nil tx (every Table method requires an explicit tx) — both close calls are wrapped in a short `Persist.Transaction` instead, matching every other `RunScopes().Close` caller in the tree. (3) **`code:control/controlapi/instance_terminator_test.go::TestInstanceTerminator_RowFoundRPCSucceedsRowDeleted` updated** — the happy path now records two lifecycle calls (`on_run_scope_terminal` then `on_instance_terminated`); the assertion verifies both fired and that the run-scope-terminal call precedes the instance-terminated call by `FakeCall.Sequence`. **Verification:** `cmd:go build ./... && go test ./control/controlapi/... -count=1` clean (testcontainers Postgres); `cmd:make lint` clean.

- **Host-agent/proxy control-api wiring landed (Pass 5 of `plan:2026-05-24-host-agent-and-proxy`, per `spec:2026-05-24-host-agent-and-proxy-design`).** The control-api now carries the late-bind config and forwards the per-instance late-bound service catalog + creating api-key into the instance-create lifecycle fan-out; the new fields stay inert until the proxy + host-agent passes wire them. (1) **New `LateBindServices []string` field on `code:foundation/spec/template.go::TemplateSpec`** (`json`/`yaml` tag `late_bind_services,omitempty`) — declares service names whose registration-time existence + schema checks defer to dispatch. It rides inside the canonical-spec bytes (`json.Marshal` in `code:graph/template/canonical/jcs.go::CanonicalSpecBytes`) so it participates in the template hash; `omitempty` keeps existing template hashes byte-identical (no fixture churn). (2) **New `LateBindServiceProxies map[string]string` on `code:control/config/stores.go::RimskyConfig`** (yaml `late_bind_service_proxies`, protocol → proxy service name), parsed via the loader's wrapper struct and threaded into the returned `RimskyConfig`. (3) **Threaded `LateBindServiceProxies` through to the controlapi deps** — added to `code:control/controlapi/app.go::AppDeps`, to `code:control/config/controlapi.go::ControlAPIConfig`, plumbed where `AppDeps` is constructed in `StartControlAPI`, and sourced from `rimskyCfg.LateBindServiceProxies` at the `code:cmd/rimsky-control-api/main.go` call site. (4) **`lifecyclePeersForSpec` renamed/exported as `code:control/controlapi/lifecycle.go::LifecyclePeersForSpec`** (the supervisor's outbound dispatch calls it cross-package in a later pass) and extended: when `spec.LateBindServices` is non-empty it appends the late-bind proxy names from `deps.LateBindServiceProxies` (deduped against referenced peers). `FanOutInstanceEvent` uses the extended version; `FanOutTemplateEvent` switched to call `peersReferencedBySpec` directly so the late-bind extension stays scoped to instance + run-scope fan-out (the proxy doesn't subscribe to template events). (5) **`service_bindings` + `created_by_api_key_id` plumbed through instance creation** — `ServiceBindings json.RawMessage` (json `service_bindings,omitempty`) added to `code:control/controlapi/instances.go::createInstanceRequest`; `ServiceBindings` + `CreatedByAPIKeyID *shared.UUID` added to `provisionArgs` and threaded into the `persistence.InstanceCreateInput` built by `provisionInstanceTx`. `handleCreateInstance` destructures `ident, _ := IdentityFromContextOK(req.Context())` at handler entry and populates `provisionArgs.ServiceBindings` from the decoded body + `CreatedByAPIKeyID` from `ident.KeyID` (nil under anonymous-mode). (6) **`InstancePayload` + `dispatchInstanceEvent` forward the new fields** — `ServiceBindings json.RawMessage` + `OwnerAPIKeyID *shared.UUID` added to `code:control/controlapi/lifecycle.go::InstancePayload`; the `EventInstanceCreated` branch of `dispatchInstanceEvent` sets both on the Go `locks.OnInstanceCreatedRequest` alias (`OwnerAPIKeyID` rendered via a new `uuidString(*shared.UUID) string` helper that returns `""` for nil — the proto carries `owner_api_key_id` as a string). The instance-create FanOut site populates the payload from `body.ServiceBindings` + `ident.KeyID`. **Verification:** `cmd:cd foundation && go test ./... -count=1 && cd .. && go test ./control/controlapi/... ./graph/... -count=1` clean (testcontainers Postgres + SQLite; graph hash fixtures unchanged).

- **Host-agent/proxy per-call header interceptor + claim-producer error_class translation landed (Pass 4 of `plan:2026-05-24-host-agent-and-proxy`, per `spec:2026-05-24-host-agent-and-proxy-design`).** Wires the supervisor's outbound executor + claim-producer gRPC dials to carry a routing header, and routes claim-producer Open faults through the existing error-policy chain — both inert for direct-dial (hosted) services, load-bearing only when a host-agent-proxy fronts the protocol. v1 fronts only the executor + claim-producer protocols. (1) **New `code:runtime/peer/service_name_interceptor.go`** — `WithServiceName(ctx, name)` (no-op when name empty) plus `ServiceNameUnaryInterceptor` / `ServiceNameStreamInterceptor`, which stamp `x-rimsky-service-name` from the per-call context onto outbound RPCs via `metadata.AppendToOutgoingContext`. Homed in `pkg:runtime/peer` (the gRPC-dial-owning package) rather than the plan's suggested `pkg:runtime/clientiface`, because `clientiface` is the pure-DTO/interface surface that deliberately imports no gRPC; `pkg:runtime/executor` takes a fresh import edge on `pkg:runtime/peer` to install the same interceptor. (2) **Both interceptors installed** on the executor dial (`code:runtime/executor/client.go::NewGRPCClient`) and the claim-producer dial (`code:runtime/peer/dial.go::Dial`). The four other peer-service dials (`DialLifecycle`, `code:runtime/peer/publisher_client.go`, `code:runtime/peer/validation_client.go`, `code:runtime/peer/data_processing_client.go`) carry a `TODO(host-agent-proxy v2)` breadcrumb — those protocols gain late-bind support in a later cycle. (3) **Service name stamped at dispatch sites.** Executor: `code:runtime/runner_dispatch.go::dispatch` stamps `acq.Executor` before `client.Execute` (and now passes the real `executor.DispatchContext{InstanceID, RunScopeID}` to `Resolver.Resolve`, replacing the Pass-3 zero-value breadcrumb so a `LateBindResolver` can resolve live). Claim-producer: `spec.ProducerName` at the Open site (`code:runtime/runner_acquire_claims.go::acquireClaim`), `producer.Name()` in `code:runtime/abandon_claim.go::abandonOpenedClaim` (the shared Abandon helper) + `code:runtime/terminal_decision.go::fireProducerVerb` (Commit/Abandon), and `producerName` at the Release site (`code:runtime/instance_termination.go::ReleaseHeldDurableClaims`). (4) **New `code:runtime/peer/errors.go::ProducerCallError`** (`ProducerName`, `Method`, `ErrorClass`, `Underlying`; `Error()`/`Unwrap()`) plus `extractErrorClass(err)` which walks the gRPC `status.FromError(err)` details for a `*errdetails.ErrorInfo` and returns its `Reason` as the rimsky error_class (`""` when absent). The `Open`/`Commit`/`Abandon`/`Release` RPC-failure returns on `code:runtime/peer/client.go::Client` now wrap the underlying error in `*ProducerCallError` (local validation errors stay bare). (5) **Claim-producer Open faults now reach the error-policy chain.** Previously a producer Open RPC fault bubbled as `openResultBail` + a wrapped error, which aborted the whole scheduler tick and never consulted the template's `error_types:` chain. New `openResultErrored` enum value + `errAcquireProducerErrored` sentinel + `ErroredSpec`/`ProducerErrorClass` carry-fields on `code:runtime/runner_acquire.go::acquisition`: `acquireClaim` returns `openResultErrored` carrying the `*peer.ProducerCallError` on Open fault; `code:runtime/runner_acquire.go::tryAcquire` builds a partial acquisition (mirroring the existing `openResultUnavailable` path) and returns the sentinel; `tryAcquireWithTx` + `acquireCandidate` catch it and dispatch to the new `code:runtime/runner_lifecycle.go::handleAcquireProducerError`, which Abandons partial locks and routes the translated `error_class` (falling back to synthetic `acquire/producer_error` when the producer attached no ErrorInfo) through `OnError` → the operator's `error_types:` chain — the same chain executor `Error{error_class}` terminals use, no new policy mechanism. The post-dispatch Commit/Abandon/Release verb failures already propagate into policy-aware terminal handling and now carry the translated `error_class` on `*ProducerCallError` for any `errors.As` consumer. **Verification:** `cmd:go build ./... && go test ./runtime/... -count=1` clean; `cmd:golangci-lint run` clean (root module + `./runtime/...`).

- **Host-agent/proxy dispatch-resolution foundations landed (Pass 3 of `plan:2026-05-24-host-agent-and-proxy`, per `spec:2026-05-24-host-agent-and-proxy-design`).** The resolution plumbing for late-bound executors and claim-producers; the actual late-bind config values are wired in later passes, so the new OR-branches stay inert (fall through to today's behavior) until then. (1) **`code:runtime/executor/resolver.go` — new `DispatchContext` type** (`InstanceID`, `RunScopeID`) carrying instance/run-scope identity into resolver lookups; named `DispatchContext` rather than `ResolveContext` to avoid the clash with `code:graph/attribute/substitution.go::ResolveContext`. `Resolver.Resolve(name)` → `Resolve(name, DispatchContext)`; `StaticResolver.Resolve` accepts and ignores the new arg. (2) **New `code:runtime/executor/resolver.go::LateBindResolver`** chaining after a static resolver: for names absent from the static map it consults a per-instance `service_bindings` lookup hook + a static `late_bind_service_proxies` map and returns the proxy's endpoint on a hit. Passthrough when the lookup hook is nil or the proxy map is empty. (3) **`code:foundation/locks/registry.go` — functional-options `NewRegistry(opts ...Option)`** with `WithLookupInstanceBindings` + `WithLateBindServiceProxies`, plus `GetWithContext(name, instanceID)` — the late-bind-aware sibling of `Get` that falls through to `Get(name)` when no late-bind config / no instance context. Foundation can't import runtime, so `GetWithContext` takes a plain `instanceID string` (annotated `@diverged: true` recording the parallelism with `Resolver.Resolve`). Existing zero-arg `NewRegistry()` callers compile unchanged (variadic). (4) **`InstanceID shared.UUID` threaded through the acquisition path** — added to `code:runtime/runner_acquire_named_locks.go::acquireOneLock`, `code:runtime/runner_acquire_claims.go::acquireClaim`, `code:runtime/runner_acquire_helpers.go::acquireFanOutIfDeclared` (all after `tx`), and as a field on `code:runtime/runner_subclaim.go::AcquireSubClaimsInput`; `code:runtime/runner_acquire.go::tryAcquire` sources it from `nd.InstanceID`. The four dispatch-time / instance-scoped claim-producer lookups (`acquireClaim`, `AcquireSubClaims`, and the instance-release path `code:runtime/instance_termination.go::ReleaseHeldDurableClaims`) switch from `StoreRegistry.Get` to `GetWithContext`. The four terminal-resolution `Get` sites (`code:runtime/auto_terminal.go::CheckAndFireResolution`, `code:runtime/auto_terminal_chain.go::resolveParentClaimChain`, both `code:runtime/terminal_decision_cancel.go` force-Abandon walks) stay bare `Get` — those resolve already-bound producers with no instance context in scope; one-line comments record why. Test fixtures pass `shared.UUID{}` (zero UUID → identical to `Get`). **Verification:** `cmd:go build ./... && go test ./runtime/... ./foundation/locks/... -count=1` clean.

- **Host-agent/proxy schema + persistence types landed (Pass 2 of `plan:2026-05-24-host-agent-and-proxy`, per `spec:2026-05-24-host-agent-and-proxy-design`).** (1) **New migrations `file:foundation/persistence/postgres/migrations/002-host-agent-proxy.sql` + `file:foundation/persistence/sqlite/migrations/002-host-agent-proxy.sql`** — add `col:rimsky_instances.service_bindings` (JSONB / TEXT) carrying the per-instance late-bound service catalog and `col:rimsky_instances.created_by_api_key_id` (UUID / TEXT FK → `table:rimsky_api_keys`) recording the creating api-key; both nullable, no backfill default. Postgres DROP+ADDs the auto-named `rimsky_lifecycle_idempotencies_{scope_kind,state}_check` constraints to extend the vocabularies; SQLite (which can't DROP CONSTRAINT in place) rebuilds `table:rimsky_lifecycle_idempotencies` via the create-`_new`/copy/drop/rename idiom with the extended CHECKs and recreates `idx_rimsky_lifecycle_idempotencies_scope`. The new scope vocabulary is `('template','instance','run_scope')` and the new state vocabulary appends `'run_scope_terminal'` to the existing `('registered','deployed','undeployed','created')`. (2) **`code:foundation/persistence/lifecycle_idempotency.go` enums extended** — new `LifecycleIdempotencyScopeRunScope = "run_scope"` and `LifecycleIdempotencyStateRunScopeTerminal = "run_scope_terminal"` for the host-agent `OnRunScopeTerminal` fan-out idempotency. (3) **`code:foundation/persistence/instances.go::InstanceCreateInput` + `InstanceRow` extended** with `ServiceBindings json.RawMessage` (json tag `service_bindings,omitempty`) and `CreatedByAPIKeyID *shared.UUID` (json tag `created_by_api_key_id,omitempty`). (4) **`code:foundation/persistence/postgres/instances.go` + `code:foundation/persistence/sqlite/instances.go` INSERT/SELECT updated** — `instanceCols` projection, the Create INSERT (columns + placeholders + args), and both `scanInstance` scanners now carry the two new columns; empty `ServiceBindings` and nil `CreatedByAPIKeyID` persist as SQL NULL. (5) **`code:foundation/persistence/node_runs.go::SelectCandidatesRequest` gained `LateBindExecutorProxy` + `LateBindClaimProducerProxy`** and `code:foundation/persistence/postgres/queue.go::SelectCandidates` extends the WHERE clause to admit late-bound rows (proxy peer name present in the accept-list AND the instance's `service_bindings` JSONB carries the named binding — JSONB key-existence for the executor, `unnest()`+`bool_and` for required_stores). The `<> ''` guards keep the new OR-branches inert when no proxy is configured, so existing call sites (which leave the fields empty) behave identically. **Verification:** `cmd:cd foundation && go build ./... && go test ./persistence/... -count=1` clean (conformance suite spins real Postgres + SQLite via testcontainers and applies migration 002); `cmd:make lint` clean.

- **Host-agent/proxy protocol surfaces landed (Pass 1 of `plan:2026-05-24-host-agent-and-proxy`, per `spec:2026-05-24-host-agent-and-proxy-design`).** (1) **New `proto:host_agent.proto`** — the internal bidi-stream protocol (`proto:host_agent.proto::HostAgent.Connect`) the proxy uses to talk to connected host-agent daemons: `ClientFrame`/`ServerFrame` oneofs carrying register/heartbeat/spawn/reap/dispatch/local-http frames. Two messages were renamed off the plan's verbatim text to avoid flat-`rimsky.v1`-namespace collisions with `executor.proto`'s existing `Heartbeat`/`Error`: the agent-liveness frames are `HostAgentHeartbeat`/`HostAgentHeartbeatAck` and the stream error is `HostAgentError`. (2) **`proto:lifecycle.proto` extended** — new 7th RPC `proto:lifecycle.proto::LifecycleSubscriber.OnRunScopeTerminal` + `OnRunScopeTerminalRequest` message (`run_scope_id`, `terminal_reason`); two new fields on `OnInstanceCreatedRequest` (`bytes service_bindings = 5;`, `string owner_api_key_id = 6;`) consumed downstream by the host-agent-proxy. (3) **`host_agent.proto` added to the `cmd:make proto-gen` proto-file list** and Go bindings regenerated. (4) **Parallel Go-interface surface extended** — `ServiceBindings json.RawMessage` + `OwnerAPIKeyID string` on `code:protocols/lifecycle/types.go::OnInstanceCreatedRequest`, new `code:protocols/lifecycle/types.go::OnRunScopeTerminalRequest`, new 7th method on `code:protocols/lifecycle/lifecycle.go::LifecycleSubscriber`; alias re-export `OnRunScopeTerminalRequest` added to `code:foundation/locks/lifecycle.go`. (5) **Adapters/fakes updated** — `code:runtime/peer/lifecycle_client.go::LifecycleClient` now forwards the two new `OnInstanceCreated` fields and implements `OnRunScopeTerminal`; the `code:foundation/locks/storetest/fake.go::Fake` records the new `on_run_scope_terminal` verb and captures `ServiceBindings`/`OwnerAPIKeyID` on `on_instance_created` (new `FakeCall` fields). The two gRPC-server test fakes embed `UnimplementedLifecycleSubscriberServer`, so they pick up the new RPC for free. **Verification:** `cmd:make proto-gen && cd protocols && go build ./... && cd .. && cd foundation && go build ./... && cd .. && go build ./...` clean.

- **Removed dead `TestLoadDeployRimskyYAML_Phase4Shape` in `code:control/config/stores_test.go`.** The test loaded the fixture `file:deploy/rimsky.yml`, which was deleted when `deploy/` moved to `pkg:github.com/rimsky-ai/rimsky-docs` (commit `696d52a`). The test had been failing at HEAD ever since (file-not-found) and can never pass in this repo again, so it is removed rather than carried forward. The `filepath` import is retained — still used by `mustLoadCfg`.

- **rimsky-services test-harness now consumes locally-built rimsky-core images.** Resolves divergences #1 and #10 from `plans/2026-05-24-repo-reorganization-divergences.md`. The 12 tests punted to `t.Skip` + minimal-DDL stubs during the Pass-5 carve-out now run end-to-end. The sibling `pkg:github.com/rimsky-ai/rimsky-services` repo introduces a `test/harness/` package that stands up `rimsky/all:latest` + Postgres on a shared docker network via testcontainers-go and exposes the public HTTP API to consumption-side tests. Operators / developers running the rimsky-services test suite locally now need: (1) `rimsky/all:latest` + `rimsky/executor-stub:latest` built via this repo's `cmd:./deploy/build-images.sh`; (2) `ghcr.io/rimsky-ai/rimsky-services/store-filesystem:latest` + `store-postgres:latest` built via `cmd:cd ../rimsky-services && ./deploy/build-images.sh`. The two `file:executors/stub/Dockerfile.stub` and `file:stores/stub/Dockerfile.stub` builders gained the missing `COPY sdk/go/go.mod sdk/go/go.sum ./sdk/go/` step (their builds had been silently broken since the SDK module was added — the workspace `pkg:sdk/go` declaration in `file:go.work` requires the manifest to be present at `go mod download` time, but the two stub Dockerfiles' COPY lists never picked up the change). No rimsky-core source touched beyond the two Dockerfile fixes. **Verification:** `cmd:./deploy/build-images.sh` succeeds and builds 7 images; `cmd:make lint && make build-all && make test-all` clean.

- **Reorganization follow-up fixes for `spec:2026-05-24-repo-reorganization-design`.** Caught by post-merge code review across the five sibling repos. (1) `file:deploy/Dockerfile.all` and `file:deploy/Dockerfile.go-base` now `COPY sdk/go/go.mod sdk/go/go.sum` before `go mod download` — `file:go.work` lists `./sdk/go` as a workspace module, so omitting its manifest broke any `docker build` of rimsky's images. (2) Stale `cmd/rimsky-verifier-{http,shape-checks}` rows removed from `file:feature-index.md` (the binaries moved to `pkg:github.com/rimsky-ai/rimsky-services/executors/verifier-*` in Pass 5 and the in-tree cmd dirs were deleted in P1.4). (3) `file:licensing.yml` Apache block scoped from `executors/` → `executors/stub/` and dropped the post-move `sensors/` and `subscribers/` entries; the four dead `executors/claude-agent/{node_modules,dist,build,.cache}/` exempt entries dropped with a breadcrumb comment. (4) Annotated the foundation/internal/pgtest ↔ sdk/go/testpg duplication of `StartFreshPostgresDSN` + `resolveConnectionString` with explicit `@source` + `@diverged: false` annotations — both copies exist because `foundation/` cannot import `sdk/go` (depguard `foundation-purity`); fixes must land in both. Same pattern applied to the internal/pgmigrate ↔ foundation/internal/pgtest helper-cluster duplication (depguard visibility scoping forces the split). (5) Dead `httpSrv *http.Server` field and its nil-check stripped from `code:test/scenarios/canary/lifecycle_subscriber_callback_test.go::fakeLifecycleServer` — never assigned anywhere; the test runs only the gRPC LifecycleSubscriber surface. **Verification:** `cmd:make lint && cmd:make build-all && cmd:make test-all` clean.

- **Repo-reorganization P6 of `spec:2026-05-24-repo-reorganization-design`: `file:dashboards/rimsky-dashboard/` migrated to `pkg:github.com/rimsky-ai/rimsky-dashboard`.** Pass 9 of `plan:2026-05-24-repo-reorganization` (final pass). The `file:dashboards/rimsky-dashboard/` tree (TypeScript / Vite operator dashboard: `src/client/`, `src/server/`, `tests/`, `index.html`, `package.json`, `package-lock.json`, `tailwind.config.js`, `postcss.config.js`, `tsconfig.json`, `tsconfig.node.json`, `vite.config.ts`, `vitest.config.ts`, `Dockerfile`, `.dockerignore`) moved out of rimsky to the sibling repo at `pkg:github.com/rimsky-ai/rimsky-dashboard` via `rsync --exclude=node_modules --exclude=dist`. The now-empty `file:dashboards/` directory was removed from rimsky. The reference deployment compose files (`file:deploy/docker-compose.yml`, `file:quickstart/docker-compose.yml`, `file:control/cli/embedded/deploy/docker-compose.yml`) replaced `build: ../dashboards/rimsky-dashboard` with `image: ghcr.io/rimsky-ai/rimsky-dashboard:latest` so the published image is pulled rather than built from source. Stale `dashboards/` license-walker entries removed from `file:licensing.yml`; vestigial `dashboards/` depguard rules removed from `file:.golangci.yml` `foundation-purity`/`graph-purity`/`runtime-purity` blocks; `file:feature-index.md` Dashboards section now points to the sibling repo; `file:quickstart/README.md` "What's running" table updated to reference the published image. **Verification:** `cmd:make lint && cmd:make test-all` clean in rimsky; `cd ../rimsky-dashboard && cmd:npm install && cmd:npm test && cmd:npm run build` clean.

- **Repo-reorganization P5 of `spec:2026-05-24-repo-reorganization-design`: `file:apps/crimefinder/` migrated to `pkg:github.com/rimsky-ai/crimefinder`.** Pass 8 of `plan:2026-05-24-repo-reorganization`. The `file:apps/crimefinder/` tree (TS sources under `cli/`, `executor/`, `producer/`, `shared/`, `test/`, plus `templates/`, `deploy/`, `proto/`, `cold-read/`, `CHANGELOG.md`, `CLAUDE.md`, `feature-index.md`, `README.md`, `package.json`, `package-lock.json`, `tsconfig.base.json`) moved out of rimsky to the sibling repo at `pkg:github.com/rimsky-ai/crimefinder` via `rsync --exclude=node_modules`. The now-empty `file:apps/` directory was removed from rimsky. Crimefinder consumes rimsky exclusively through its public protocol surfaces (executor, claim_producer) and rimsky's published container images — its `file:deploy/docker-compose.fragment.yml` references `image: crimefinder/producer:latest` and `file:deploy/rimsky.yml.fragment` carries only service-registration entries with no in-tree path coupling. The drift signal that `apps/crimefinder/` provided pre-move (a real consumer exercising the executor + claim_producer wire surface end-to-end) is replaced by the in-tree canary scenarios at `file:test/scenarios/canary/` added in Pass 3 of this reorganization (`template_registration_run_a_pass_test.go`, `lifecycle_subscriber_callback_test.go`). **Verification:** `cmd:make lint && cmd:make test-all` clean in rimsky; `cd ../crimefinder && cmd:npm install && cmd:npm test && cmd:npm run build` clean.

- **Repo-reorganization P4 of `spec:2026-05-24-repo-reorganization-design`: docs + docs-tooling + atomic-staging-fs-producer example migrated to `pkg:github.com/rimsky-ai/rimsky-docs`.** Pass 7 of `plan:2026-05-24-repo-reorganization`. (1) **`file:docs/` moved** — all 49 markdown files (concepts, protocols, agents guide, blob-backend / store / executor / mcp-server / patterns references, operator guide) relocated to `../rimsky-docs/docs/`. (2) **`file:docs/.vocabulary-lint.yml` relocated** to `../rimsky-docs/.vocabulary-lint.yml` at the rimsky-docs repo root so the file lives next to the binary that reads it; the vocabulary subcommand's default path updated accordingly. (3) **`pkg:cmd/rimsky-docs-lint`, `pkg:cmd/rimsky-docs-llms-full`, `pkg:cmd/rimsky-docs-glossary` moved** to `../rimsky-docs/cmd/` under a new Go module (`pkg:github.com/rimsky-ai/rimsky-docs/cmd`, stdlib + `pkg:gopkg.in/yaml.v3`). Path defaults in every subcommand retargeted at the new layout (`../docs/...`, `../.vocabulary-lint.yml`, `../llms*.txt`, `${RIMSKY_REPO}/protocols/proto/v1`) so binaries are invoked from `cmd/` cwd. The `glossary-parity` subcommand's exec shell-out points at `./rimsky-docs-glossary` (not the rimsky-side `./cmd/rimsky-docs-glossary` path). `cmd:rimsky-docs-llms-full` gained a `-root-output` flag and now writes both `docs/agents/llms-full.txt` and the repo-root copy in a single invocation, subsuming the dropped `Makefile docs-roots` cp-step. (4) **`pkg:examples/atomic-staging-fs-producer` moved** to `../rimsky-docs/examples/atomic-staging-fs-producer/` under its own Go module (`pkg:github.com/rimsky-ai/rimsky-docs/examples`) with a local `replace` directive pointing at `../../rimsky/protocols` (matches the rimsky-services Pass-5 pattern). All four scenario tests (`abandon_on_any_failure_test.go`, `commit_on_all_success_test.go`, `concurrent_staging_test.go`, `sub_stage_verifier_failure_test.go`) moved alongside under `scenarios/` and pass without modification beyond import-path retargeting — they exercise the example's own `store` package, not `pkg:graph/scenario`, so no `t.Skip` was needed. The now-empty `file:examples/` and `file:test/scenarios/atomic_staging/` directories deleted from rimsky. (5) **Root-level `file:llms.txt` and `file:llms-full.txt` moved** to `../rimsky-docs/llms.txt` and `../rimsky-docs/llms-full.txt`. Both files were generated copies of `docs/agents/llms-full.txt` (formerly via `Makefile docs-roots`), so they follow the docs to their canonical home. The updated `cmd:rimsky-docs-llms-full` writes both rimsky-docs locations in one invocation. (6) **`file:Makefile` docs targets dropped** — `docs-glossary`, `docs-llms-full`, `docs-lint`, `docs-roots`, `docs-build` (and their `.PHONY` entries) removed; a header comment points operators at the new rimsky-docs invocation pattern. The four removed targets had no remaining callers in rimsky. (7) **`env:RIMSKY_REPO` convention** documented in `../rimsky-docs/README.md` and enforced at startup by all three lint binaries (exit 2 with a helpful message when unset). `public-anchor-validity`'s `-proto-dir` default is anchored at `${RIMSKY_REPO}/protocols/proto/v1` since the proto sources live in rimsky, not rimsky-docs. (8) **Pre-release reconciliation gate added at `file:scripts/release.sh`.** New stub release script with one first-class responsibility today: invoke `RIMSKY_REPO=$(pwd) go run ./rimsky-docs-lint all` from a sibling `../rimsky-docs/cmd` checkout. Lint drift blocks the release. Supports `--skip-docs-reconciliation` for emergency releases (documented in the script header + `--help`). `env:RIMSKY_DOCS_REPO` overrides the default sibling path. Tagging / pushing / release-notes generation are out of scope for the stub. Also: removed dead `pkg:sdk/go/conformance/claimproducer-tmp/` directory (leftover from Pass 5; the canonical impl at `pkg:sdk/go/conformance/claimproducer/` superseded it, but the `-tmp` copy was never deleted — and it carried a stale `pkg:foundation/locks` import that violated `sdk-purity` once `make lint` re-checked). Also: `file:licensing.yml` lost the now-orphan apache-list entries for `docs/`, `examples/`, `cmd/rimsky-docs-{glossary,llms-full,lint}/`, and the exempt entry for `cmd/rimsky-docs-lint/testdata/` (each directory moved to rimsky-docs); inline comments left as breadcrumbs pointing at the new home. `file:CLAUDE.md` and `file:README.md` updated to point readers at the rimsky-docs home for the public-docs surface (`docs/concepts/`, `docs/protocols/`, `docs/agents/llms.txt`, `docs/agents/examples/`, `docs/roadmap.md`). `file:feature-index.md` lost its `rimsky-docs-*/` row in the cmd table and its atomic-staging-fs-producer row in the reference-impls table; inline notes point at the new homes. **Verification:** `cd ../rimsky-docs/cmd && go build ./... && go test ./...` clean; `cd ../rimsky-docs/examples && go build ./... && go test ./...` clean (4 scenario tests pass, 0 skipped); `cmd:make lint && cmd:make test-all` clean in rimsky; `RIMSKY_REPO=<rimsky> go run ./rimsky-docs-lint all` from `../rimsky-docs/cmd/` exits non-zero only on legitimate pre-existing drift (root `llms.txt` was already slightly stale vs `docs/agents/llms.txt`; one vocabulary hit in `agents/examples/claude-agent-attribute-defaults.md`) — to be reconciled by the user in a follow-up rimsky-docs PR.

- **Repo-reorganization P3c of `spec:2026-05-24-repo-reorganization-design`: rimsky deploy/build wired to rimsky-services + P3 concept-doc mutations.** Pass 6 of `plan:2026-05-24-repo-reorganization`. (1) **`file:deploy/docker-compose.yml` retargeted** — every bundled-service `image:` entry (`store-filesystem`, `store-postgres`, `http-node`, `claude-agent`, `sensor-{cron,http,object-store,webhook}`) now points at `ghcr.io/rimsky-ai/rimsky-services/<service>:latest`; rimsky-core images (`rimsky/migrate`, `rimsky/scheduler`, `rimsky/supervisor`, `rimsky/control-api`, `rimsky/rimsky-dashboard`) and the in-tree test-double `quickstart/store-stub.yml` reference are unchanged. The helm chart at `file:deploy/kubernetes/rimsky-chart/` gains a `bundledServicesImage:` values block (defaults to `ghcr.io/rimsky-ai/rimsky-services:latest`); bundled-service deployment templates (`http-node`, `claude-agent`, `sensor-{cron,http,object-store,webhook}`) reference `.Values.bundledServicesImage.*` instead of `.Values.image.*`. (2) **`file:deploy/build-images.sh` restricted to rimsky-core images.** The script now builds only the four runtime binaries via `Dockerfile.go-base`, the unified `rimsky/all` image, and the two in-tree test doubles (`rimsky/executor-stub`, `rimsky/store-stub`). Build steps for `stores/{filesystem,postgres}`, `sensors/*`, and production-side executor Dockerfiles were removed — those images are built by `file:../rimsky-services/deploy/build-images.sh`. Header comment documents the split. Dead `file:deploy/Dockerfile.http-node` and `file:deploy/Dockerfile.claude-agent` (targeted moved-out source trees) deleted. (3) **P3 concept-doc mutations.** `file:.ok-planner/design/concepts/claim-producer.md` — "What it is" paragraph + "Boundaries" section + "Aliases and historical names" section + Notes entry updated to retarget `stores/{filesystem,postgres}` references to `pkg:github.com/rimsky-ai/rimsky-services/stores/`; `pkg:stores/stub` references preserved. `file:.ok-planner/design/concepts/publisher.md` — bundled-sensor path retargeted to `pkg:github.com/rimsky-ai/rimsky-services/sensors/sensor-*/`; Notes entry appended. `file:.ok-planner/design/concepts/sensor.md` — both `sensors/sensor-*/` references (definition + annotation sites) retargeted; Notes entry appended. `file:.ok-planner/design/concepts/executor.md` — bundled-impl enumeration retargeted (claude-agent, http-node, verifier-http, verifier-shape-checks to rimsky-services; `pkg:executors/stub` preserved), `stores/postgres/` cross-reference retargeted; Notes entry appended. `file:.ok-planner/design/concepts/replica.md` — three production-side `pkg:` path bullets (`sensors`, `executors`, `stores`) retargeted; stub-carve-out callouts appended; Notes entry appended. `file:.ok-planner/design/concepts/sdk.md` — Notes entry appended recording the Pass-5 promotion of `pkg:stores/common/action` → `pkg:sdk/go/stores/action`. (4) **`file:.ok-planner/design/concepts.md` TOC refreshed** for the four concepts whose one-line summary changed: `claim-producer`, `executor`, `sensor`, `replica` now mention the rimsky-services home; `module-layout`, `conformance`, `sdk` TOC entries already reflected post-P2 reality from Pass 3. Active/retired counts (70 / 7) line up with `.ok-planner/design/concepts/*.md` and `_retired/*.md` respectively. **Verification:** `cmd:make lint && cmd:make test-all` clean; `cmd:bash -n deploy/build-images.sh` clean; `cmd:docker compose -f deploy/docker-compose.yml config` clean (placeholder `latest` tag — operator pins at deploy time).

- **Repo-reorganization P3b of `spec:2026-05-24-repo-reorganization-design`: production-side bundled impls moved out to `pkg:github.com/rimsky-ai/rimsky-services`.** Pass 5 of `plan:2026-05-24-repo-reorganization`. (1) **Production-side trees moved:** `pkg:stores/filesystem/{cmd,server,store,lifecycle}` + `file:stores/filesystem/Dockerfile.filesystem` + `file:stores/filesystem/config-example.yml`; `pkg:stores/postgres/{cmd,server,store,lifecycle}` + `file:stores/postgres/Dockerfile.postgres` + `file:stores/postgres/config-example.yml`; `pkg:stores/shared/sql-checks` (only consumer was `pkg:stores/postgres/server`); `pkg:sensors/{sensor-cron,sensor-http,sensor-object-store,sensor-webhook}`; `pkg:subscribers/openlineage`; `pkg:executors/{claude-agent,http-node,verifier-http,verifier-shape-checks}` — all relocated under `pkg:github.com/rimsky-ai/rimsky-services/...`. **Test-infrastructure carve-outs (`pkg:stores/stub`, `pkg:executors/stub`, `pkg:stores/{filesystem,postgres}/testfixture`) stay in rimsky per spec P3.1.** (2) **`pkg:stores/common/action` promoted into the SDK** at `pkg:sdk/go/stores/action`. The action-vocabulary types (`Action`, `Kind`, `Pop` / `PopAndMove` / `PopAndDelete` / `Recycle`) are the canonical cross-implementer surface every claim-producer store implements against; with the filesystem + postgres stores moving to rimsky-services and `pkg:stores/stub` staying in rimsky, the only consistent home for the shared vocabulary is the SDK. All call-sites swept across both repos. The empty `pkg:stores/common/` directory was removed. (3) **MOVE-TO-RIMSKY-SERVICES tests relocated** per the Pass 4 audit (`file:.ok-planner/plans/2026-05-24-repo-reorganization-test-audit.md`): the three `test/scenarios/stores/fs_*_test.go`, the two `test/scenarios/atomic_staging/pg_verifier_*_test.go` files behind the `rimskyservices` build tag, the un-tagged `pg_verifier_test.go` (pure wire-shape; still runs) and `bundled_executor_vocab_test.go`, plus the five `test/smoke/*.go` files (`setup.go` + four `*_smoke_test.go`). Tests whose original implementation depended on rimsky-internal harness packages (`pkg:graph/scenario`, `pkg:foundation/*`, `pkg:internal/pgmigrate`, `pkg:runtime/peer`, `pkg:control/config`) are now `t.Skip` stubs with `TODO: 2026-05-24-repo-reorganization — needs rimsky image bring-up harness` comments; full bring-up against a published rimsky image is future work (depends on rimsky-services CI publishing rimsky-core images). The `rimskyservices` build tag was stripped on the move. (4) **Operators upgrading past this point must pull bundled services as separate images from rimsky-services** (`ghcr.io/rimsky-ai/rimsky-services/<service>` or a chosen registry). Rimsky's `file:deploy/` still references in-tree image names today; Pass 6 updates those compose fragments to point at the published rimsky-services images and restricts `file:deploy/build-images.sh` to rimsky-core images only. **Verification:** `cmd:make lint && cmd:make build-all && cmd:make test-all` clean in rimsky; `cd ../rimsky-services && cmd:go build ./... && cmd:go test ./...` clean in rimsky-services.

- **Repo-reorganization P3a of `spec:2026-05-24-repo-reorganization-design`: scenario-test audit + testfixture refactor.** Pass 4 of `plan:2026-05-24-repo-reorganization`. (1) **Test audit landed at `file:.ok-planner/plans/2026-05-24-repo-reorganization-test-audit.md`** (workflow scratch; archived with the plan). Categorizes every `_test.go` file that imports `pkg:stores/{filesystem,postgres}` plus every file under `test/scenarios/stores/`, `test/scenarios/atomic_staging/`, and `test/smoke/` into REWRITE-TO-STUB (default rule per spec P3.4), MOVE-TO-RIMSKY-SERVICES (Pass-5 destination), MOVE-TO-RIMSKY-DOCS (Pass-7 destination), and out-of-scope (stub-conformance tests staying in rimsky). Audit confirms the REWRITE-TO-STUB column needs no actual rewriting work: every test in that category was already authored against `pkg:stores/stub` from the start. (2) **`pkg:stores/filesystem/testfixture` and `pkg:stores/postgres/testfixture` refactored to wrap `pkg:stores/stub`.** The two testfixture packages no longer import their sibling production packages (`stores/{filesystem,postgres}/{cmd,server,store,lifecycle}`), so when Pass 5 relocates those production trees to `pkg:github.com/rimsky-ai/rimsky-services`, the testfixture packages (staying in rimsky as test-infrastructure carve-outs per spec P3.1) continue to compile. Public surface preserved: each testfixture exposes a `Config` struct with the same field names as before plus a local `PickPolicy` type (mirroring the production store's `PickPolicy` shape) so call-site signatures don't break; the filesystem testfixture's directory-scan still discovers `<Root>/<sub>/*` folders and seeds them as stub-store queue items. The postgres testfixture accepts `Connection`, `WriteSemantics`, `EnableExecutor` etc. for source compatibility and ignores them — the stub-store substrate has no executor role or admin surface. Tests that genuinely need postgres or filesystem semantics belong in rimsky-services. (3) **MOVE-TO-RIMSKY-SERVICES tests parked behind a `//go:build rimskyservices` build tag pending the Pass-5 move.** Nine files carry the tag: the three `test/scenarios/stores/fs_*_test.go` (need real fs), the two pg-verifier tests under `test/scenarios/atomic_staging/` that hit the postgres-store testfixture, and all five files in `test/smoke/` (the `BringUpStack` harness is fs+pg-bound). Two other MOVE files stay un-tagged because they don't depend on the testfixture or its types: `test/scenarios/atomic_staging/pg_verifier_test.go` (pure wire-shape test), `test/scenarios/bundled_executor_vocab_test.go` (imports pg-store directly, still compiles pre-Pass-5). The tag excludes the parked files from default `go build` and `go test`; Pass 5 strips the tag when each file lands in `../rimsky-services/test/`. Audit document records the precise per-file disposition. **Verification:** `cmd:make lint && cmd:make test-all` clean; `grep -E '"github\.com/rimsky-ai/rimsky/stores/(filesystem|postgres)/(cmd|server|store|lifecycle)' stores/filesystem/testfixture/testfixture.go stores/postgres/testfixture/testfixture.go` returns no matches.

- **Repo-reorganization P2 of `spec:2026-05-24-repo-reorganization-design`: SDK birth + `runtime/remote` → `runtime/peer` rename + conformance reorg + in-tree canaries.** (1) **New `pkg:sdk/go` peer Go module** added to `file:go.work` alongside `protocols/` and `foundation/`, carrying the implementer-facing surface for service authors: server scaffolding (`pkg:sdk/go/server` — extracted from `pkg:stores/internal/bridge` and the inlined sensor/subscriber/executor cmd/main.go boilerplate), publisher-side helpers (`pkg:sdk/go/publisher` — extracted from `pkg:sensors/internal/post`), conformance library (`pkg:sdk/go/conformance` — one sub-package per protocol, claimproducer / executor / blobbackend / dataprocessing / publisher / validation), testcontainer helpers (`pkg:sdk/go/testpg` — extracted from `pkg:internal/pgtest`, split off the migrations-applying surface which stays rimsky-internal at `pkg:internal/pgmigrate`), and ops glue (`pkg:sdk/go/ops` — slog setup, healthcheck, DSN env-var parser). New `sdk-purity` depguard rule in `file:.golangci.yml` keeps `pkg:sdk/go` clean: imports only `protocols/` + stdlib + minimal third-party (`go-chi/chi`, `pgx/v5`, `testcontainers-go`, `google.golang.org/grpc`). `foundation-purity` and `graph-purity` extended to deny `pkg:sdk/go`. `pgx-isolation` allowlist extended to include `pkg:sdk/go` so testpg can use pgx. (2) **`pkg:runtime/remote` → `pkg:runtime/peer` rename** (breaking change for any external consumer; pre-v1 license applies). The old name implied an externally-facing surface, but the package is rimsky-internal infrastructure tightly coupled to `concept:supervisor`, `concept:terminal-resolution`, and `concept:discovery-cache` — "peer" matches the `concept:service` vocabulary better. `git mv runtime/remote runtime/peer`; package declarations updated; every caller's import + `remote.X` → `peer.X` references swept across `cmd/`, `control/`, `test/scenarios/`. Docstrings + `licensing.yml` + `feature-index.md` + `protocols/proto/v1/claim_producer.proto` references updated; `make proto-gen` regenerated. (3) **Conformance CLI binaries become thin wrappers over `pkg:sdk/go/conformance`.** Runner logic extracted from `pkg:cmd/rimsky-{claim-producer,executor,blob-backend,data-processing,publisher,validation}-conformance/main.go` into `pkg:sdk/go/conformance/<protocol>/runner.go`. CLI binaries now ~30-100 lines each: parse flags, dial endpoint, invoke library, format output, exit. External Go service authors can invoke conformance from their own Go tests (`go test ./...` against a testcontainers-hosted target) instead of shelling out to the binary. The executor surface lives at `pkg:sdk/go/conformance/executor` with package name `conformance` (preserves the historical `conformance.Register` / `conformance.AwaitTerminal` API for scenario init() registrations). A minimal `Client` / `EventStream` / `Endpoint` surface lives in `pkg:sdk/go/conformance/executor/client.go` (mirrors `pkg:runtime/executor` — kept in semantic lockstep with `@source` annotations because sdk-purity denies importing the rimsky-internal version). The `cmd/rimsky-conformance-probe` sidecar also routes through the SDK surface. The blob-backend conformance CLI adapts the rimsky-internal `persistence.BlobBackend` to the SDK's reduced `Backend` interface so the in-process suite stays sdk-purity-clean. The legacy `conformance/` repo-root directory is retired. (4) **In-tree canary scenarios** added under `pkg:test/scenarios/canary/` to replace what `apps/crimefinder` and the openlineage white-box test caught before P5 / P1 respectively: `template_registration_run_a_pass_test.go` exercises the public control-API surface end-to-end (POST template → POST deploy → POST instance → observe nodes walk to terminal via a node-subscription cascade edge); `lifecycle_subscriber_callback_test.go` drives a fake LifecycleSubscriber peer through all six lifecycle verbs (`OnTemplate{Registered,Deployed,Undeployed,Deregistered}`, `OnInstance{Created,Terminated}`) and asserts each event arrived exactly once with the expected envelope. (5) **Design-doc mutations** applied per spec §Design changes: new `concept:sdk` at `file:.ok-planner/design/concepts/sdk.md`; `concept:module-layout` reshaped to describe the four-Go-module workspace (drops the stale "MCP-server module" claim — operator MCP shim at `pkg:control/controlapi/mcp` is in-tree, part of the root module); `concept:conformance` reshaped to describe the six thin CLI wrappers over `pkg:sdk/go/conformance`; `concept:claim-producer` line 19 + `concept:publisher` lines 23 + 42 updated `runtime/remote/` → `runtime/peer/`; auto-generated `concepts.md` TOC regenerated to include the `sdk` entry and the updated `module-layout` / `conformance` one-liners. **Verification:** `cmd:make lint && cmd:make build-all && cmd:make test-all && (cd sdk/go && go build ./... && go test ./...)` clean.

- **Repo-reorganization audit prep (P1 of `spec:2026-05-24-repo-reorganization-design`).** Lands the five in-repo cleanups that ready the codebase for the `pkg:sdk/go` peer module to be born without inheriting any rimsky-internal leaks from the bundled consumption-side code. (1) **Cosmetic import swap in 11 `stores/` files** — `corestore "github.com/rimsky-ai/rimsky-core/foundation/locks"` → `claimproducer "github.com/rimsky-ai/rimsky-core/protocols/claimproducer"`. The four types (`Capabilities`, `ClaimResult`, `OpenOutcome`, `WriteSemantics`) are `type X = claimproducer.X` aliases at `code:foundation/locks/types.go:102,109,116,161`; the two constants (`WriteSemanticsSync`, `WriteSemanticsStagedAsync`) are re-declared at lines 129,134 because Go disallows const aliasing. Semantic-preserving in both cases; `pkg:foundation/locks` continues to own `NamedLockSpec` and other rimsky-internal types. (2) **`code:subscribers/openlineage/subscriber_test.go` rewrite — white-box → integration.** The prior shape used `code:subscribers/openlineage/subscriber_test.go::seedInstanceWithMainScope` to INSERT into `table:rimsky_instances` and `table:rimsky_run_scopes` directly (with deferrable-FK gymnastics) and imported `pkg:foundation/persistence` + `pkg:foundation/shared` + `pkg:internal/pgtest` to apply rimsky migrations. The rewrite drives a vanilla Postgres container via `testcontainers-go` directly, installs only the subscriber's read-contract DDL (a minimal `rimsky_lineage` table with the columns `code:subscribers/openlineage/subscriber.go::fetchSince` queries — no FKs to rimsky internals, no `record_kind` CHECK constraint), and exercises the polling subscriber's actual public contract: read lineage rows, emit OpenLineage events to an httptest backend. Same correctness assertions (event-type discrimination, run/job naming, cursor persistence, tie-breaker by `(observed_at, id)`, decode-failure cursor-advance, emit-failure halt, cursor migration repair), driven from outside rimsky's internal table layout. The wire-contract reflection tests (`TestLeafRunRecord_*`, `TestClaimTerminalRecord_*`) survive unchanged — they have no DB dependency. (3) **Three sensor tests swap to no-migrations Postgres.** `code:sensors/sensor-http/state_db_test.go`, `code:sensors/sensor-webhook/state_db_test.go`, `code:sensors/sensor-object-store/state_db_test.go` switched from `pgtest.StartPostgres` (which applied rimsky migrations as a side effect) to `pgtest.StartFreshPostgresDSN` (no migrations); each sensor's own `openStateDB` migrates the per-sensor tables it owns. (4) **Deleted two empty `cmd/rimsky-verifier-*` directories** (`cmd/rimsky-verifier-http`, `cmd/rimsky-verifier-shape-checks`). (5) **New `consumption-side-isolation` depguard rule in `file:.golangci.yml`** denies imports of `pkg:foundation/`, `pkg:internal/`, `pkg:graph/`, `pkg:runtime/`, `pkg:control/`, `pkg:cmd/` from `stores/`, `sensors/`, `subscribers/`, `executors/`. Patterns root-anchored intentionally (`stores/**` not `**/stores/**`) so the rule does not match rimsky-internal scenario tests under `test/scenarios/stores/`; P3.4 will categorize those tests for rewrite/move. After P3 lands, the rule's target paths no longer have local matches; the rule stays as a defensive guard against re-bundling. **Verification:** `cmd:make lint && cmd:make test-all` clean; `cmd:go test ./subscribers/openlineage/... ./sensors/sensor-http/... ./sensors/sensor-webhook/... ./sensors/sensor-object-store/... ./stores/... -count=1` clean.
...
- **2026-05-18 sensor-messaging-unification review fixes.** Post-merge
  ...
  - New publisher-side retry-with-backoff helper at
    `pkg:github.com/rimsky-ai/rimsky-core/sensors/internal/post`; all four
...
- **Cycle-4 review cleanup (2026-05-17).** Four issues from the
  ...
  - `code:runtime/lineage_writer_test.go::cascadeFreshForTest` retired
    ...
    direct import of `pkg:github.com/rimsky-ai/rimsky-core/foundation/cascade`
...
- **`docs/executor-author-guide.md` rewritten** for the new module layout (external Go authors import `github.com/rimsky-ai/rimsky-core/protocols/executor`). References the service-protocol contract. YAML examples updated to Option II shape. Async-callback path documented (POST `${callback_url}/v1/callback/{async_ack_id}` body keyed `type` — not `kind`). Phase 3's region→scope deferral folded in.
- **`docs/store-author-guide.md` renamed** (via `git mv`) to `docs/claim-producer-author-guide.md` and rewritten as "Writing a Claim Producer". External Go authors import `github.com/rimsky-ai/rimsky-core/protocols/claimproducer` (and `protocols/lifecycle` if implementing both). YAML config: `claim_producers:` block with per-peer `protocols:` and `write_semantics_envelope:`. Conformance: `rimsky-claim-producer-conformance --endpoint <yourservice>:7000`. Note clarifies that "store" is the colloquial term for data-backed producers; the protocol-level term is "claim producer". CLAUDE.md and other docs updated to the new path.
...
- **Three Go modules established.** `github.com/rimsky-ai/rimsky-core/foundation`,
  `github.com/rimsky-ai/rimsky-core/protocols`, and the root `github.com/rimsky-ai/rimsky-core`.
