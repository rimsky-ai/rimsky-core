...
- **TS claude-agent async-callback body key.** The TS executor must POST to `${callback_url}/v1/callback/{async_ack_id}` with the body keyed `type` (not `kind`) — enforced by the Go supervisor's chi route. End-to-end test in `pkg:github.com/fallguyconsulting/rimsky-services/executors/claude-agent/src/server.test.ts`.
