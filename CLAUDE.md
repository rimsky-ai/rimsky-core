...
- **`on_event` handlers are validated at template registration against the executor's `Capabilities.declared_events`.** When the executor's capabilities are not visible at registration time (peer unreachable), the cross-check is skipped silently — runtime treats unknown event names as no-ops.
