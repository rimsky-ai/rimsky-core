...
**Public docs (cite from external/agent material)** — `docs/concepts/`, `docs/protocols/`, `docs/agents/llms.txt`, `docs/humans/landing.md`, `docs/glossary.md`.
...
**TypeScript executor** — `executors/claude-agent/` has its own toolchain: `cd executors/claude-agent && npm install && npm test && npm run build`.
...
- **TS claude-agent async-callback body key.** The TS executor must POST to `${callback_url}/v1/callback/{async_ack_id}` with the body keyed `type` (not `kind`) — enforced by the Go supervisor's chi route. End-to-end test in `executors/claude-agent/src/server.test.ts`.
