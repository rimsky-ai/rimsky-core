...
// HTTP+JSON bridge. The types are intentionally local to the SDK —
// rimsky's supervisor uses an equivalent (`pkg:runtime/executor`) for
// production dispatch, but the SDK cannot import that package
// (`sdk-purity` denies it). The two surfaces are kept in semantic
...
// HTTP transport is not yet supported in the SDK conformance client;
...
func (p *ClientPool) GetOrCreate(ep Endpoint) (Client, error) {
	...
	case "grpc":
		...
		return c, nil
	default:
		return nil, fmt.Errorf("conformance.ClientPool: unknown transport %q (sdk conformance supports grpc only; use rimsky-internal runtime/executor for HTTP bridges)", ep.Transport)
	}
}
