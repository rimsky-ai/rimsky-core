...
export interface PromptLoaderInput {
  mission: string;
  systemPromptFromAttributes?: string;
  userPromptTemplateFromAttributes?: string;
}
...
const FALLBACK_PROMPTS: Record<string, ResolvedPrompts> = {
  "review-zone": {
    systemPrompt:
      ...
      "attributes.system_prompt; this is a safety-net fallback. Call review_context to learn your " +
  ...
  "fix-cycle": {
    systemPrompt:
      ...
      "attributes.system_prompt; this is a safety-net fallback. Call review_context to load your " +
  ...
  "re-review": {
    systemPrompt:
      ...
      "attributes.system_prompt. Re-survey the affected zone, focusing on regressions from fixes.",
    userPrompt: "Call review_context to begin.",
  },
  dedup: {
    systemPrompt:
      "You are crimefinder's dedup agent. Your rich prompt belongs in the template's " +
      "attributes.system_prompt. Identify duplicate findings in your assigned file groups.",
    userPrompt: "Call review_context to begin.",
  },
};
...
export function loadPrompts(input: PromptLoaderInput, logger: Logger): ResolvedPrompts {
  const sys = input.systemPromptFromAttributes?.trim() ?? "";
  const usr = input.userPromptTemplateFromAttributes?.trim() ?? "";
  if (sys && usr) return { systemPrompt: sys, userPrompt: usr };

  const missing = !sys && !usr ? "both" : !sys ? "system" : "user";
  logger.warn(
    { event: "prompt_missing_from_attributes", mission: input.mission, missing },
