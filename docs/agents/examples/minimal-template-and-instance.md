...
The bundled `executor-stub` is launched in stub mode (`RIMSKY_EXECUTOR_STUB_MODE=1`); the stub returns a canned `Complete` event keyed only on `node_type`, ignoring `userdata` for behavior selection.
