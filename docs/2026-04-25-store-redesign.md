...
- Design notes, 2026-04-25.
- Builds on (does not replace) `docs/2026-04-25-stores-redesign.md`, which is the in-flight implementation. That doc remains the authoritative starting point for the work currently underway. This doc captures a follow-on conversation about (a) extending the store concept to enable a third-party ecosystem and (b) refining the lock primitive surface presented to graph authors.
- Authored as conversation notes for use in a future session. The user is intentionally NOT changing code while the prior redesign's implementation is in flight. Nothing here should land before that work is complete.
...
- Three modes (direct, sidecar, versioned).
- Claim stores (queues, ring buffers) as a configuration of one `claim_store` kind.
- Attributes replacing inputs/outputs/claim metadata; rimsky owns all `{{...}}` substitution.
- A six-commit implementation sequence; v0 hard break at commit 4.
...
3. **Versions, semantically.** The prior design references versions in passing. This conversation worked through what the orchestrator actually needs from versions (very little) versus what the substrate may offer (a lot).
...
The recommendations here are **compatible** with the prior redesign's machinery. They're a vocabulary and surface refinement plus an additive extension (the bridge protocol) — not a contradiction of the underlying architecture.
...
- Version pinning (GC gating against active readers).
- Implicit data-structure semantics (queues, ring buffers — fall out of the locking + sequencing primitives).
...
- **Region** = `(store, selector)`. One concept. Selectors range from fully static (file path, table name, explicit key) to fully dynamic (predicate over rows, glob over files). There is **no separate type** of region — selector dynamism is an axis, not a partition.
- **Claim** = "node X is operating on region R at version N in mode M." Always present; always pins a version against GC. The version pin is what prevents premature cleanup; everything else is the mode.
...
- **Commit** = action that resolves a write-mode claim and advances the version.
...
|---|---|---|
| `none` | All others | No exclusion; just version pin. Pure-MVCC reads use this. |
| `sync` | Nothing | Mutex against all other claims on the region. |
| `async-read` | Other async-read, async-write | Blocks against sync. Stable view of a version. |
...
| Lock kinds (`named`, `region`, `claim`) | The redesign keeps named locks for cross-cutting concurrency budgets (rate limits). This doc renames those to "concurrency budgets" for clarity but keeps the mechanism. Region locks become "claims with mode r/rw." Claim locks (queue/ring-buffer item acquisition) stay distinct. |
...
1. **Predicate stability** for dynamic-membership selectors. Sub-detail of `read_during_write` capability. Probably manifest field or validation-time check.
...
3. **Inlined-vs-standalone convergence.** Whether to migrate the blessed defaults onto the bridge protocol (one path everywhere) or keep two paths (cheaper to ship). The CHANGELOG already flags the hardcoded `"inline-jsonb"` lookup as a post-v1 issue; bridge unification resolves it. Decision: defer until after the prior redesign lands.

4. **Subgraph-level read consistency.** Multi-node consistent views. Three modeling options in §11. Probably a v2 expansion; v1 just needs to keep `AcquireRead/ReleaseRead` as the primitive so subgraph-scope leases can layer on top.

5. **Bridge-vs-scheduler enforcement of read leases.** For `read_during_write: block` substrates, scheduler-mediated mutex is preferable to bridge-mediated stall (no idle executor processes). For `read_during_write: async` substrates with reader-lease serialization, bridge-side enforcement is unavoidable. Worth documenting which path each substrate takes in the bridge author guide.
...
### Settled across both sessions

- **Claim is the only primitive.** Lock disappears as a separate concept.
- **Graph DSL exposes `r` and `rw` only.** Sync/async lives in store config, not graph syntax.
- **Store config: `read_during_write: async | block`.** Same vocabulary at every level (store default, per-region override, bridge manifest).
- **Three-row collision matrix.** r-vs-r never blocks; rw-vs-rw always blocks; r-vs-rw depends on store config.
- **Versions are substrate's business.** Orchestrator only tracks change-signal + GC pins.
...
- **Multi-tenant stores** (§15): graphs self-service logical sub-namespaces within ops-provisioned substrates via an optional bridge admin verb set (`ProvisionSubstore`, `DestroySubstore`, `ListSubstores`, `GetSubstoreUsage`). The IaC boundary holds — Rimsky carves namespaces, ops provisions resources. Single-tenant bridges remain valid via `supports_provisioning: false`.
...
- **Don't break the prior redesign's machinery.** Everything here is compatible with the landed stores work. If the implementation of this doc requires touching that work's core mechanisms, back up and re-evaluate — the conversation arrived at a refinement, not a replacement.
...
- **Resist over-specifying the manifest.** Add fields only when the orchestrator actually consumes them. Each capability flag is a contract the bridge author must honor and the orchestrator must validate; the cost compounds.
...
- **The claim payload audit is the next task, but it's engineering hygiene rather than a security blocker.** Encrypt-before-pass (§14.5) is what prevents secret exposure under Rimsky compromise; the audit hardens inertness for the cases the policy doesn't cover (non-secret-but-private content, ciphertext accumulation, third-party bridges, compliance regimes — see §14.5 Layered defense). Any leak (a debug log of payload content, a schema validator that touches payload values, an event-log entry that includes payload, a span that attaches payload as an attribute) breaks inertness; the fix for each is local (redact / mark sensitive), but the discovery requires a careful sweep across every consumer of `ClaimResult.Payload`.

### What's deliberately not in this doc

- **Implementation sequence for the bridge protocol.** Premature; the prior redesign's six commits land first.
- **Specific manifest schema (YAML/proto).** Worth doing once predicate-stability and concurrent-writer-safety questions land.
- **Migration path from inlined to standalone for existing stores.** Per the project's "pre-v1, break freely" rule, the answer is probably "rewrite, don't migrate" once the bridge protocol exists.
- **Specific bridge implementations.** Filesystem, postgres, redis, mongo, git, S3. Each gets its own design when prioritized.
- **Discovery / package registry mechanics.** OCI is the obvious answer; specifics deferred.
...
This doc is the conversational state of the design as of 2026-04-26 (auth section added). Ready to inform the opacity audit and any future implementation of the bridge protocol after the prior redesign's work lands.
