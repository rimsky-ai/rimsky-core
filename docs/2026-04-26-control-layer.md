...
This doc is about **everything else** the control-api does. Specifically, the store-side state lifecycle and the auth mechanics that surround instance creation. The architectural backbone:
...
2. For each dependency that requires provisioning, calls the store-service's store-kind-specific namespace operation.
3. Records the provisioned namespace in `rimsky_substores`.
4. Registers a runtime store config that points at the provisioned namespace (store-service endpoint + namespace identifier).
...
If a store-kind accepts new state, it has SOME isolation primitive — schema, key prefix, directory, keyspace, branch — that can be used to carve a namespace. The store-kinds that don't support provisioning are read-only sources, and they don't need it.

So provisioning isn't a per-store capability flag. It's a baseline expectation for writeable stores. The interesting work is per-store-kind: how does each store-kind's isolation primitive (rooted in its underlying storage technology) express each lifecycle shape?
...
### 1.4 Per-store-kind "store rules" matrix

How each store-kind expresses each lifecycle shape (rooted in its underlying storage technology):

| Store kind | Isolation primitive | Per-instance | Per-DAG | Per-tenant |
|---|---|---|---|---|
...
The naming conventions above are illustrative. Spec session lands on canonical formats per store-kind; store-services document their format in their manifest.

Store-kind-specific quirks worth highlighting:
...
- **Create namespace.** Store-kind-specific. The store-service holds admin credentials; carves the isolation primitive.
- **Grant runtime access.** Store-kind-specific. The store-service configures permissions so the runtime user (or a per-instance scoped credential) can read/write the namespace.
- **Bind to runtime store registration.** Common across store-kinds. Produces the runtime-visible store config (store-service endpoint + namespace identifier) and registers it in the runtime's store registry.
- **Destroy on uninstall.** Store-kind-specific. `DROP SCHEMA`, S3 prefix delete, `rm -rf`, `DEL` keys, branch delete.
...
CREATE TABLE rimsky_substores (
    ...
    parent_store    TEXT NOT NULL,            -- the parent store's registered name
    namespace       TEXT NOT NULL,            -- store-kind-specific identifier
...
Read by control-api during install (idempotency check) and uninstall (cleanup orchestration); not read by runtime. The store is the ground truth on namespace existence; this table is the bookkeeping layer that lets control-api know what to clean up later, what scope a namespace belongs to, and whether to destroy on uninstall.

### 1.7 Store-service admin verbs

To execute provisioning, store-services expose store-kind-specific admin operations control-api invokes:
...
The runtime endpoint of the store-service (the verb set per the current contract: `Open` / `Commit` / `Abandon` / `Release` plus `Capabilities()`) is unchanged by provisioning. Provisioning happens through the admin endpoint; runtime ops happen through the runtime endpoint.

### 1.8 Quotas and capacity

Out of scope for v1. Underlying-storage quotas (postgres connection limits, S3 storage caps, filesystem disk space) are operator concerns and are enforced by the underlying technology when reached. If the orchestrator needs to participate in quota enforcement (refuse to provision when capacity is low, throttle writes near limits), that's a v2 extension.
...
### 1.9 Cross-store transactional install

A graph with substores from 3 different store-services where 2 succeed and 1 fails: handled by **idempotent retry + opt-in cleanup**.

- Install is retry-safe by (parent_store, scope_kind, scope_id). On failure, re-running install picks up where it left off.
- Cleanup is explicit: `POST /v1/instances/{id}/uninstall` walks the `rimsky_substores` rows for this scope and calls `DestroyNamespace` per store-service.
- No 2-phase commit. The store-side state is the source of truth; the bookkeeping is best-effort eventual.
...
| Control-api → store-services (admin endpoint) | Provisioning operations | Admin-role credentials; firewalled to control-api only |
| Control-api → store-services (runtime endpoint) | N/A | Runtime path is supervisor → store-service, not control-api |
...
- **Tenant identity propagation and isolation.** How tenant identity arrives in an instance-creation request, how it flows to the store-service for namespace selection, how it's enforced at runtime claim time.
- **Credential rotation and federation.** Store-service admin creds rotated by ops; control-api credentials rotated by ops; runtime user credentials managed per-substore.
- **Audit logging of admin operations.** Durable trail of who provisioned/destroyed what, when.
- **Relationship to the auth-blind philosophy** in `docs/2026-04-25-store-redesign.md` §14: the runtime is auth-blind (transports bytes, doesn't understand auth); the control layer IS auth-aware (authenticates clients, holds store-service admin creds, enforces scope). The boundary is precise — runtime never sees auth state; control-layer never reaches into runtime decisions. Spec needs to formalize the boundary.
...
1. **Provisioning spec (§1).** Most of the design space is enumerated. Spec work:
   - Settle the canonical naming formats per store-kind (the §1.4 matrix is illustrative).
   - Specify the `rimsky_substores` schema and migrations.
   - Specify the `ProvisionNamespace`/`DestroyNamespace`/`ListNamespaces` admin verb set on the store-service protocol.
   ...
   - Per-store-kind "store rules" sections — postgres/s3/filesystem/redis/git in detail.
   - Out-of-scope reminder: quotas, capacity planning, cross-store 2PC.
