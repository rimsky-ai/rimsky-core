...
- **CLI**: `rimsky tag create`, `rimsky tag mv`, `rimsky tag list`, `rimsky tag rm`.
...
- Tags reserved by `rimsky compose` (the `compose:<project>:<...>` namespace) are rejected client-side when manually created. Compose owns project-prefixed tags.
...
- Manually creating a tag with the `compose:<project>:<...>` prefix. The CLI rejects this client-side; that namespace is reserved for `rimsky compose up`.
