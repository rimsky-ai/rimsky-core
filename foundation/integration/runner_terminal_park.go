...
// Park-terminal handler — applies the protocol-level ParkRequested
// terminal event. Per the 2026-05-08 platform-extensions plan E1:
//
//   - Logs WARN when ParkRequested.reason is empty (permitted, discouraged).
//   - Persists park metadata (parked_at, resume_at, parked_reason,
//     session_token) to rimsky_worker_request.
...
// applyTerminalPark handles the ParkRequested terminal event. Persists
// the park metadata, spills large payloads, transitions the worker_request
...
func applyTerminalPark(
	...
	if err := args.Persist.Transaction(ctx, func(ctx context.Context, tx persistence.Tx) error {
		...
		if err := args.Persist.Nodes().UpdateState(ctx, acq.NodeID,
			shared.NodeStateParked, cascade.ReasonHandlerPark, "", tx); err != nil {
			return err
		}
...
// for a worker_request, applying the per-row override → deployment
