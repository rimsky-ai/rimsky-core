...
func (c *Client) Open(ctx context.Context, claimID locks.ClaimID, spec locks.ClaimSpec) (locks.OpenOutcome, error) {
	...
	if !c.caps.Contains(rws) {
		return locks.OpenOutcome{}, fmt.Errorf("remote producer %q: Open: realized_write_semantics %q not in advertised envelope %v", c.name, rws, c.caps.WriteSemanticsEnvelope)
	}
...
func (c *Client) ValidateCapabilities(declared locks.Capabilities) error {
	if len(declared.WriteSemanticsEnvelope) == 0 {
		return fmt.Errorf("remote producer %q: operator-declared write_semantics_envelope is empty", c.name)
	}
	for _, want := range declared.WriteSemanticsEnvelope {
		if !c.caps.Contains(want) {
			return fmt.Errorf("remote producer %q: capabilities mismatch — operator declared %v, producer advertised %v",
				c.name, declared.WriteSemanticsEnvelope, c.caps.WriteSemanticsEnvelope)
		}
	}
	return nil
}
...
// writeSemanticsToProto is the reverse of writeSemanticsFromProto. Used
// by producer servers (under stores/) when constructing replies.
func writeSemanticsToProto(ws locks.WriteSemantics) genv1.WriteSemantics {
	switch ws {
	case locks.WriteSemanticsSync:
		return genv1.WriteSemantics_WRITE_SEMANTICS_SYNC
	case locks.WriteSemanticsStagedAsync:
		return genv1.WriteSemantics_WRITE_SEMANTICS_STAGED_ASYNC
	case locks.WriteSemanticsBlockingAsync:
		return genv1.WriteSemantics_WRITE_SEMANTICS_BLOCKING_ASYNC
	case locks.WriteSemanticsReadOnly:
