...
// fakeBlobOrphansStore is a tiny in-memory BlobOrphansStore used by the
// sweep test. It tracks rows and deletes; backend interaction goes
// through fakeBlobBackend.
type fakeBlobOrphansStore struct {
	mu   sync.Mutex
	rows []persistence.BlobOrphanRow
}

func (f *fakeBlobOrphansStore) Insert(_ context.Context, row persistence.BlobOrphanRow, _ persistence.Tx) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeBlobOrphansStore) DueBefore(_ context.Context, cutoff time.Time, limit int) ([]persistence.BlobOrphanRow, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeBlobOrphansStore) Delete(_ context.Context, handle string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	out := f.rows[:0]
	for _, r := range f.rows {
