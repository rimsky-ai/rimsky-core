...
func TestRowCountRatio_PassesOnRatioMet(t *testing.T) {
	...
	passed, details, err := ev.Evaluate(context.Background(), EvalInput{
		NewData:      newRows,
...
func TestRowCountRatio_SkippedWhenNoPrevious(t *testing.T) {
	...
	passed, _, err := ev.Evaluate(context.Background(), EvalInput{
...
func TestCustomRuleNotRegistered_ReturnsError(t *testing.T) {
	specs := []Spec{{Type: "custom", Config: map[string]any{"handler": "my_handler"}, Severity: shared.SeverityError}}

	_, _, err := EvaluateAll(context.Background(), specs, EvalInput{NewData: []map[string]any{}})
	require.Error(t, err)
	require.Contains(t, err.Error(), "custom")
}

func TestUnknownRuleType_ReturnsError(t *testing.T) {
	specs := []Spec{{Type: "bogus_never_registered", Severity: shared.SeverityError}}
...
func TestEvaluateAll_PartitionsBySeverity(t *testing.T) {
	...
	errs, warns, err := EvaluateAll(context.Background(), specs, EvalInput{NewData: rows})
	require.NoError(t, err)
