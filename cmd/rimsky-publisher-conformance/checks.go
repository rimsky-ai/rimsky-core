...
// checks.go carries the Publisher conformance check battery.
...
// RunPublisherConformance drives the lifecycle + (optional) message
// push checks against the supplied Publisher client.
...
// does not outlive WaitForMessage's stack frame (the original
// implementation spawned one watchdog per cond-wait iteration without
// stopping the timer, leaking goroutines in long-running harness
// runs).
