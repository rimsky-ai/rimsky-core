...
// main_test.go drives the Publisher conformance suite against an
// in-process Publisher server that mimics the bundled sensor-cron
// shape: a single supported kind "cron", a poll-loop that fires
// on the per-subscription interval.
...
func TestPublisherConformance_FixtureCron(t *testing.T) {
	...
	results := RunPublisherConformance(context.Background(), client, opts)
...
func startReceiver(t *testing.T, r *MessageReceiver) (endpoint string, teardown func()) {
	t.Helper()
