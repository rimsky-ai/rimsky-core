...
func executorScript(t *testing.T) dispatchHandler {
	...
	return func(protocol string, payload []byte) [][]byte {
		...
		if err := proto.Unmarshal(payload, &req); err != nil {
			t.Errorf("daemon: unmarshal execute request: %v", err)
		}
...
func TestExecuteHappyPath(t *testing.T) {
	...
	connectFakeDaemon(t, ts, "owner-1", "http://127.0.0.1:7777", executorScript(t))
...
func TestExecuteCallbackRewrite(t *testing.T) {
	...
	connectFakeDaemon(t, ts, "owner-1", "http://127.0.0.1:7777", handler)
	...
	if seenCallback != "http://127.0.0.1:7777/v1/callback/ack-1" {
		t.Fatalf("callback not rewritten to daemon host: %q", seenCallback)
	}
}
...
func TestExecuteMissingServiceName(t *testing.T) {
	ts := newProxyTestServer(t, nil)
	connectFakeDaemon(t, ts, "owner-1", "", executorScript(t))
	cacheReadyInstance(ts, "inst-1", "owner-1", map[string]bindingSpec{"codegen": {Path: "./c"}})
...
func TestExecuteDaemonNotConnected(t *testing.T) {
	ts := newProxyTestServer(t, nil)
	cacheReadyInstance(ts, "inst-1", "owner-1", map[string]bindingSpec{"codegen": {Path: "./c"}})
	...
	defer cancel()
	outcome := collectExecute(t, client, ctx, &genv1.ExecuteRequest{InstanceId: "inst-1"})
	if got := terminalErrorClass(outcome); got != errClassHostDaemonNotConnected {
		t.Fatalf("expected %s, got %q", errClassHostDaemonNotConnected, got)
	}
}
...
func TestExecuteSpawnFailed(t *testing.T) {
	ts := newProxyTestServer(t, nil)
	fa := connectFakeDaemon(t, ts, "owner-1", "", executorScript(t))
...
func TestExecuteFailsSpawnWhenTheDaemonNeverAcks(t *testing.T) {
	ts := newProxyTestServer(t, nil)
	fa := connectFakeDaemon(t, ts, "owner-1", "", executorScript(t))
...
func TestSpawnAckDeadlineOutlastsTheReadinessDeadlineTheDaemonIsGiven(t *testing.T) {
	cases := []struct {
		name      string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if ack <= ready {
				t.Fatalf("ack deadline %s does not outlast the readiness deadline %s the daemon is given — the "+
					"proxy would abandon the wait as the daemon reports why the child never bound, and the "+
...
func TestExecuteDisconnectMidDispatch(t *testing.T) {
	ts := newProxyTestServer(t, nil)
	fa := connectFakeDaemon(t, ts, "owner-1", "", nil)
	...
	defer cancel()
	outcome := collectExecute(t, client, ctx, &genv1.ExecuteRequest{InstanceId: "inst-1"})
	if got := terminalErrorClass(outcome); got != errClassHostDaemonDisconnected {
		t.Fatalf("expected %s, got %q", errClassHostDaemonDisconnected, got)
	}
}
...
func TestExecuteSupervisorCancelSendsCancelFrame(t *testing.T) {
	...
	waitFor(t, "the daemon to receive the stalled dispatch, so the cancel lands mid-stream",
