...
func TestRegisterUnverifiedKeyIsRejectedAndCannotDisplace(t *testing.T) {
	srv := authenticatedModeControlAPIStub(t)
	verify := newControlAPIRegisterIdentityVerifier(srv.Client(), srv.URL)
	state, client := newDaemonTestServerWithVerifier(t, verify)
	...
	legitConn, ok := state.lookupDaemon(testOwnerKeyID)
	if !ok {
		t.Fatalf("verified daemon should be registered under its key id")
	}
	...
	if got, ok := state.lookupDaemon(testOwnerKeyID); !ok || got != legitConn {
		t.Fatalf("legit daemon connection must survive rejected register attempts")
	}

	mustSend(t, legit, &genv1.ClientFrame{Body: &genv1.ClientFrame_Heartbeat{Heartbeat: &genv1.HostDaemonHeartbeat{SentAtUnixMs: 7}}})
	frame, err := legit.Recv()
	if err != nil || frame.GetHeartbeatAck() == nil {
		t.Fatalf("legit daemon stream must stay live after rejected registers: err=%v frame=%T", err, frame.GetBody())
	}
}

func TestAdoptRoutingIdentityRejectsSameLabelCollision(t *testing.T) {
	state := newProxyState()
	srv := newDaemonServer(state, presentedKeyIsIdentity)
	...
	if got, ok := state.lookupDaemon(label); !ok || got != first {
		t.Fatalf("first anonymous daemon must still be registered after the rejected collision")
	}
}
