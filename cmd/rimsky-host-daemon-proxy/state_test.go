...
func TestRegisterDaemonDisplacesPrior(t *testing.T) {
	s := newProxyState()

	first, prior, collided := s.registerDaemon("key-1", "host-a", "http://127.0.0.1:5001", registerDisplacePrior)
	if collided || prior != nil {
		t.Fatalf("first register should not collide: collided=%v prior=%v", collided, prior)
	}

	second, prior2, collided2 := s.registerDaemon("key-1", "host-b", "http://127.0.0.1:5002", registerDisplacePrior)
	if collided2 {
	...
	if got, ok := s.lookupDaemon("key-1"); !ok || got != second {
		t.Fatalf("lookupDaemon should return the latest connection")
	}
}

func TestRegisterDaemonRejectOnCollisionKeepsPrior(t *testing.T) {
	s := newProxyState()

	first, _, collided := s.registerDaemon("sparkling-wombat", "alpha", "", registerRejectOnCollision)
	if collided || first == nil {
		t.Fatalf("first register must succeed: collided=%v first=%v", collided, first)
	}

	second, prior, collided2 := s.registerDaemon("sparkling-wombat", "beta", "", registerRejectOnCollision)
	if !collided2 {
	...
	if got, ok := s.lookupDaemon("sparkling-wombat"); !ok || got != first {
		t.Fatalf("prior connection must still be the registered daemon after a rejected collision")
	}
}

func TestRegisterDaemonDropsPriorSpawnsOnDisplacement(t *testing.T) {
	s := newProxyState()
	s.registerDaemon("key-1", "host-a", "", registerDisplacePrior)
	...
	s.registerDaemon("key-1", "host-b", "", registerDisplacePrior)
...
func TestDropDaemonOnlyEvictsCurrent(t *testing.T) {
	s := newProxyState()
	first, _, _ := s.registerDaemon("key-1", "a", "", registerDisplacePrior)
	second, _, _ := s.registerDaemon("key-1", "b", "", registerDisplacePrior)

	s.dropDaemon("key-1", first)
	if got, ok := s.lookupDaemon("key-1"); !ok || got != second {
		t.Fatalf("dropping prior must not evict successor")
	}

	s.dropDaemon("key-1", second)
	if _, ok := s.lookupDaemon("key-1"); ok {
...
func TestDeliverDispatchOverflowDoesNotBlock(t *testing.T) {
	conn := newDaemonConnection("owner-1", "label", "")
	streamID := "stream-1"
...
func TestConcurrentAccessSafety(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			...
			conn, _, _ := s.registerDaemon(key, "label", "", registerDisplacePrior)
