...
// checks.go carries the DataProcessing conformance check battery.
// Exported so the in-process self-test can drive the same set without
// re-implementing the wire dial layer.
...
// CheckResult is one row of conformance output. Err is nil on pass.
type CheckResult struct {
	Name string
	Err  error
}

// RunDataProcessingConformance drives the DataProcessing conformance
// checks against the supplied client. Each check is independent; a
// failure does not short-circuit so the operator sees the full
// surface.
...
// trip per advertised materialization. Stub-style materializations
// don't take per-materialization parameters in the proto; the check
// simply pins that each entry in caps.Materializations supports the
// minimal round-trip without errors.
...
// checkListVersionsSmoke commits one candidate then probes
// ListVersions for the claim_handle. Asserts at least one version
// is returned and the version_id is non-empty.
...
// into distinct versions. The producer is required to serialize
// commits per claim_handle (the proto contract carries an implicit
// version_id ordering); the check asserts the ListVersions output
