...
func printRootUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  run {<file>|--template <name>}")
	fmt.Fprintln(w, "                        Register, deploy, instantiate in one shot; self-hosts")
	fmt.Fprintln(w, "                        rimsky in-process when no endpoint resolves")
	...
	fmt.Fprintln(w, "  instance create | list | get | status | kill | nodes | events")
	fmt.Fprintln(w, "  instance delete <id-or-key>")
	...
	fmt.Fprintln(w, "  auth login                   Interactive: reads URL and api-key from the terminal")
	fmt.Fprintln(w, "  auth create-key --name=... {--role=...|--role-file=...} [--expires=...]")
	fmt.Fprintln(w, "                               [--add <action> ...] [--remove <action> ...]")
