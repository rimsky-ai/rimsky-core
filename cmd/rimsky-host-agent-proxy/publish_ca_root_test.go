...
func TestLocalLeafHolderReusesTheCAItPublishedOnAnEarlierRun(t *testing.T) {
	...
	if _, err := leaf.Verify(x509.VerifyOptions{Roots: pool, CurrentTime: at, DNSName: enroll.PeerServerName}); err != nil {
		t.Fatalf("a restarted proxy must serve a leaf the root every running agent pinned verifies: %v", err)
	}
...
func TestLocalLeafHolderKeepsItsCAKeyPrivate(t *testing.T) {
	...
	if info.Mode().Perm() != 0o600 {
		t.Fatalf("the CA key is mode %v, want 0600: anyone who reads it mints a leaf every agent trusts",
			info.Mode().Perm())
	}
	...
	if root.Mode().Perm() != 0o644 {
		t.Fatalf("the published root is mode %v, want 0644: every agent reads it", root.Mode().Perm())
	}
}
...
func TestLocalLeafHolderMintsAFreshCAWhenTheKeptPairDoesNotMatch(t *testing.T) {
	...
	if string(republishedRoot) == string(pinnedRoot) {
		t.Fatal("a key that does not match the kept root must produce a fresh CA and a republished root; " +
			"reusing the root serves leaves no pinned agent can verify")
	}
...
func TestPersistLocalCADropsBothFilesWhenItCannotWriteTheRoot(t *testing.T) {
	...
	if _, err := newLocalLeafHolder(time.Now, caPath); err == nil {
		t.Fatal("a proxy that cannot keep its CA must fail, not serve a root no agent can read")
	}
	if _, err := os.Stat(caPath); !os.IsNotExist(err) {
		t.Fatalf("a failed persist must leave no root at the path an agent pins; stat = %v", err)
	}
