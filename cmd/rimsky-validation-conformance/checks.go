...
// checks.go drives the Validation conformance test battery. Each
// check is independent; failures do not short-circuit so the operator
// sees the full surface in one run.
//
...
// RunValidationConformance drives the conformance suite for the
// given role. Each role has its own happy-path and malformed-input
// probes; the dispatcher routes on the role string.
...
// context. The check asserts: the RPC returns without error, and the
// response's `valid` field carries a definite boolean (no panic).
...
// but no executor context. The check asserts the validator surfaces
// an error finding rather than crashing or panicking.
