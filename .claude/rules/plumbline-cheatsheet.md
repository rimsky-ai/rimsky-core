...
- Project config lives in `.ok-plumbline/plumbline.json` (optional; a legacy root `.plumbline.json` is still honored until migrated). The `citations` array adds project-specific structured-tag exemptions (each pairs a tag with a resolution rule); `ignore` adds paths to skip.
