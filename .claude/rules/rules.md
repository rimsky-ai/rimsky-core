...
- Design questions and proposals go into the issue queue (`.ok-planner/issues.jsonl`, append-only `open` rows) to be drained at `/sprint`; ad-hoc working documents live in `.ok-planner/workbench/` with a YYYY-MM-DD prefix.
