...
- **Retire by measured context, only at an item boundary.** Each task notification carries the worker's token count (`subagent_tokens`): the context it carries now, not a running tally. It can dip between rounds; read the current number each time. Retire a worker when it finishes an item and its last round exceeded the threshold, and spawn a replacement so N holds. Hold the threshold below the harness's compaction window — ~300k tokens on a 1M-token window; scale it on smaller windows — so the hand-off is the ceremony's, with a record it kept, and never the harness's summary. Nothing can steer a subagent's compaction or force a hand-off mid-item.
