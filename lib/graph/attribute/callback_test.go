...
func (f *fakeAttributesStore) GetByRun(_ context.Context, runID shared.UUID) (*Row, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	r, ok := f.rows[runID]
	if !ok {
...
func (f *fakeAttributesStore) Upsert(_ context.Context, runID, nodeID shared.UUID, data map[string]any) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.rows[runID] = &Row{RunID: runID, NodeID: nodeID, Data: cloneMap(data)}
	return nil
}

func (f *fakeAttributesStore) MergeDelta(_ context.Context, runID shared.UUID, delta map[string]any) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	r, ok := f.rows[runID]
	if !ok {
		return errors.New("no row for run")
	}
	for k, v := range delta {
		r.Data[k] = v
	}
...
func mountHandler(t *testing.T, deps HandlerDeps) *httptest.Server {
	t.Helper()
	r := chi.NewRouter()
	r.Method(http.MethodPost, "/v1/runs/{run_id}/attributes", Handler(deps))
	srv := httptest.NewServer(r)
	t.Cleanup(srv.Close)
	return srv
}

func postDelta(t *testing.T, srv *httptest.Server, runID shared.UUID, token string, body any) *http.Response {
	t.Helper()
	buf, err := json.Marshal(body)
	if err != nil {
		t.Fatalf("marshal body: %v", err)
	}
	req, err := http.NewRequest(http.MethodPost, srv.URL+"/v1/runs/"+runID.String()+"/attributes", bytes.NewReader(buf))
	if err != nil {
...
func TestHandler_HappyPath(t *testing.T) {
	...
	resp := postDelta(t, srv, runID, "tok-123", map[string]any{
	...
	row, err := store.GetByRun(context.Background(), runID)
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	if row == nil {
...
func TestHandler_BearerPrefixAccepted(t *testing.T) {
	...
	runID := uuid.New()
	nodeID := uuid.New()
	_ = store.Upsert(context.Background(), runID, nodeID, map[string]any{})
	...
	resp := postDelta(t, srv, runID, "Bearer tok-bearer", map[string]any{"delta": map[string]any{}})
	if resp.StatusCode != http.StatusNoContent {
		t.Fatalf("status: want 204 got %d", resp.StatusCode)
	}
}
...
func TestHandler_BadRunID(t *testing.T) {
	...
	resp, err := http.Post(srv.URL+"/v1/runs/not-a-uuid/attributes", "application/json",
...
func TestHandler_InvalidJSON(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/v1/runs/"+uuid.New().String()+"/attributes",
