...
func (s *nodeAttributesImpl) SnapshotBagForNewRun(
	...
	if priorHandle.Valid {
		handleArg = priorHandle.String
	}
	if priorHandleBackend.Valid {
		backendArg = priorHandleBackend.String
	}
	if _, err := s.q(tx).ExecContext(ctx,
		`INSERT INTO rimsky_node_attributes
		 ...
		 VALUES (?, ?, ?, ?, ?, ?, ?)
		 ON CONFLICT(node_run_id) DO NOTHING`,
		newRunID.String(), nodeID.String(), priorData, priorData,
...
func (s *nodeAttributesImpl) GetPriorRunData(
	...
	row := s.q(tx).QueryRowContext(ctx,
		`WITH cur AS (
		   SELECT n.node_id AS node_id, n.run_scope_id AS run_scope_id, n.sequence AS sequence, n.enqueued_at AS enqueued_at, s.parent_run_id AS parent_run_id
		     FROM rimsky_node_runs n
		     JOIN rimsky_run_scopes s ON s.id = n.run_scope_id
		    WHERE n.id = ?
		 ),
		 same_scope AS (
		   SELECT a.node_run_id, a.node_id, a.data, a.updated_at, a.value_handle, a.value_handle_backend, 1 AS ord
		     FROM rimsky_node_attributes a
		     JOIN rimsky_node_runs r ON r.id = a.node_run_id
		 ...
		 cross_instance AS (
		   SELECT a.node_run_id, a.node_id, a.data, a.updated_at, a.value_handle, a.value_handle_backend, 2 AS ord
		     FROM rimsky_node_attributes a
		     JOIN rimsky_node_runs r ON r.id = a.node_run_id
		     JOIN cur ON r.node_id = cur.node_id AND r.id <> ?
		         AND r.enqueued_at < cur.enqueued_at
		         AND r.state IN ('fresh','failed')
		 ...
		 SELECT node_run_id, node_id, data, updated_at, value_handle, value_handle_backend
		   FROM (
		     SELECT * FROM same_scope
		     UNION ALL
		     SELECT * FROM cross_instance
		   )
		  ORDER BY ord ASC
		  LIMIT 1`,
