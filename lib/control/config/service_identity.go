...
// @concept: service-auth
func installServiceIdentity(ctx context.Context, tables persistence.Tables, principal string, clock shared.Clock, logger shared.Logger) (*service.IdentityHolder, *pki.CA, func(), error) {
	if clock == nil {
...
func setupOutboundIdentity(ctx context.Context, ca *pki.CA, principal string, clock shared.Clock, logger shared.Logger) (*service.IdentityHolder, error) {
	holder := service.NewIdentityHolder()
	...
	if err != nil {
		return nil, fmt.Errorf("service identity for %q: %w", principal, err)
	}
	if err := holder.Set(certPEM, keyPEM, notBefore, notAfter); err != nil {
		return nil, fmt.Errorf("service identity for %q: %w", principal, err)
	}
	service.SetClientIdentity(holder)
	service.SetTLSRootCAs(ca.CertPool())
	go service.MaintainIdentity(ctx, holder, enroll, clock.Now, identityRenewalCheckInterval, func(err error) {
		logger.Warn("SERVICEAUTH.IDENTITY.RENEWFAILED", "principal", principal, "error", err.Error())
	})
	return holder, nil
}
