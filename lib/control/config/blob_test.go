...
func TestOpenBlobBackend_Inline(t *testing.T) {
	...
	bb, err := OpenBlobBackend(persistence.DefaultBlobConfig(), db)
	if err != nil {
...
func TestOpenBlobBackend_MemoryGatedByProcessRole(t *testing.T) {
	db := openMigratedSQLite(t)
	t.Setenv(persistence.ProcessRoleEnv, "")
	_, err := OpenBlobBackend(persistence.BlobConfig{Backend: "memory"}, db)
	if err == nil {
		t.Fatal("want error opening memory backend outside unified role, got nil")
	}
	if !errors.Is(err, persistence.ErrInvalidBlobConfig) {
		t.Fatalf("want errors.Is(persistence.ErrInvalidBlobConfig), got %v", err)
	}
	...
	t.Setenv(persistence.ProcessRoleEnv, "unified")
	bb, err := OpenBlobBackend(persistence.BlobConfig{Backend: "memory"}, db)
	if err != nil {
		t.Fatalf("OpenBlobBackend with unified role: %v", err)
	}
	if bb.Name() != "memory" {
...
func TestOpenBlobBackend_Filesystem(t *testing.T) {
	db := openMigratedSQLite(t)
	bb, err := OpenBlobBackend(persistence.BlobConfig{Backend: "filesystem", Filesystem: persistence.FilesystemBlobConfig{Root: t.TempDir()}}, db)
	if err != nil {
		t.Fatalf("OpenBlobBackend: %v", err)
	}
...
func TestOpenBlobBackend_PgLargeObjectRequiresPostgresDriver(t *testing.T) {
	db := openMigratedSQLite(t)
	_, err := OpenBlobBackend(persistence.BlobConfig{Backend: "pg-largeobject"}, db)
	if err == nil {
