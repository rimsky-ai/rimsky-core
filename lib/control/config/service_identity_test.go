...
func TestOutboundIdentity_MTLSPresentsClientCert(t *testing.T) {
	t.Cleanup(func() { service.SetClientIdentity(nil) })
...
func TestOutboundIdentity_MTLSPinsDeploymentRootCAs(t *testing.T) {
	...
	cfg := service.TLSClientConfig()
	if cfg.RootCAs == nil {
		t.Fatal("production mtls wiring must pin the deployment CA into RootCAs (nil means core verifies services against the system pool)")
	}
...
func TestInstallServiceIdentity_ConcurrentInstallersShareOneRenewingIdentity(t *testing.T) {
	key := make([]byte, 32)
	for i := range key {
		key[i] = byte(i + 1)
	}
	...
	var (
		wg       sync.WaitGroup
		holders  [installers]*service.IdentityHolder
	...
	for i := 0; i < installers; i++ {
		...
		go func(i int) {
			defer wg.Done()
			<-start
			holders[i], _, releases[i], errs[i] = installServiceIdentity(ctx, db.Tables(), fmt.Sprintf("role-%d", i), shared.SystemClock{}, shared.SilentLogger{})
		}(i)
	}
	close(start)
	wg.Wait()
