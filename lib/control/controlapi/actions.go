...
//
// Every Route.Path carries the `/v1/` version prefix; the chi router
// mounts the entire control-API surface under `/v1/` (see
// concept:control-api and tension:control-api-version-prefix). The MCP
// catalog dispatches in-process through chi, so the registered route
// patterns must match the live router patterns — otherwise re-entry
// 404s.
//
