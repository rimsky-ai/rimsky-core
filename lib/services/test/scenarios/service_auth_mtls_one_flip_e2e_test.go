...
func TestServiceAuthMTLSOneFlipMutuallyAuthenticatesTheForwardDispatchLegAndTheControlAPIPort(t *testing.T) {
	t.Parallel()
	ctx := context.Background()

	netName := harness.SharedNetworkName(ctx, t)
	rimskyAlias := harness.NextRimskyAlias()
	executorAlias := harness.NextServiceAlias("executor-http-node-mtls")

	h := harness.BringUpRimskyHandle(ctx, t,
		harness.WithExistingNetwork(netName),
		harness.WithRimskyAlias(rimskyAlias),
		harness.WithServiceAuthMTLS(randomCAEncryptionKey(t)),
	...
	if !strings.HasPrefix(ep.BaseURL, "https://") {
		t.Fatalf("under service_auth: mtls the control API must be reached over TLS, got base URL %q", ep.BaseURL)
	}
	...
	if err == nil {
		...
		if resp.StatusCode == http.StatusOK {
			t.Fatalf("the control API port answered plaintext HTTP under service_auth: mtls (body %q); the flip must cover the inbound listener too", body)
		}
	}

	caRootPEM := h.DeploymentCARootPEM(ctx, t)
	apiKey := ep.MintAPIKey(t, "service-auth-mtls-one-flip")
	ep.Bearer = apiKey
	...
	if mode := ep.WaitForExecutorServiceReachable(t, "http-node-remote"); mode != "required" {
		t.Fatalf("the executor entry declared no tls key, so service_auth: mtls must have defaulted it to required; "+
			"the stack reports tls=%q — one flip means one flip", mode)
	}

	templateID := deployScenarioTemplate(t, ep, map[string]any{
		"spec": map[string]any{
			"name":    "service-auth-mtls-one-flip",
			"version": "1",
	...
	instanceID := createScenarioInstance(t, ep, templateID, "ck-service-auth-mtls-one-flip")
	t.Cleanup(func() {
		if t.Failed() {
...
func assertRefusesAnUnauthenticatedClient(t *testing.T, addr, caRootPEM string) {
	...
	issued, err := impostorCA.IssueLeaf("key-01JIMPOSTORSERVICE", time.Now().Add(-time.Hour), pki.LeafTTL)
	if err != nil {
