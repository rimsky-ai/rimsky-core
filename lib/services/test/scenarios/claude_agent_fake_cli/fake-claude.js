...
// `--resume <token>` is mutually exclusive with `--session-id` in the
// real Claude CLI argv (resume restores the prior conversation; a fresh
// spawn assigns a new session). For the session-resume scenario the
// presence of `--resume` is the only signal the fake CLI uses to
// decide turn 1 vs turn N+1 — matching real CLI semantics, since a
// dispatch launched without `--resume` is by definition a fresh
// conversation regardless of any in-container state.
...
// `--session-id` is present on spawn invocations and ABSENT on resume
// invocations — the real Claude CLI accepts one or the other, never both.
// Require either to be set so a malformed argv (neither flag) fails loudly
// rather than silently turning into a fresh conversation.
...
// Per-chain conversation log used by the session_resume scenario.
// The fake CLI keys turns off the `--resume` argv:
...
function chainIDFromPrompt(prompt) {
  // Look for `scenario:session_resume:<chain_id>` and extract whatever
  // follows the second colon up to the next whitespace, period, or end.
  // The default chain id is "default" so a prompt without the suffix
  // still gets a single, well-defined file.
...
async function scenarioSessionResume() {
  ...
  // Turn detection: `--resume <token>` is the wire-level continuation
  // signal. Absence MUST reset the conversation per the real CLI's
  // `--resume`-or-fresh semantics. A test that asserts "turn 1 again"
  // for a fresh-RunScope dispatch relies on this branch firing whenever
  // `--resume` is missing.
  let turn;
  let priorRecall;
  if (resumeToken === "") {
    // Fresh conversation. Reset the log and seed the canonical
    // "name=Alpha" memory the resumed turns will recall.
    turn = 1;
    priorRecall = "";
    writeSessionResumeLog(chainID, { turn: 1, name: "Alpha" });
  } else {
    // Resumed conversation. The prior turn's log MUST be present —
    // an absent log on a resume call means the runtime launched
    // `--resume <token>` without a prior conversation having
    // existed, which is itself a defect worth surfacing.
  ...
  // Write the observed turn + prior_recall via attributes_set so the
  // rimsky-side test can read them off the persisted attribute bag.
  // The `resumed_with` field records the `--resume <token>` value
  // verbatim so the test can additionally assert which prior runId
  // the resume actually targeted (matching carry-forward). The
  // platform stamps session_token: runId on top during onComplete;
