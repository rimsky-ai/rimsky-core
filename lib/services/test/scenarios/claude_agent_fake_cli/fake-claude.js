...
// @deliberate: fake-claude.js — stub Claude CLI for the claude-agent cross-stack proof.
//
...
//                                    plaintext.
//   - "scenario:session_resume[:<chain_id>]"
//                                  → demonstrates real CLI session-resume
//                                    semantics: the fake CLI keys off the
//                                    `--resume <token>` argv to decide
//                                    whether this is turn 1 of a fresh
//                                    conversation or turn N+1 of a
//                                    resumed one, reads the prior turn's
//                                    in-container memory file, and writes
//                                    {turn, prior_recall, resumed_with}
//                                    via attributes_set. The optional
//                                    `:<chain_id>` suffix scopes the
//                                    memory file so a test driving
//                                    parallel chains (e.g. main-graph
//                                    cascade + sub-graph cascade) does
//                                    not let one chain's fresh dispatch
//                                    silently truncate the other chain's
//                                    accumulated state. Used by
//                                    STORY-claude-agent-session-resume's
//                                    services-harness proof. See
//                                    scenarioSessionResume() for the
...
// @constraint: `--resume <token>` is mutually exclusive with
// `--session-id` in the real Claude CLI argv (resume restores the
// prior conversation; a fresh spawn assigns a new session). For the
// session-resume scenario the presence of `--resume` is the only
// signal the fake CLI uses to decide turn 1 vs turn N+1 — matching
// real CLI semantics, since a dispatch launched without `--resume`
// is by definition a fresh conversation regardless of any
// in-container state.
...
try {
  ...
  // @deliberate: the not-signoff scenarios (rate-limited / inline-refused) never reach
...
// @deliberate: emit a couple of NDJSON lines on stdout so the executor's silence-
...
// @constraint: `--session-id` is present on spawn invocations and
// ABSENT on resume invocations — the real Claude CLI accepts one or
// the other, never both. Require either to be set so a malformed
// argv (neither flag) fails loudly rather than silently turning into
...
async function mcpRequest(method, params) {
  ...
  // @deliberate: the streamable-HTTP server may assign a session id on initialize.
  ...
  // @deliberate: streamable-HTTP servers may reply as SSE event stream when the client
  ...
  if (
    ...
    // @deliberate: the terminal response for an id we issued is the last data line
...
async function initializeMcp() {
  ...
  // @deliberate: initialized notification is best-effort.
  try {
    const headers = {
      "Content-Type": "application/json",
      Accept: "application/json, text/event-stream",
    ...
    /* @deliberate: notification is fire-and-forget */
  }
}
...
async function scenarioSignoffOK() {
  ...
  // @deliberate: signoffs is an array of bare base64 signature strings (per the
...
async function scenarioSignoffMissing() {
  // @deliberate: deliberately omit `signoffs` so the gate rejects each attempt;
...
async function scenarioEnvRefWitness() {
  // @deliberate: the witness path is two-pronged:
  ...
  const digest = crypto
    ...
    .digest("hex");
  // @deliberate: also write the resolved header (plaintext) to a witness file the
  ...
  try {
    ...
    // @deliberate: witness file failure does not abort the dispatch — the rimsky-
    // side absent-plaintext check is the primary acceptance.
    process.stderr.write(`fake-claude: witness write failed: ${e}\n`);
  }
  // @deliberate: incremental writeback so the supervisor records the digest as a
  ...
  // @deliberate: sign the value at the configured signoff path (cli_observation) so
...
// @constraint: per-chain conversation log used by the session_resume
// scenario. The fake CLI keys turns off the `--resume` argv:
//   - resumeToken === ""  → fresh conversation. Reset the log to a
//                           single-turn record with name="Alpha".
//   - resumeToken !== ""  → continuation. Read the log, increment turn,
//                           recall the name.
//
// Real Claude CLI semantics: `--resume <id>` restores the prior
// conversation; absence is a fresh one. The fake mirrors that —
// resetting on absence is correct (a fresh dispatch the executor
// launches without `--resume` MUST start at turn 1 even if a prior
// in-scope dispatch left in-container state behind).
//
// Chain scoping: a test that drives MULTIPLE conversation chains in
// the same fake-CLI container (e.g. a main-graph cascade running in
// parallel with a sub-graph cascade) MUST not let one chain's fresh
// dispatch silently truncate the OTHER chain's accumulated state. The
// fake CLI keys its memory file off a `chain:` suffix in the user
// prompt — `scenario:session_resume:<chain_id>`. Absence is treated
// as the single-chain case (`chain_id = "default"`). The file path
// is `/tmp/fake-claude-session-resume-<chain_id>.json`. The
// container runs as a single nonroot user (parent image's USER), so
// /tmp is writable and shared across spawns within one container.
function chainIDFromPrompt(prompt) {
  // @deliberate: look for `scenario:session_resume:<chain_id>` and
  // extract whatever follows the second colon up to the next
  // whitespace, period, or end. The default chain id is "default" so
  // a prompt without the suffix still gets a single, well-defined
  // file.
...
async function scenarioSessionResume() {
  ...
  // @constraint: turn detection — `--resume <token>` is the
  // wire-level continuation signal. Absence MUST reset the
  // conversation per the real CLI's `--resume`-or-fresh semantics.
  // A test that asserts "turn 1 again" for a fresh-RunScope dispatch
  // relies on this branch firing whenever `--resume` is missing.
  let turn;
  let priorRecall;
  if (resumeToken === "") {
    // @deliberate: fresh conversation — reset the log and seed the
    // canonical "name=Alpha" memory the resumed turns will recall.
    turn = 1;
    priorRecall = "";
    writeSessionResumeLog(chainID, { turn: 1, name: "Alpha" });
  } else {
    // @constraint: resumed conversation — the prior turn's log MUST
    // be present. An absent log on a resume call means the runtime
    // launched `--resume <token>` without a prior conversation
    // having existed, which is itself a defect worth surfacing.
  ...
  // @constraint: write the observed turn + prior_recall via
  // attributes_set so the rimsky-side test can read them off the
  // persisted attribute bag. The `resumed_with` field records the
  // `--resume <token>` value verbatim so the test can additionally
  // assert which prior runId the resume actually targeted (matching
  // carry-forward). The platform stamps session_token: runId on top
  // during onComplete;
  // these custom keys ride alongside.
  await callTool("attributes_set", {
    delta: {
      fake_cli_turn: turn,
...
async function main() {
  ...
  if (userPrompt.includes("scenario:signoff_ok")) {
    ...
    // @deliberate: default: a vanilla success so accidental probes don't dangle.
    await callTool("report_complete", {
      changed: true,
