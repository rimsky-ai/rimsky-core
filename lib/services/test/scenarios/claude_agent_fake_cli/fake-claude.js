...
async function scenarioEnvRefWitness() {
  ...
  if (typeof token !== "string" || token.length === 0) {
    fail(
      "scenario:env_ref_witness: VALIDATOR_TOKEN not visible in the agent's own environment — " +
        "the executor's expose-env allowlist did not pass the secret through to the CLI child",
    );
  }
  const resolvedHeader = `Bearer ${token}`;
  const digest = crypto
    .createHash("sha256")
