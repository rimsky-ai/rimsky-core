...
const DISPATCH_CONTEXT_RETRY_LOG_PATH = "/tmp/fake-claude-dispatch-context-retry.json";

function readDispatchContextRetryLog() {
  try {
    const raw = fs.readFileSync(DISPATCH_CONTEXT_RETRY_LOG_PATH, "utf8");
    return JSON.parse(raw);
  } catch {
    return null;
  }
...
function writeDispatchContextRetryLog(state) {
  fs.writeFileSync(DISPATCH_CONTEXT_RETRY_LOG_PATH, JSON.stringify(state));
}

async function scenarioDispatchContextRetryProbe() {
  const result = await callTool("dispatch_context_read", {});
  const arr = result.content;
  const observed = JSON.parse(arr[0].text);
  const log = readDispatchContextRetryLog();
  if (log === null || log.failed !== true) {
    writeDispatchContextRetryLog({ failed: true });
    await callTool("report_error", {
      error_class: "stub/forced_retry",
      payload: { reason: "fake-claude scripted first-attempt failure for retry probe" },
    });
    return;
  }
  await callTool("report_complete", {
...
async function main() {
  ...
  } else if (userPrompt.includes("scenario:dispatch_context_retry_probe")) {
    await scenarioDispatchContextRetryProbe();
