...
func (h *RimskyHandle) DeploymentCARootPEM(ctx context.Context, t testing.TB) string {
	...
	if !ok {
		t.Fatalf("harness: the stack has no deployment CA row — service_auth: mtls stands one up at startup, so its absence " +
...
// @concept: service-auth
func (e RimskyEndpoint) WaitForExecutorServiceReachable(t testing.TB, name string) string {
	t.Helper()
	var last string
	for poll := 1; ; poll++ {
		...
		default:
			...
			if resp.Service.Reachability == "reachable" {
				return resp.Service.TLS
			}
			last = fmt.Sprintf("reachability=%s tls=%s last_error=%s", resp.Service.Reachability, resp.Service.TLS, resp.Service.LastError)
		}
		if poll == 1 {
			t.Logf("harness: waiting for executor service %q to become reachable (first observation: %s) — the helper "+
		...
		//nolint:testwallclock-outcome inter-poll cadence; this loop returns only when the service reports reachable
...
func NextServiceAlias(prefix string) string {
...
func StartEnrollingHTTPNodeExecutor(
	...
	if !strings.HasPrefix(controlAPIURL, "https://") {
		t.Fatalf("harness: StartEnrollingHTTPNodeExecutor expects the stack's internal control-API URL to be https "+
			"under service_auth: mtls, got %q", controlAPIURL)
	}
