...
const serverName = "rimsky-service"

func mtlsIdentity(t *testing.T, ca *mtlstest.CA, apiKey string) *Identity {
	t.Helper()
	srv := ca.EnrollServer(serverName, apiKey)
	t.Cleanup(srv.Close)
	cfg := Config{Mode: enroll.ServiceAuthMTLS, ControlAPIURL: srv.URL, APIKey: apiKey, Label: serverName}
...
func TestNoneModeTLSConfigsNil(t *testing.T) {
	id := &Identity{mode: enroll.ServiceAuthNone, now: time.Now}
	if id.Enabled() {
...
func TestNeedsRenewalUsesInjectedClock(t *testing.T) {
	notBefore := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	notAfter := notBefore.Add(3 * time.Hour)
	id := &Identity{mode: enroll.ServiceAuthMTLS, cert: &tls.Certificate{}, notBefore: notBefore, notAfter: notAfter}
...
func TestNeedsRenewalWhenNoCert(t *testing.T) {
	id := &Identity{mode: enroll.ServiceAuthMTLS}
...
func TestLoadMTLSFailsClosedWithoutInputs(t *testing.T) {
	_, err := Load(context.Background(), Config{Mode: enroll.ServiceAuthMTLS, Label: "x"}, nil, time.Now)
	if err == nil {
...
func TestLoadMTLSFailsClosedWhenEnrollUnreachable(t *testing.T) {
	cfg := Config{Mode: enroll.ServiceAuthMTLS, ControlAPIURL: "http://127.0.0.1:1", APIKey: "k", Label: "x"}
	if _, err := Load(context.Background(), cfg, nil, time.Now); err == nil {
		t.Fatal("unreachable enroll endpoint must fail closed at startup")
	}
}

func TestLoadNoneReturnsDisabledIdentity(t *testing.T) {
	id, err := Load(context.Background(), Config{Mode: enroll.ServiceAuthNone}, nil, time.Now)
	if err != nil {
		t.Fatal(err)
	}
	if id.Enabled() {
...
func TestLoadConfigFromEnvRejectsBogusMode(t *testing.T) {
	t.Setenv(EnvServiceAuth, "on")
	if _, err := LoadConfigFromEnv("svc"); err == nil {
		t.Fatal("bogus RIMSKY_SERVICE_AUTH must error")
	}
}

func TestLoadConfigFromEnvReadsOnlyControlAPIURL(t *testing.T) {
	t.Setenv(EnvServiceAuth, enroll.ServiceAuthMTLS)
...
func TestEnrollmentReachesAnHTTPSControlAPIThroughThePinnedCARoot(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("a service must be able to enroll against a control API serving TLS under service_auth: mtls, "+
