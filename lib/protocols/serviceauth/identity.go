...
var ErrNoIdentity = errors.New("serviceauth: no leaf identity loaded")
...
func (id *Identity) Enabled() bool { return id != nil && id.mode == enroll.ServiceAuthMTLS }
...
func (id *Identity) refresh(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("serviceauth: load leaf keypair: %w", err)
	}
	pool := x509.NewCertPool()
	if !pool.AppendCertsFromPEM([]byte(resp.CARootPEM)) {
		return fmt.Errorf("serviceauth: enroll response ca_root_pem is not a valid certificate")
	}
	id.mu.Lock()
	defer id.mu.Unlock()
	id.cert = &cert
