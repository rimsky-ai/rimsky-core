...
// ClaimProducer wire protocol (renamed from "store-service" under the
// layer-crystallization plan, Phase 4). Four runtime verbs (Open / Commit /
// Abandon / Release) plus one startup-handshake verb (Capabilities).
// HTTP+JSON bridge handlers decode JSON to these messages and call the
// same internal handler the gRPC server uses.
//
// Field shapes:
...
//
// Errors flow as gRPC status codes.
...
// concurrent claims on byte-equal scopes. UNKNOWN is the proto-default
// zero value; producers MUST NOT return it. The
