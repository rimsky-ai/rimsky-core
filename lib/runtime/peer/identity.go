...
const (
	PeerAuthNone = enroll.PeerAuthNone
	PeerAuthMTLS = enroll.PeerAuthMTLS
)

var ErrNoServerIdentity = errors.New("peer: no server identity loaded")
...
func (h *IdentityHolder) Set(certPEM, keyPEM []byte, notBefore, notAfter time.Time) error {
	...
	if err != nil {
		return fmt.Errorf("peer.IdentityHolder.Set: load keypair: %w", err)
	}
	h.mu.Lock()
	defer h.mu.Unlock()
	h.cert = &cert
