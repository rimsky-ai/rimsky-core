...
func TestMutualTLSHandshakeSucceedsUnderDeploymentCA(t *testing.T) {
	...
	serverCfg := TLSServerConfig(PeerAuthMTLS, holderFor(t, ca, serverKeyID), pool)
	if serverCfg == nil {
...
func TestServerRejectsClientCertFromImpostorCA(t *testing.T) {
	...
	SetTLSRootCAsForTesting(pool)
	t.Cleanup(func() { SetTLSRootCAsForTesting(nil) })
	SetClientIdentity(holderFor(t, impostor, clientKeyID))
	t.Cleanup(func() { SetClientIdentity(nil) })
...
func TestClientRejectsServerCertFromImpostorCA(t *testing.T) {
	...
	trusted := ca.CertPool()
	SetTLSRootCAsForTesting(trusted)
	t.Cleanup(func() { SetTLSRootCAsForTesting(nil) })
	SetClientIdentity(holderFor(t, ca, clientKeyID))
	t.Cleanup(func() { SetClientIdentity(nil) })

	serverCfg := TLSServerConfig(PeerAuthMTLS, holderFor(t, impostor, serverKeyID), impostor.CertPool())
	clientCfg := TLSClientConfig()
	clientCfg.ServerName = serverKeyID
...
func TestTLSServerConfigNilWhenPeerAuthNone(t *testing.T) {
	if cfg := TLSServerConfig(PeerAuthNone, NewIdentityHolder(), x509.NewCertPool()); cfg != nil {
