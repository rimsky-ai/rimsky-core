...
func (a *daemon) handleDispatchFrame(ctx context.Context, df *genv1.DispatchFrame) {
	switch df.GetKind() {
	...
	default:
		slog.Warn("HOSTDAEMON.DISPATCHFRAME.UNSUPPORTED",
	...
	default:
		slog.Warn("HOSTDAEMON.DISPATCHPROTOCOL.UNKNOWN", "protocol", df.GetProtocol(), "spawn_id", df.GetSpawnId())
		a.sendDispatchCancel(df)
	}
}

func (a *daemon) dispatchExecutor(ctx context.Context, child *liveChild, df *genv1.DispatchFrame) {
	var req genv1.ExecuteRequest
	if err := proto.Unmarshal(df.GetPayload(), &req); err != nil {
		slog.Warn("HOSTDAEMON.EXECUTORDISPATCH.UNMARSHALFAILED", "spawn_id", df.GetSpawnId(), "error", err)
		a.sendDispatchCancel(df)
		return
	}
	...
	go func() {
		...
		if err != nil {
			slog.Warn("HOSTDAEMON.EXECUTORDISPATCH.RPCFAILED", "spawn_id", df.GetSpawnId(), "error", err)
			a.sendDispatchCancel(df)
			return
		}
		payload, marshalErr := proto.Marshal(outcome)
		if marshalErr != nil {
			slog.Warn("HOSTDAEMON.EXECUTORDISPATCHOUTCOME.MARSHALFAILED", "spawn_id", df.GetSpawnId(), "error", marshalErr)
			a.sendDispatchCancel(df)
			return
		}
...
func (a *daemon) dispatchClaimProducer(ctx context.Context, child *liveChild, df *genv1.DispatchFrame) {
	dispatchCtx, cancel := context.WithCancel(ctx)
	a.registerDispatchCancel(df.GetStreamId(), cancel)
	...
	go func() {
		...
		if err != nil {
			slog.Warn("HOSTDAEMON.CLAIMPRODUCERDISPATCH.FAILED", "spawn_id", df.GetSpawnId(), "verb", df.GetClaimProducerVerb(), "error", err)
			a.sendDispatchCancel(df)
			return
		}
...
func (a *daemon) sendDispatchData(df *genv1.DispatchFrame, payload []byte) bool {
...
func (a *daemon) registerDispatchCancel(streamID string, cancel context.CancelFunc) {
	a.dispatchMu.Lock()
	a.dispatchCancels[streamID] = cancel
	a.dispatchMu.Unlock()
}

func (a *daemon) clearDispatchCancel(streamID string) {
	a.dispatchMu.Lock()
	delete(a.dispatchCancels, streamID)
	a.dispatchMu.Unlock()
}

func (a *daemon) cancelDispatch(streamID string) {
	a.dispatchMu.Lock()
...
func (a *daemon) sendDispatchCancel(df *genv1.DispatchFrame) {
