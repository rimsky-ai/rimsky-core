...
func waitForStatusChildren(t *testing.T, path string, want int) []ChildStatus {
	t.Helper()
	var out []ChildStatus
	awaited.Until(t, fmt.Sprintf("the daemon's status file to report %d spawned child(ren)", want), func() bool {
...
func TestStatusFileReportsSpawnedChildrenByRunScopeAndBinding(t *testing.T) {
	...
	fp := startFakeProxy(t)
	statusPath := filepath.Join(t.TempDir(), "daemon.status")
	connectDaemonToFakeProxy(t, fp, Config{StatusFile: statusPath})
