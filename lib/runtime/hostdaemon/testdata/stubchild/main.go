...
func main() {
	...
	if port == "" {
		fmt.Fprintln(os.Stderr, "stubchild: RIMSKY_DAEMON_PORT unset")
		os.Exit(1)
	}
...
func mtlsServerOptions() ([]grpc.ServerOption, error) {
	if os.Getenv(enroll.EnvServiceAuth) != enroll.ServiceAuthMTLS {
		return nil, nil
	}
	//nolint:testwallclock-pacing the stub bounds its own enrollment call, matching the HTTP client timeout beside it; no test reads the elapsed time
