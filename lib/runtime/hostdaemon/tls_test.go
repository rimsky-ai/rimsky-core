...
func (p *capturingProxy) Connect(stream genv1.HostDaemon_ConnectServer) error {
...
func startTLSProxy(t *testing.T, ca *pki.CA) (addr string, proxy *capturingProxy) {
	...
	genv1.RegisterHostDaemonServer(srv, proxy)
...
func TestDaemonTLSDialTrustsPinnedCAAndCarriesKey(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("daemonTransportCredentials: %v", err)
	}
...
func TestDaemonTLSDialRejectsWrongCAPin(t *testing.T) {
...
func TestDaemonTransportCredentialsDialTLSByDefault(t *testing.T) {
	creds, err := daemonTransportCredentials(Config{})
	if err != nil {
...
// @decision: host-daemon-proxy-tls
func TestDaemonTransportCredentialsPlaintextOnlyBehindTheInsecureSwitch(t *testing.T) {
	creds, err := daemonTransportCredentials(Config{Insecure: true})
	if err != nil {
...
// @decision: host-daemon-proxy-tls
func TestDaemonTransportCredentialsRejectAnUnreadableCARoot(t *testing.T) {
	if _, err := daemonTransportCredentials(Config{TLSCAPath: filepath.Join(t.TempDir(), "absent.pem")}); err == nil {
		t.Fatal("a CA root the daemon cannot read must fail closed rather than fall back to system roots")
	}
}
