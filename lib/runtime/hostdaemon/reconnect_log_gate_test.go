...
func TestReconnectLogGateStopsRepeatingOneErrorAtTheMaximumBackoff(t *testing.T) {
	...
	if !emit || !last {
		t.Fatalf("the daemon must say once that it is going quiet; emit=%v last=%v", emit, last)
	}
...
// @concept: host-daemon
func TestReconnectLogGateSpeaksAgainWhenTheErrorChangesOrTheDaemonConnects(t *testing.T) {
	var gate reconnectLogGate
	...
	gate.reset()
	if emit, _ := gate.decide("x509: proxy certificate is not trusted", reconnectMaxBackoff); !emit {
		t.Fatal("after a successful connect the daemon starts over, so the next failure must reach the log")
	}
}
