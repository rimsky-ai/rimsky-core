...
func (s *silentProxy) Connect(stream genv1.HostDaemon_ConnectServer) error {
...
func startSilentProxy(t *testing.T) (addr string, received <-chan struct{}) {
	...
	srv := grpc.NewServer()
	genv1.RegisterHostDaemonServer(srv, sp)
...
func TestRecvWithTimeout_ReturnsFrameWhenDeliveredBeforeDeadline(t *testing.T) {
	...
	cfg := Config{ProxyURL: fp.addr, APIKey: "k", RegisterAckTimeout: 5 * time.Second, Insecure: true}.withDefaults()
