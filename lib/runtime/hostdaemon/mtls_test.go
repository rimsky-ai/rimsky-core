...
func startDaemonStack(t *testing.T) *daemonStack {
	t.Helper()
	...
	var mu sync.Mutex
	var cur *daemon
	get := func() *daemon { mu.Lock(); defer mu.Unlock(); return cur }
	...
	return &daemonStack{trust: trust, enrollBase: enrollBase, callbackBase: callbackBase, fp: fp, a: a}
}
...
func TestCallbackListenerAcceptsMutualChildRejectsOthers(t *testing.T) {
	...
	foreignClient := &http.Client{Transport: &http.Transport{TLSClientConfig: &tls.Config{
		MinVersion:   tls.VersionTLS12,
		Certificates: []tls.Certificate{foreign.daemonCert},
		RootCAs:      st.trust.caPool,
	}}}
...
func TestLocalReadinessRequiresMTLSHandshake(t *testing.T) {
	...
	if !probeReady(net.JoinHostPort("127.0.0.1", strconv.Itoa(mtlsPort)), trust.dialChildTLSConfig()) {
		t.Fatal("a service completing the mutual-TLS handshake must be deemed ready")
	}
	if probeReady(net.JoinHostPort("127.0.0.1", strconv.Itoa(plainPort)), trust.dialChildTLSConfig()) {
		t.Fatal("a plaintext service must never be deemed ready under mTLS readiness")
	}
}
...
func TestSpawnProvisionsMTLSEnvToChild(t *testing.T) {
	...
	t.Setenv("STUBCHILD_EXEC_ENV_KEY", enroll.EnvServiceAuth)
	...
	if got := readInjectedEnv(t, execLog); got != enroll.ServiceAuthMTLS {
		t.Fatalf("child %s = %q, want %q", enroll.EnvServiceAuth, got, enroll.ServiceAuthMTLS)
	}
