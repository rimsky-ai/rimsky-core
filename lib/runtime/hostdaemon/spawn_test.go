...
func connectDaemonToFakeProxy(t *testing.T, fp *fakeProxy, cfg Config) {
	t.Helper()
	...
	runDaemonInBackground(t, cfg)
	fp.waitConnected(t)
}

func spawnVia(t *testing.T, fp *fakeProxy, sp *genv1.Spawn) *genv1.SpawnAck {
	t.Helper()
	fp.sendToDaemon(t, &genv1.ServerFrame{Body: &genv1.ServerFrame_Spawn{Spawn: sp}})
...
func reapVia(t *testing.T, fp *fakeProxy, spawnID string, graceSec int32) *genv1.Reaped {
	t.Helper()
	fp.sendToDaemon(t, &genv1.ServerFrame{Body: &genv1.ServerFrame_Reap{Reap: &genv1.Reap{
		SpawnId:             spawnID,
...
func TestSpawnRejectedByAllowPaths(t *testing.T) {
	bin := buildStubChild(t)
	fp := startFakeProxy(t)
	connectDaemonToFakeProxy(t, fp, Config{AllowPaths: []string{"/usr/local/bin/*"}})
...
func TestSpawnDeniesSymlinkEscapingAllowedDir(t *testing.T) {
	...
	fp := startFakeProxy(t)
	connectDaemonToFakeProxy(t, fp, Config{AllowPaths: []string{filepath.Join(allowedDir, "*")}})
...
func TestSpawnDeniesRelativeAllowPatternAgainstForeignBinary(t *testing.T) {
	...
	fp := startFakeProxy(t)
	connectDaemonToFakeProxy(t, fp, Config{AllowPaths: []string{"stubchild"}})
	...
	if ack.GetStatus() != genv1.SpawnAck_SPAWN_STATUS_FAILED {
		t.Fatalf("status = %v, want FAILED (a relative allow pattern must anchor to the daemon cwd, not a remote-supplied binding cwd, so it cannot match a foreign same-named binary)", ack.GetStatus())
	}
}
...
func TestSpawnServiceCtxCancelDuringPortWaitIsNotClassifiedAsBindFailure(t *testing.T) {
	...
	go func() {
		_, spawnErr := SpawnService(waitCtx, SpawnServiceParams{
			BinaryPath:   bin,
			Env:          os.Environ(),
			ReadyTimeout: 10 * time.Second,
			portSource: func() (int, error) {
				port, portErr := FreeLocalPort()
				close(picked)
				return port, portErr
			},
		})
	...
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("SpawnService error = %v, want context.Canceled (the caller context, not a readiness timeout)", err)
	}
	if errors.Is(err, errChildDidNotBind) {
...
func TestDispatchClaimProducerVerbFidelity(t *testing.T) {
	...
	if abandonResp.GetKind() == genv1.DispatchFrame_DISPATCH_FRAME_KIND_CANCEL {
		t.Fatalf("abandon dispatch was cancelled (daemon rejected the verb)")
	}
	...
	if releaseResp.GetKind() == genv1.DispatchFrame_DISPATCH_FRAME_KIND_CANCEL {
		t.Fatalf("release dispatch was cancelled (daemon rejected the verb)")
	}
...
func TestConfigDefaults(t *testing.T) {
	...
	if c.DaemonLabel == "" {
		t.Fatal("daemon label should default to hostname-pid")
	}
	idx := strings.LastIndex(c.DaemonLabel, "-")
	if idx < 0 {
		t.Fatalf("daemon label %q does not have a hostname-pid shape (no trailing dash)", c.DaemonLabel)
	}
	pidPart := c.DaemonLabel[idx+1:]
	pid, err := strconv.Atoi(pidPart)
	if err != nil {
		t.Fatalf("daemon label %q pid suffix %q is not numeric: %v", c.DaemonLabel, pidPart, err)
	}
	if pid != os.Getpid() {
		t.Fatalf("daemon label %q pid suffix = %d, want current pid %d", c.DaemonLabel, pid, os.Getpid())
	}
}
