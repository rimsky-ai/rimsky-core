...
const daemonPortEnvVar = "RIMSKY_DAEMON_PORT"

const daemonRoutingLabelEnvVar = "RIMSKY_DAEMON_ROUTING_LABEL"
...
func (a *daemon) runSpawn(ctx context.Context, sp *genv1.Spawn) {
...
func spawnOnPickedPort(ctx context.Context, params SpawnServiceParams, pickPort func() (int, error)) (*SpawnedService, error) {
	...
	env := setEnvVar(params.Env, daemonPortEnvVar, strconv.Itoa(port))
	cmd.Env = env
...
func (a *daemon) handleSpawn(ctx context.Context, sp *genv1.Spawn) *genv1.SpawnAck {
	...
	env = setEnvVar(env, enroll.EnvServiceAuth, enroll.ServiceAuthMTLS)
	...
	env = setEnvVar(env, daemonRoutingLabelEnvVar, a.cfg.RoutingLabel)
	...
	a.childMu.Unlock()
	if hadPrior {
		slog.Warn("HOSTDAEMON.SPAWNID.COLLIDED", "detail", "terminating the still-live prior process before replacing it",
			"spawn_id", spawnID)
	...
	slog.Info("HOSTDAEMON.CHILD.SPAWNED", "spawn_id", sp.GetSpawnId(), "path", execPath, "port", spawned.Port, "protocols", sp.GetExpectedProtocols())
...
func (a *daemon) dialChild(port int) (*grpc.ClientConn, error) {
...
func (a *daemon) resolveBindingPath(path string) (string, error) {
...
func (a *daemon) runReap(reap *genv1.Reap) {
	a.send(&genv1.ClientFrame{Body: &genv1.ClientFrame_Reaped{Reaped: a.handleReap(reap)}})
}

func (a *daemon) handleReap(reap *genv1.Reap) *genv1.Reaped {
	a.childMu.Lock()
	...
	slog.Info("HOSTDAEMON.CHILD.REAPED", "spawn_id", reap.GetSpawnId(), "clean", clean)
...
func (a *daemon) reapAllOrphans(grace time.Duration) {
	a.childMu.Lock()
	...
	slog.Info("HOSTDAEMON.ORPHANEDCHILDREN.REAPING", "detail", "the stream closed", "count", len(orphans), "grace", grace)
...
func spawnFailed(spawnID, msg string) *genv1.SpawnAck {
	return &genv1.SpawnAck{
		...
		Error:   &genv1.HostDaemonError{Class: errClassSpawnFailed, Message: msg},
	}
}
...
func generateSpawnSecret() (string, error) {
	...
	if _, err := rand.Read(buf); err != nil {
		return "", fmt.Errorf("hostdaemon: generate spawn secret: %w", err)
	}
