...
type servicePublisherServer struct {
	genv1.UnimplementedPublisherServer
}

type serviceLifecycleServer struct {
	genv1.UnimplementedLifecycleSubscriberServer
}

type serviceDataProcessingServer struct {
	genv1.UnimplementedDataProcessingServer
}

type serviceValidationServer struct {
	genv1.UnimplementedValidationServer
}

func startAllServiceProtocolsServer(t *testing.T) (addr string, captured *capturedServiceNames) {
	t.Helper()
	...
	genv1.RegisterPublisherServer(srv, servicePublisherServer{})
	genv1.RegisterLifecycleSubscriberServer(srv, serviceLifecycleServer{})
	genv1.RegisterDataProcessingServer(srv, serviceDataProcessingServer{})
	genv1.RegisterValidationServer(srv, serviceValidationServer{})
...
func TestDialPublisher_InstallsServiceNameInterceptor(t *testing.T) {
	addr, captured := startAllServiceProtocolsServer(t)
