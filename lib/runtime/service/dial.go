...
func UnaryDialOptions(name, tlsMode string) []grpc.DialOption {
	return unaryDialOptions(name, tlsMode, ProcessClientCredentials())
}

func unaryDialOptions(name, tlsMode string, creds ClientCredentials) []grpc.DialOption {
	return []grpc.DialOption{
		grpc.WithTransportCredentials(TransportCredentialsFor(tlsMode, creds)),
...
func dialOptions(name, tlsMode string) []grpc.DialOption {
	return dialOptionsWith(name, tlsMode, ProcessClientCredentials())
}

func dialOptionsWith(name, tlsMode string, creds ClientCredentials) []grpc.DialOption {
	return append(unaryDialOptions(name, tlsMode, creds),
...
func Dial(ctx context.Context, name, endpoint, tlsMode string) (*Client, error) {
	return DialWith(ctx, name, endpoint, tlsMode, ProcessClientCredentials())
}

func DialWith(ctx context.Context, name, endpoint, tlsMode string, creds ClientCredentials) (*Client, error) {
	target, err := StripScheme(name, endpoint)
	if err != nil {
		return nil, err
	}
	conn, err := grpc.NewClient(target, dialOptionsWith(name, tlsMode, creds)...)
	if err != nil {
...
func StripScheme(name, endpoint string) (string, error) {
	if endpoint == "" {
		return "", fmt.Errorf("remote service %q: endpoint is required", name)
	}
	for _, badScheme := range []string{"http://", "https://", "tcp://", "unix://"} {
		if strings.HasPrefix(endpoint, badScheme) {
			return "", fmt.Errorf("remote service %q: endpoint scheme must be grpc:// (got %s)", name, badScheme)
		}
	}
