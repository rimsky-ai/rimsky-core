...
# @deliberate: step: clean — start from an empty ~/.rimsky/config.yml.
echo "step: clean"
rm -f "${HOME}/.rimsky/config.yml"

# @deliberate: step: add-staging — register the first context. After
# this, the staging context exists and (because it's the first) is
# implicitly current.
echo "step: add-staging"
rimsky ctx add staging --endpoint "${STAGING_URL}"

# @deliberate: step: add-prod — register the second context. The current
# context stays on staging (the first one); we'll switch in the next
# step.
echo "step: add-prod"
rimsky ctx add prod --endpoint "${PROD_URL}"

# @deliberate: step: list-after-add — assert both contexts exist with
# their endpoints.
echo "step: list-after-add"
rimsky ctx list

# @deliberate: step: use-staging — switch the active context to staging.
# From this point on, every CLI command without an explicit --endpoint
# hits STAGING_URL.
echo "step: use-staging"
rimsky ctx use staging

# @deliberate: step: ls-instances-staging — observable proof that the
# active-context endpoint resolves to STAGING_URL: `rimsky ls instances`
# hits the staging control-api. A successful exit here means the CLI
# reached the staging stack; if the switch were silently a no-op against
# an unreachable URL the command would error out.
echo "step: ls-instances-staging"
rimsky ls instances

# @deliberate: step: health-endpoint-is-staging — `rimsky health` prints
# the resolved control-api URL on its own line (the `endpoint` key in
# the human KV output and the `endpoint:` field in JSON). After `use
...
# @deliberate: step: use-prod — switch the active context. Subsequent
# commands now hit PROD_URL.
echo "step: use-prod"
rimsky ctx use prod

# @deliberate: step: ls-instances-prod — observable proof that the
# switch is honored by the next command: `rimsky ls instances` now hits
# the prod control-api, not the staging one we hit a moment ago.
echo "step: ls-instances-prod"
rimsky ls instances

# @deliberate: step: health-endpoint-is-prod — same observable as above,
# against the prod URL. Combined with the staging assertion above, this
# is the proof that the active-context switch is picked up by the next
# command (the spec's first falsifier clause). The two URLs differ;
# both assertions cannot pass unless the CLI is actually consulting the
# active context for each subsequent command.
...
# @deliberate: step: current-is-prod — `rimsky ctx current` names the
# active context and its endpoint. After `use prod` it must name prod /
# PROD_URL.
...
# @deliberate: step: rm-staging — remove a non-current context. After
# this, attempts to `ctx use staging` fail (no such context).
echo "step: rm-staging"
rimsky ctx rm staging

# @deliberate: step: rm-staging-no-longer-resolves — the falsifier
# ("removed context still resolves") is exhibited as a failure here: if
# `rm` left `staging` resolvable, the next `ctx use staging` would
# succeed. It must fail with the "not found" diagnostic.
...
# @deliberate: step: done — all steps green.
echo "step: done"
