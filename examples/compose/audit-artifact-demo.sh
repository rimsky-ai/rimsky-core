...
# sqlite3 is required for the post-mortem walkthrough — the story's
# point is that the artifact is in a widely-available format,
# queryable without rimsky-specific tooling, so a fixture pre-flight
# checks for sqlite3 on PATH and bails with a clear message if it is
# missing.
...
# Mixed-outcome manifest → any-failure → exit 1. Anything else means
# the wait loop did not classify the per-instance roster correctly.
[[ "$rc" == "1" ]] || die "FAIL: verb exited $rc; expected 1 (any-failure for mixed-outcome manifest)"

# Post-mortem walkthrough: the operator points at the run directory
# via .rimsky/latest (the symlink the verb updates after apply),
# inspects the per-run files on disk, then opens state.db with the
# stock sqlite3 binary and pulls instance + node-run rows by hand.
...
# Falsifier #1: the run directory must contain state.db AND a blobs/
# subdir. Either one missing is a structural artifact-layout failure.
...
# Falsifier #2a: instance rows must be present, addressable by the
# manifest-supplied instance keys, both terminated.
...
# Falsifier #2b: per-node-run history must be present. The story's
# Falsifier rules out "only state metadata (last-known status flags)
# without per-node-run history". The rimsky_node_runs table carries
# the per-dispatch settled rows; a zero count means the artifact
# captured the instance lifecycle but not the per-run history.
...
# Falsifier #2c: per-node names are recorded — the operator can
# follow the manifest's node type to a per-node-run row.
...
# Falsifier #2d: pull the failing node-run's terminal class out by
# hand — the literal post-mortem walkthrough the story's Proof field
# requires. rimsky_node_runs.phase is the operator-facing terminal
# label per dispatch (CHECK pending|active|held|parked|completed|
# failed). The mixed-outcome manifest lands one row in phase='failed'
# (the `oops` instance's worker, error_class 'stub/failed' from the
# stub executor) and one in phase='completed' (the `ok` instance's
# worker). Operator selects the failing row directly:
