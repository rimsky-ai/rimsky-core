...
# The per-line timestamper reads stderr from the verb and prefixes
# each line with a wall-clock epoch (seconds.nanoseconds). The verb's
# stderr is line-buffered through its slog handler (the default
...
#
# The `script -q /dev/null` indirection on macOS / FreeBSD lets us
# put a pty on the verb's stderr so its line-buffered writes do not
# get block-buffered when piped through another process. Linux's
# `stdbuf -oL -eL` is the equivalent shape; we prefer plain `bash`
# composition by routing through a `while read` loop, which forces
# per-line emission without requiring either.
set +e
...
# Falsifier #1: both `fast: success` and `slow: success` lines must
# be present in the transcript — proves the verb ran the wait loop
# through to both instances' terminals.
...
# Falsifier #2: the fast instance's terminal line must arrive at
# least 1 second BEFORE the slow instance's terminal line. If both
# appeared at roughly the same wall-clock time, the verb buffered
# all lifecycle events to the end of the run — exactly the failure
# mode the story rules out.
...
# bash's [[ ... -gt ... ]] is integer-only; compare via awk to
# admit a fractional threshold.
...
# Falsifier #3 (sanity): the delta should also be bounded above —
# anything beyond 6s suggests the slow path took much longer than
# its delay_ms attribute (3s + 2s slop + 1s wait-poll margin). A
# blown-up delta would mean the slow path didn't actually start
# until after the fast one finished — a serialized execution shape
# we want to rule out.
