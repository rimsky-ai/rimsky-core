...
echo "cascade-send-demo: registering template at ${RIMSKY_ENDPOINT}"
...
# @constraint: every message send must carry an Idempotency-Key (concept:message);
...
# send-node's loop/iterate message opened it); without this the
# cascade exhibited only the wake → A chain and the send-node never
