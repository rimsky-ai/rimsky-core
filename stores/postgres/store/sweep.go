...
// RunSweep starts the substrate-internal visibility-timeout sweep.
// Substrate-internal sweep — runs over the substrate's own data only,
// by design. v3 spec §7.5 makes this independent of rimsky's
// `rimsky_lock_holders` orphan reaper; no cross-database join is
// attempted (the substrate's pool may be on a separate database from
// rimsky's control plane).
//
// Operator timing constraint: set `visibility_timeout > 5 ×
// heartbeat_interval` so the substrate cannot strip a healthy claim
// while rimsky still believes it holds. If `visibility_timeout` is
// shorter, a healthy supervisor whose lock-holder row is still
// refreshing within the orphan-reap window may have its substrate
...
func (s *Store) sweepOnce(ctx context.Context) error {
	for selector, pp := range s.pickPolicies {
		...
		// because the substrate's pool may be separate from rimsky's
		// control-plane DB — and even when colocated, the substrate
		// owns its own state. Per spec §7.5, the orphan reaper runs
		// in rimsky and deletes lock-holder rows; the substrate's
